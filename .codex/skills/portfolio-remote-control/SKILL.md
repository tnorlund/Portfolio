...
3. Pre-loads each session's mission as a **positional prompt** at launch (no fragile TUI paste). You then accept the bypass-permissions prompt in each Terminal (`Down` -> `Yes, I accept` -> `Enter`); the mission runs automatically on accept. That one human accept per agent is the deliberate checkpoint for starting an autonomous bypass-permissions session.
