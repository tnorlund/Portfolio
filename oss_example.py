# oss_example.py  (works with Ollama or vLLM on localhost)

import asyncio
from openai import AsyncOpenAI
from agents import Agent, Runner, function_tool, OpenAIChatCompletionsModel  # <- fixed
...
MAX_AGENT_TURNS = 10
LOG_AGENT_FUNCTION_CALLS = True

# Mock ValidationMethod enum for the example
class ValidationMethod:
    def __init__(self, value):
        self.value = value
        
ValidationMethod.PHONE = ValidationMethod("phone")
ValidationMethod.ADDRESS = ValidationMethod("address")
ValidationMethod.TEXT = ValidationMethod("text")
ValidationMethod.INFERENCE = ValidationMethod("inference")

logger = logging.getLogger(__name__)

client = AsyncOpenAI(  # Ollama default port; change if you proxy
    base_url="http://localhost:11434/v1",
    api_key="ollama",  # any string is fine for Ollama
)


@function_tool
def get_total(price: float, tax: float) -> float:
    return price * (1 + tax)


async def main() -> None:
    agent = Agent(
        name="receipt-labeler",
        instructions="Tag every token with its receipt label.",
        model=OpenAIChatCompletionsModel(
            model="gpt-oss:20b",  # or any local/remote model
            openai_client=client,
        ),
        tools=[get_total],
    )
    result = await Runner.run(agent, "Big Mac $5.34")
    print(result.final_output)


# asyncio.run(main())  # Comment out the simple example

# Example usage of MerchantValidationAgent with OSS model
if __name__ == "__main__":
    # Example: Using the merchant validation agent with local OSS model
    
    # You'll need to provide your Google Places API key
    GOOGLE_PLACES_API_KEY = "your-google-places-api-key-here"
    
    # Create agent using local OSS model
    agent = MerchantValidationAgent(
        google_places_api_key=GOOGLE_PLACES_API_KEY,
        model="gpt-oss:20b",  # or whatever model you have locally
        use_local_oss=True    # Set to False to use OpenAI instead
    )
    ...
    # Run validation
    print("Running merchant validation with local OSS model...")
    metadata, partial_results = agent.validate_receipt(receipt_data)
    
    if metadata:
        print(f"Successfully validated merchant: {metadata}")
    else:
        print("Failed to validate merchant")
        
    if partial_results:
        print(f"Partial results collected: {partial_results}")


class MerchantValidationAgent:
    """Encapsulates the merchant validation agent and its tools."""

    def __init__(self, google_places_api_key: str, model: str = "gpt-oss:20b", use_local_oss: bool = True):
        """
        ...
        Args:
            ...
            use_local_oss: Whether to use local OSS model (True) or OpenAI (False)
        """
        self.google_places_api_key = google_places_api_key
        self.model = model
        self.use_local_oss = use_local_oss
    ...
    def _create_agent(self) -> Agent:
        ...
        # Configure model based on whether we're using local OSS or OpenAI
        if self.use_local_oss:
            model_config = OpenAIChatCompletionsModel(
                model=self.model,
                openai_client=client,  # Uses the local OSS client defined at the top
            )
        else:
