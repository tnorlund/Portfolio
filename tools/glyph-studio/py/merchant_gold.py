#!/usr/bin/env python3
"""merchant_gold.py -- the standing "how close to 1:1 are we" instrument.

ONE command that takes a render + a merchant's gold reference set and emits the
full L0-L4 JSON scorecard (current values, 1:1 targets, pass/fail vs
regression thresholds) defined by ``GOLD_STANDARD.md`` Part 3. It is
merchant-parameterized: the merchant name, gold receipt key, refpack, optional
chart anchor, and threshold bands all come from the ``--config`` /
command-line, so the same instrument scores Costco, Sprouts, or any future
merchant. ``costco_gold.py`` is the frozen Costco entry point (a thin shim over
this module) that keeps the committed Costco baseline record reproducible.

The chart anchor is OPTIONAL: Costco has a published specimen chart
(bitMatrix-C2), so its design->real (``ce``) and render->design (``rc``) rungs
are live; merchants without a chart (e.g. Sprouts) get those chart-anchored
metrics marked ``SKIPPED`` -- never a fabricated number -- and their L1 gate
falls back to the crop-anchored GlyphScore + render->real IoU.

It wires the existing machinery rather than reinventing it:
...
# A metric that is not applicable to this merchant's gold set (e.g. a
# chart-anchored rung for a merchant with no specimen chart). Distinct from
# NOT_IMPLEMENTED (a model we lack) and from None/nan (an evaluable metric that
# could not be computed this run). SKIPPED gates are dropped from level-pass.
SKIPPED = "SKIPPED"
...
def _log(msg: str) -> None:
    print(f"[merchant_gold {time.strftime('%H:%M:%S')}] {msg}", file=sys.stderr, flush=True)
...
def _iou_decomposition(GS, render_path, labels_path, refpack_path, chart_path,
    ...
    ``chart_path`` may be None (merchant with no specimen chart): the
    chart-anchored ``rc`` and ``ce`` columns then come back as NaN and the
    worst-glyph diagnostic ranks by render->real (``re``) instead of
    render->design. The crop-anchored ``re`` column is always live.
    """
    gsc = GS["gsc"]
    ...
    # Rank worst glyphs by render->design (rc) when a chart exists, else by the
    # crop-anchored render->real (re) so the diagnostic stays live chart-free.
    rank_key = "rc" if chart else "re"
    worst = sorted(
        [r for r in rows_hyg if r[rank_key] == r[rank_key]],
        key=lambda r: r[rank_key])[:8]
    if chart:
        # Costco path: unchanged entry shape (rc/ce are live floats).
        worst_out = [
            {"char": r["char"], "rc": round(r["rc"], 3), "ce": round(r["ce"], 3),
             "n": r["n_real"]} for r in worst]
    else:
        # Chart-free path: rc/ce SKIPPED, rank/report the live render->real IoU.
        worst_out = [
            {"char": r["char"], "rc": SKIPPED, "ce": SKIPPED,
             "re": round(r["re"], 3), "n": r["n_real"]} for r in worst]
...
def build_scorecard(args) -> dict:
    ...
    tool_name = getattr(args, "tool", None) or "merchant_gold.py"
    ...
    scorecard = {
        "merchant": getattr(args, "merchant", None) or cfg.get("merchant", "costco"),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--chart", default=None,
                    help="specimen-chart glyphs.npz (optional): enables the "
                         "chart-anchored rc/ce L1 rungs. Omit for merchants "
                         "with no chart (crop-anchored rungs only).")
    ap.add_argument("--config", required=True)
    ap.add_argument("--merchant",
                    help="merchant tag override (default: from --config)")
    ap.add_argument("--tool", default="merchant_gold.py",
...
def run(args) -> int:
    """Build the scorecard from a parsed args namespace, optionally run the
    regression gate, write/print it. Shared by ``main`` and the
    ``costco_gold.py`` shim so both take exactly one code path."""
