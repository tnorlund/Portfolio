...
def known_rule_slugs() -> frozenset[str]:
    """Slugs stylescan has merchant-specific rules for (Sprouts included)."""
    return frozenset(_MERCHANT_RULES) | frozenset(_STYLEMAP_RULE_SLUGS)
        ...
        out_lines.append(
            {
                ...
                "box_h": round(lb - lt, 1),
                # Real-image pixel box [l, t, r, b] so downstream tooling can
                # crop the ACTUAL print for hand-checks (M4 pilot evidence).
                "bbox": [int(ll), int(lt), int(lr), int(lb)],
                "letters": chars,
            }
        )
