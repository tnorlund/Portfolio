...
1. **Create virtual environment with Python 3.14** (matches Lambda runtime):
   ```bash
...
**Missing type hints:**
- Verify Python interpreter is set correctly (should auto-detect Python 3.14 from venv)
...
Test jobs run on GitHub-hosted `ubuntu-latest`. The repository is public, so
standard hosted runners are free and up to 20 jobs run concurrently; a full
pull-request run takes about 2.5 minutes. The 2025 plan that moved CI onto
self-hosted Apple Silicon Macs to save ~$48/month used to live here and is
retired: no workflow references those runners any more. See
[CI/CD Pipeline](ci-cd.md) for the current job layout, runtime baseline and
caching, and the sections above for running tests locally.
