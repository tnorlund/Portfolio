...
Assets are generated by `tools/glyph-studio/py/export_pipeline_assets.py`
(reuses the letterform corpus, the compiled fonts, the production renderer
and the CDN scans; source receipts in
`tools/glyph-studio/fixtures/pipeline_merchants.json`); the frontend
consumes only static files + JSON. The original one-off generator lived in
a scratch worktree and was lost; the CLI is its committed replacement.
