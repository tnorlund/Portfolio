...
## Current Goal: Synthetic Receipt Augmentation

When working on receipt synthesis, keep the full goal in scope: combine the
LLM/pattern-discovery tools, merchant parameterization, local synthetic receipt
generation, LayoutLM training gates, Dynamo training metrics, and frontend
visualization so we can synthesize accurate, robust train-only receipts across
merchants.
Sprouts is the dense first case, not a special-case boundary; generic merchant
contracts and gates should remain the default path for every merchant.

Default to a local, no-spend workflow unless the user explicitly approves paid
API or cloud work:
...
Do not start SageMaker jobs, Step Functions, OpenRouter/OpenAI calls, or other
paid infrastructure for this goal without explicit approval. The deterministic
path should mine existing merchant receipts, generate fewer high-fidelity
examples, and prove candidates with local gates before training uses them.
The current OpenAI latest-model metadata was verified against official OpenAI
developer docs on 2026-06-23 and should be rechecked before updating defaults.

Key implementation areas:

- `receipt_agent/receipt_agent/agents/label_evaluator/merchant_synthesis.py`
  builds generic merchant-local structure, catalog, category, arithmetic, and
  mutable-field synthesis candidates. It records tax-rate evidence but keeps
  tax-changing mutations disabled until LayoutLM gates explicitly support them.
- `receipt_agent/receipt_agent/agents/label_evaluator/sprouts_parameterization.py`
  contains the Sprouts-specific high-fidelity path.
- `scripts/verify_synthetic_replay.py` builds local pattern artifacts, merchant
  contracts, LayoutLM-ready bundles, and synthesis quality reports.
  Inspect `selection.accepted_candidate_examples` in generated bundles to see
  the accepted rank, merchant, operation, structure similarity, candidate
  quality, preview lines, and selection reason without making any API calls.
- `receipt_layoutlm/receipt_layoutlm/data_loader.py` is the final training gate:
  it rejects ungrounded, weak-geometry, unsupported, or arithmetic-invalid
  synthetic examples and keeps validation real-only.
- `receipt_layoutlm/receipt_layoutlm/trainer.py` records accepted synthetic mix
  metrics to Dynamo.
- `receipt_agent/receipt_agent/agents/label_evaluator/augmentation_audit.py`
  compares augmented training runs against baselines and now treats weak
  accepted synthetic candidate quality as a hold/reject signal even if F1 moves.
- `infra/routes/job_training_metrics/handler/index.py`,
  `portfolio/types/api.ts`, and
  `portfolio/components/ui/Figures/TrainingMetricsAnimation/index.tsx` surface
  synthesis evidence in the training metrics UI.

For Claude Code handoff, verify focused changes with:
...
For real-data validation without paid services, point the local pipeline at an
exported receipt JSON directory. Grouped merchant exports work directly; for
ungrouped exports the local loader attaches matching top-level `lines`, `words`,
and `word_labels`, normalizes bounding boxes, ignores invalid labels, and infers
merchant names from `MERCHANT_NAME` labels before falling back to header text.
...
Review accepted candidate examples, merchant contracts, rejection reasons,
structure components, arithmetic reconciliation, `source_receipt_quality`, and
`accepted_mix_balance` before using a bundle for LayoutLM training. The balance
object is the local overtraining/concentration signal: top merchant share, top
operation share, entropy, risk level, and reasons. The synthetic augmentation
audit treats medium/high balance risk as a promotion hold even when target
confusions and F1 improve. If source quality is blocked or limited, fix the
exported receipt/word/label data before tuning synthesis. Blocked source
quality is an enforced contract gate:
that merchant's generated candidates are rejected before they can become
LayoutLM training examples. The synthesis quality report and training-metrics
API surface the same source-quality status, label counts, and operation
blockers so downstream reviews do not have to re-run the bundle builder.
Generated candidate quality also records the nearest-real structure gate:
inspect `structure_similarity.nearest_real_receipt_key`, `shape_deltas`, and
`match_summary.shape_checks` before trusting a candidate. The same evidence now
includes `real_baseline_comparison`, which compares the synthetic structure
score with the merchant's real-to-real receipt variation. `high_fidelity`
requires the minimum structure similarity and component thresholds that the
LayoutLM loader enforces.
