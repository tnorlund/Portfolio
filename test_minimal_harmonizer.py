...
sys.path.insert(0, str(Path(__file__).parent / "receipt_agent"))
...
def test_tools_changes():
    ...
    try:
        ...
        tools_section = content[
            content.find("tools = [") : content.find("return tools, state")
        ]

        has_validate_in_tools = (
            "validate_financial_consistency," in tools_section
        )
        has_submit_in_tools = "submit_harmonization," in tools_section

        print(
            f"validate_financial_consistency in tools: {has_validate_in_tools}"
        )
