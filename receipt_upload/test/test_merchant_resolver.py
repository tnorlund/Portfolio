...
class TestModuleLevelFunctions:
    ...
    def test_redact_pii_masks_phone_and_address(self):
        """Resolver log redaction masks phone/address but keeps the merchant hint."""
        from receipt_upload.merchant_resolution.resolver import redact_pii

        line = (
            "Resolving merchant for X#1 (hint=COSTCO WHOLESALE, "
            "phone=7025551234, address=3411 ST ROSE PKWY HENDERSON NV)"
        )
        out = redact_pii(line)
        assert "7025551234" not in out
        assert "ST ROSE PKWY" not in out
        assert "phone=<redacted>" in out
        assert "address=<redacted>" in out
        assert "COSTCO" in out  # merchant hint (business name) preserved
        # 'none' placeholders carry no PII and are left alone.
        assert "<redacted>" not in redact_pii("phone=none, address=none")

    def test_three_char_distinctive_brand_passes(self):
        """3-char distinctive brand tokens (CVS, IGA, KFC) must pass the guard
        (threshold matches _MIN_TOKEN_LEN=3, not 4)."""
        for brand, line_text in [
            ("CVS Pharmacy", "CVS PHARMACY #4821"),
            ("IGA Supermarket", "IGA SUPERMARKET"),
            ("KFC Restaurant", "KFC RESTAURANT 102"),
        ]:
            lines = [self._make_line(1, line_text, y=0.95)]
            assert merchant_name_matches_receipt(brand, lines), brand

    def test_redact_pii_masks_unprefixed_address_and_paren_phone(self):
        """Raw addresses logged WITHOUT an address= prefix, and parenthesized
        phones, are still masked."""
        from receipt_upload.merchant_resolution.resolver import redact_pii

        out = redact_pii("Tier 2: Similarity search for address line: 123 Main Street")
        assert "123 Main Street" not in out
        assert "<redacted-address>" in out

        out2 = redact_pii("Calling (702) 555-1234 now")
        assert "555-1234" not in out2
        assert "<redacted-phone>" in out2
...
class TestMerchantResolverLabeledFields:
    ...
    def test_places_first_uses_nlp_phone_even_with_pending_name(self):
        ...
        line = MagicMock(spec=ReceiptLine, line_id=1, text="Whole Foods")
        line.calculate_centroid.return_value = (0.5, 0.9)
        labels = [
            _label(1, 1, "MERCHANT_NAME"),  # PENDING — only a hint
            _label(1, 2, "MERCHANT_NAME"),
        ]
        places_result = MerchantResult(
            place_id="ChIJ_whole_foods",
            merchant_name="Whole Foods Market",
            confidence=0.95,
            resolution_tier="place_id_labeled_fields",
    ...
    def test_places_not_attempted_without_phone_or_address(self):
        ...
        line = MagicMock(spec=ReceiptLine, line_id=1, text="Bad Hint")
        line.calculate_centroid.return_value = (0.5, 0.9)
