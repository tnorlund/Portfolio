...
def test_shared_auth_id_plus_content_is_duplicate():
    a = tf(
        "SPROUTS AUTH 337030 MILK 17.99 EGGS 4.50 TOTAL 42.54",
        42.54,
        "Sprouts",
    )
    b = tf(
        "SPROUTS auth 337030 MILK 17.99 EGGS 4.50 total 42.54 xx",
        42.54,
        "Sprouts",
    )
...
def test_reprint_with_extra_text_still_matches():
    orig = tf(
        "TRADER JOES AUTH 204334 COBB 5.99 SALAD 4.99 TOTAL 29.44",
        29.44,
        "Trader Joes",
    )
    reprint = tf(
        "REPRINT TRADER JOES AUTH 204334 COBB 5.99 SALAD 4.99 TOTAL 29.44 COPY",
        29.44,
        "Trader Joes",
    )
...
def test_restaurant_check_number_plus_prices():
    a = tf(
        "PASTA SISTERS Check #239 Salame 14.00 Milanese 19.00 Total 52.93",
        merchant="Pasta Sisters",
    )
    b = tf(
        "Pasta Sisters check 239 Salame 14.00 Milanese 19.00 Total 52.93 reprint",
        merchant="Pasta Sisters",
    )
    ok, why = same_transaction(a, b)
...
def test_disjoint_printed_times_rejected():
    ...
    a = tf(
        "EASTWOOD AUTH 591311 PLATE 9.20 TOTAL 9.20 20:06", 9.20, "Eastwood"
    )
    b = tf(
        "EASTWOOD AUTH 591311 PLATE 9.20 TOTAL 9.20 20:16", 9.20, "Eastwood"
    )
    ok, why = same_transaction(a, b)
...
def test_denylist_excludes_recurring_id():
    ...
    assert not same_transaction(a, b, denylist={"414010"})[
        0
    ]  # excluded -> no match
...
def test_find_duplicate_among_candidates():
    ...
    cands = [
        ("other", "STARBUCKS AUTH 999111 LATTE 5.50 TOTAL 5.50".split()),
...
def test_find_duplicate_none_when_unique():
    ...
    assert (
        find_duplicate(
            new, cands, new_total=7.00, candidate_totals={"x": 3.00}
        )
        is None
    )
