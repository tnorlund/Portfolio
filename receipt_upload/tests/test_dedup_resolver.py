...
def test_survivor_label_wins_no_gap_fill_when_survivor_labels_word():
    ...
    labels = {
        ("a", 1): [
            _obs("MERCHANT_NAME", 1, 1, "M", "VALID"),
            _obs("GRAND_TOTAL", 3, 4, "$5.90", "VALID"),
        ],
        ("a", 2): [
            _obs("SUBTOTAL", 3, 4, "$5.90", "VALID")
        ],  # disagrees on 3:4
    }
...
def test_gap_disagreement_left_unlabeled():
    ...
    labels = {
        ("a", 1): [
            _obs("MERCHANT_NAME", 1, 1, "M", "VALID"),
            _obs("DATE", 8, 8, "x", "VALID"),
            _obs("TIME", 9, 9, "y", "VALID"),
        ],
...
def test_cross_image_gap_fill_targets_concrete_position():
    ...
    assert len(res.gap_fills) == 1
    assert (
        res.gap_fills[0].locus == "4:2"
    )  # the survivor's TAX word, not b's "3:3"
...
def test_legacy_alias_gap_fill_uses_canonical():
    ...
    receipts = [_r("a", 1, "S"), _r("a", 2, "S")]
    words = {
        ("a", 1): {(1, 1): "M", (3, 3): "x", (2, 2): "9.99"},
        ("a", 2): {(1, 1): "M", (3, 3): "x", (2, 2): "9.99"},
    }
    labels = {
        ("a", 1): [
            _obs("MERCHANT_NAME", 1, 1, "M", "VALID"),
            _obs("DATE", 3, 3, "x", "VALID"),
        ],
        ("a", 2): [
            _obs("ITEM_TOTAL", 2, 2, "9.99", "VALID")
        ],  # dropped; survivor lacks 2:2
    }
    res = _resolve(receipts, words, labels)
...
def test_cross_image_gap_fill_requires_survivor_to_have_word():
    ...
    receipts = [_r("a", 1, "S"), _r("b", 1, "S")]
    words = {
        ("a", 1): {(1, 1): "ACME", (2, 2): "MILK"},
        ("b", 1): {(5, 5): "ACME", (6, 6): "EGGS"},
    }
    labels = {
        ("a", 1): [_obs("MERCHANT_NAME", 1, 1, "ACME", "VALID")],
        ("b", 1): [
            _obs("MERCHANT_NAME", 5, 5, "ACME", "VALID"),
            _obs("PRODUCT_NAME", 6, 6, "EGGS", "VALID"),
        ],
    }
    res = _resolve(receipts, words, labels)
