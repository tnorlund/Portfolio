...
async def test_llm_driven_financial_discovery():
    ...
    sample_words = [
        ...
        {"line_id": 11, "word_id": 3, "text": "Unsweetened", "confidence": 0.96},
    ...
    sample_labels = [
        ...
        {"line_id": 8, "word_id": 3, "label": "PRODUCT_NAME", "validation_status": "VALID"},
    ...
    sample_table_structure = {
        ...
        "columns": [
            {"column_id": 0, "type": "product_info", "words": ["365", "Organic", "Bananas", "Avocados", "Large"]},
            {"column_id": 1, "type": "quantity", "words": ["2.5", "lbs", "3", "ea", "1"]},
            {"column_id": 2, "type": "unit_price", "words": ["$1.99/lb", "$1.29", "$4.99", "$6.49"]},
            {"column_id": 3, "type": "line_total", "words": ["$4.98", "$3.87", "$3.49", "$4.99", "$6.49"]},
        ],
        "rows": [
    ...
    if financial_candidates:
        ...
        for financial_type, candidates in financial_candidates.items():
            ...
            for i, candidate in enumerate(candidates, 1):
                print(f"      {i}. Line {candidate.get('line_id')}, Word {candidate.get('word_id')}")
    ...
    if summary:
        print(f"\n📈 Summary:")
        print(f"   - Total financial values: {summary.get('total_financial_values', 0)}")
