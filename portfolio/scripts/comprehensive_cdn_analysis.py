        ...
        if "Contents" in page:
            for obj in page["Contents"]:
                ...
                if "/" in path:
                    ...
                    if filename.startswith("1_") or filename.startswith("2_") or filename == "1.avif" or filename == "1.webp":
...
def check_entity_cdn_mapping(dynamodb, table_name: str, image_id: str) -> Dict[str, Any]:
...
def main():
    ...
    for image_id in subdirectory_images:
        ...
        if dev_result['image_entity']:
            ...
            if dev_result['image_entity']['cdn_fields']:
                logger.info(f"    CDN fields: {', '.join(dev_result['image_entity']['cdn_fields'])}")
                dev_has_cdn += 1
        else:
        ...
        if prod_result['image_entity']:
            ...
            if prod_result['image_entity']['cdn_fields']:
                logger.info(f"    CDN fields: {', '.join(prod_result['image_entity']['cdn_fields'])}")
                prod_has_cdn += 1
        else:
    ...
    logger.info(f"\nS3 CDN Structure:")
    logger.info(f"  Subdirectory pattern (new): {patterns['subdirectory_pattern']['count']} files")
    ...
    logger.info("\nRECOMMENDATION:")
    logger.info("The PROD CDN bucket uses the subdirectory pattern: assets/{image_id}/1_size.ext")
    logger.info("This pattern supports multiple sizes (thumbnail, small, medium) and formats (jpg, webp, avif)")
