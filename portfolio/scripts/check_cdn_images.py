...
def check_cdn_images(table_name: str, cdn_bucket: str, limit: int = 10):
    ...
    for entity in image_entities:
        ...
        logger.info(f"  CDN fields: {sum(1 for field in cdn_fields if entity.get(field) is not None)}/9 populated")
        
        # Check if CDN images actually exist in S3
        expected_cdn_files = [
            f"{image_id}_thumbnail.jpg", f"{image_id}_thumbnail.webp", f"{image_id}_thumbnail.avif",
            f"{image_id}_small.jpg", f"{image_id}_small.webp", f"{image_id}_small.avif",
            f"{image_id}_medium.jpg", f"{image_id}_medium.webp", f"{image_id}_medium.avif"
        ]
        ...
        for field, expected_file in zip(cdn_fields, expected_cdn_files):
            ...
            if file_exists and current_value is None:
                ...
                logger.info(f"  ⚠️  {field}: {current_value} but file missing from bucket")
    ...
    logger.info(f"Entities with populated CDN fields: {stats['entities_with_cdn_values']}")
