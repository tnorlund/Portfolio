...
def check_subdirectory_pattern_for_entity(entity_id: str, entity_type: str, s3_client, cdn_bucket: str, receipt_number: str = None):
...
def main():
    ...
    for entity in images:
        ...
        found_files = check_subdirectory_pattern_for_entity(entity_id, "IMAGE", s3_client, cdn_bucket)
        
        if found_files:
            images_with_subdirectory += 1
            logger.info(f"✅ Image {entity_id}: {len(found_files)} files in subdirectory pattern")
    ...
    for entity in receipts:
        ...
        found_files = check_subdirectory_pattern_for_entity(image_id, "RECEIPT", s3_client, cdn_bucket, receipt_id)
        
        if found_files:
            receipts_with_subdirectory += 1
            logger.info(f"✅ Receipt {image_id}/{receipt_id}: {len(found_files)} files in subdirectory pattern")
            logger.info(f"   Sample files: {found_files[:3]}...")
        else:
            logger.info(f"❌ Receipt {image_id}/{receipt_id}: No subdirectory pattern files found")
    ...
    if images_with_subdirectory > 0 or receipts_with_subdirectory > 0:
        ...
        logger.info("The flat pattern seems to be the correct one for current entities.")
