...
def main():
    ...
    size_stats = {"total_size": 0, "min_size": float("inf"), "max_size": 0, "count": 0}
    ...
    if size_stats["count"] > 0:
        ...
        print(
            f"Total size: {size_stats['total_size']:,} bytes ({size_stats['total_size'] / (1024**3):.2f} GB)"
        )
    ...
    for key, size in receipt_files[:100]:  # Check first 100
        if not re.match(
            r"assets/[a-f0-9-]+_RECEIPT_\d{5}\.(png|jpg|jpeg|avif|webp)$", key
        ):
    ...
    if non_standard:
        ...
        print(
            "All receipts follow standard pattern: assets/{UUID}_RECEIPT_{NUMBER}.{EXT}"
        )
