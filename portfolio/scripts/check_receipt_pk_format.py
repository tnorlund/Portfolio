...
if receipts:
    ...
    if "image_id" in sample_receipt:
        ...
        try:
            response = prod_table.query(
                KeyConditionExpression=Key("PK").eq(f"IMAGE#{image_id}"), Limit=5
            )
            items = response.get("Items", [])
            print(f"   Found {len(items)} items")
            for item in items:
                print(
                    f"   - SK: {item.get('SK', 'N/A')}, TYPE: {item.get('TYPE', 'N/A')}"
                )
        ...
        try:
            response = prod_table.query(
                KeyConditionExpression=Key("PK").eq(f"RECEIPT#{image_id}"), Limit=5
            )
            items = response.get("Items", [])
            print(f"   Found {len(items)} items")
            for item in items:
...
print("\n=== Conclusion ===")
print(
    "Based on the query results above, we can determine the actual PK format for Receipt entities."
)
