                        ...
                        if "_RECEIPT_" in filename:
                            ...
                            if len(parts) == 2:
                                ...
                                if "." in receipt_part:
                                    ...
                                    receipt_patterns[pattern_key].append(
                                        format_ext.lower()
                                    )
...
def check_receipt_cdn_files(
    bucket: str, image_id: str, receipt_id: int
) -> Dict[str, Any]:
    ...
    if result["files_found"]:
        ...
        if base_format:
            result["recommended_mapping"][
                "cdn_s3_key"
            ] = f"assets/{image_id}_RECEIPT_{receipt_id_str}.{base_format}"

        if "webp" in result["formats"]:
            result["recommended_mapping"][
                "cdn_webp_s3_key"
            ] = f"assets/{image_id}_RECEIPT_{receipt_id_str}.webp"

        if "avif" in result["formats"]:
            result["recommended_mapping"][
                "cdn_avif_s3_key"
            ] = f"assets/{image_id}_RECEIPT_{receipt_id_str}.avif"
...
def analyze_receipt_entities(table_name: str, cdn_bucket: str, sample_size: int = 10):
    ...
    for entity in receipt_entities:
        ...
        logger.info(
            f"Receipt IMAGE#{image_id}/RECEIPT#{receipt_id:05d}: "
...
def main():
    ...
    if args.analyze_bucket:
        ...
        logger.info(
            f"Total receipt files found: {bucket_analysis['total_receipt_files']}"
        )
        logger.info(
            f"Unique image IDs with receipts: {bucket_analysis['unique_image_ids']}"
        )
        ...
        for count, num_receipts in sorted(
            bucket_analysis["formats_per_receipt"].items()
        ):
    ...
    if not args.check_specific:
        ...
        entity_results = analyze_receipt_entities(
            table_name, cdn_bucket, args.sample_size
        )
        ...
        if total_entities > 0:
            logger.info(
                f"Already have CDN fields: {already_mapped} ({already_mapped/total_entities*100:.1f}%)"
            )
            logger.info(
        ...
        for i, result in enumerate(entity_results[:3]):
            ...
            logger.info(
                f"Formats available: {', '.join(result['s3_check']['formats'])}"
            )
