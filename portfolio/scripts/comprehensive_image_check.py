...
def comprehensive_image_check(table_name: str, raw_bucket: str, cdn_bucket: str):
    ...
    if results['total_with_any_image'] > 0:
        logger.info(f"✅ {results['total_with_any_image']} entities have image data and can potentially be reconstructed")
        logger.info("   Recommendation: Create reconstruction script for these entities")
        
    if results['no_images_found'] > 0:
        logger.info(f"❌ {results['no_images_found']} entities have NO image data")
        logger.info("   Recommendation: These are phantom entities and should be deleted")
    
    if results['cdn_images_found'] > 0 and results['raw_images_found'] == 0:
        logger.info("⚠️  Found CDN images but no raw images")
        logger.info("   This suggests processing occurred but raw files were cleaned up")
