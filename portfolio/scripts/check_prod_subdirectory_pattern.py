...
def main():
    ...
    if subdirectory_items:
        print(f"⚠️  Found {len(subdirectory_items)} items with subdirectory pattern in PROD!")
    ...
    print(f"\nEntity counts:")
    print(f"  DEV:  {dev_counts.get('IMAGE', 0)} images, {dev_counts.get('RECEIPT', 0)} receipts")
    print(f"  PROD: {prod_counts.get('IMAGE', 0)} images, {prod_counts.get('RECEIPT', 0)} receipts")
    
    if dev_counts.get('IMAGE', 0) != prod_counts.get('IMAGE', 0):
        print(f"\n⚠️  Image counts differ between DEV and PROD!")
        print(f"   DEV has {dev_counts.get('IMAGE', 0) - prod_counts.get('IMAGE', 0)} more/fewer images")
