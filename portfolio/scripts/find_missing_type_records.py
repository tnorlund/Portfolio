...
def find_missing_type_records(table_name: str):
    ...
    while not done:
        ...
        for item in items:
            ...
            missing_records.append({"PK": pk, "SK": sk, "TYPE": record_type})

        # Progress update
        logger.info(
            f"Scanned {total_scanned} records, found {len(missing_records)} missing TYPE..."
        )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Find all records missing TYPE field")
    parser.add_argument(
        "--stack",
        required=True,
        choices=["dev", "prod"],
