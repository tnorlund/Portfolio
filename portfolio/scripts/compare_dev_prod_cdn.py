...
def main():
    ...
    if common_image_ids:
        ...
        for i, img_id in enumerate(sorted(list(common_image_ids))[:5]):
            ...
            if dev_img and prod_img:
                ...
                print(f"\nImage ID: {img_id}")
                print(
                    f"  DEV CDN:  {dev_cdn['cdn_s3_key']} (pattern: {dev_cdn['pattern']})"
                )
                print(
                    f"  PROD CDN: {prod_cdn['cdn_s3_key']} (pattern: {prod_cdn['pattern']})"
                )
    ...
    print(
        f"   - DEV has {len([i for i in dev_images if analyze_cdn_fields(i)['cdn_s3_key']])} images with CDN fields"
    )
    print(f"   - PROD has {prod_images_with_cdn} images with CDN fields")
    print(
        f"   - DEV has {len([r for r in dev_receipts if analyze_cdn_fields(r)['cdn_s3_key']])} receipts with CDN fields"
    )
