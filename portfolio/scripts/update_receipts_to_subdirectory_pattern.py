...
def generate_subdirectory_cdn_fields(
    image_id: str, receipt_number: int, s3_client, cdn_bucket: str
    ...
    logger.info(
        f"  Receipt {image_id}/{receipt_number}: {found_files}/12 CDN files found"
    )
    if found_files > 0:
        size_summary = []
...
def update_receipts_to_subdirectory(
    table_name: str, cdn_bucket: str, dry_run: bool = True
):
    ...
    logger.info(
        f"{'[DRY RUN] ' if dry_run else ''}Updating Receipt entities to subdirectory CDN pattern..."
    )
    ...
    while not done:
        ...
        for entity in items:
            ...
            # Track size statistics
            if (
                size_stats["thumbnail"] > 0
                or size_stats["small"] > 0
                or size_stats["medium"] > 0
            ):
                ...
                if (
                    size_stats["thumbnail"] > 0
                    and size_stats["small"] > 0
                    and size_stats["medium"] > 0
                ):
            ...
            try:
                ...
                logger.info(
                    f"  ✅ {'[DRY RUN] ' if dry_run else ''}Updated receipt with {found_count} CDN files"
                )
    ...
    logger.info("\n" + "=" * 60)
    logger.info(
        f"{'[DRY RUN] ' if dry_run else ''}RECEIPT SUBDIRECTORY PATTERN UPDATE SUMMARY"
    )
    logger.info("=" * 60)
    ...
    avg_files = (
        stats["total_cdn_files_found"] / stats["receipts_updated"]
    ...
    if dry_run:
        ...
        if stats["receipts_updated"] > 0:
            ...
            logger.info(
                f"   {stats['receipts_with_sized_images']} receipts now have sized images!"
            )
