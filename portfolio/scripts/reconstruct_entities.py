...
class EntityReconstructor:
    ...
    def __init__(self, table_name: str, raw_bucket: str, cdn_bucket: str, dry_run: bool = True):
    ...
    def find_corrupted_image_entities(self) -> List[Dict[str, Any]]:
        ...
        while not done:
            ...
            if len(corrupted_images) % 25 == 0 and len(corrupted_images) > 0:
                logger.info(f"Found {len(corrupted_images)} corrupted IMAGE entities so far...")
    ...
    def get_image_dimensions(self, bucket: str, key: str) -> Tuple[Optional[int], Optional[int]]:
    ...
    def reconstruct_image_entity(self, corrupted_image: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def run(self):
        ...
        logger.info(f"Attempting to reconstruct {len(corrupted_images)} IMAGE entities...")
        ...
        logger.info(f"Successfully reconstructed {successful_reconstructions} IMAGE entities")
        ...
        while not done:
            ...
            for item in items:
                ...
                if missing_fields:
                    ...
                    try:
                        image_response = self.table.get_item(Key={'PK': image_pk, 'SK': image_sk})
                        if 'Item' in image_response:
                            ...
                            for field in ['width', 'height', 'timestamp_added', 'raw_s3_bucket', 'raw_s3_key']:
                                if field in image_item and field not in reconstructed_receipt:
                            ...
                            if 'top_left' not in reconstructed_receipt:
                                reconstructed_receipt.update({
                                    ...
                                    'bottom_left': [0, image_item.get('height', 100)],
                                    'bottom_right': [image_item.get('width', 100), image_item.get('height', 100)]
                                })
                            
                            if self.update_entity(reconstructed_receipt):
                                receipt_count += 1
                                logger.debug(f"Reconstructed RECEIPT: {item['PK']}, {item['SK']}")
                        
                    except Exception as e:
                        logger.error(f"Error reconstructing receipt {item['PK']}, {item['SK']}: {e}")
                        self.stats["errors"] += 1
