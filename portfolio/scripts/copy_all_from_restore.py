...
def copy_all_entities_from_restore(prod_table: str, restore_table: str, dry_run: bool = True):
    ...
    while not done:
        ...
        if len(all_entities) % 50 == 0 and len(all_entities) > 0:
            logger.info(f"Found {len(all_entities)} entities in restore table so far...")
    
    logger.info(f"Found {stats['images_found']} Images and {stats['receipts_found']} Receipts in restore table")
    ...
    def copy_entity(entity):
        ...
        try:
            ...
            if added_fields:
                logger.debug(f"Copying {entity_type} {pk}: added {len(added_fields)} CDN fields")
            else:
    ...
    with ThreadPoolExecutor(max_workers=25) as executor:  # Higher concurrency for copying
        futures = {
            executor.submit(copy_entity, entity): entity 
        ...
        for future in as_completed(futures):
            try:
                ...
                if completed % 50 == 0:
                    logger.info(f"Progress: {completed}/{len(all_entities)} entities copied")
...
def main():
    ...
    copy_all_entities_from_restore(prod_table, args.restore_table, dry_run=not args.no_dry_run)
