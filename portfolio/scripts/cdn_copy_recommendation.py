...
def check_entity_exists(table: any, entity_type: str, entity_id: str) -> bool:
    ...
    try:
        response = table.get_item(
            Key={"PK": f"{entity_type}#{entity_id}", "SK": entity_id},
            ProjectionExpression="PK",
        )
...
def main():
        ...
        for img_id in prod_images_no_cdn[:5]:  # Check first 5
            try:
                response = dev_table.get_item(
                    Key={"PK": f"IMAGE#{img_id}", "SK": img_id},
    ...
    print(
        f"  3. Only {len(prod_images_no_cdn)} images and {len(prod_receipts_no_cdn)} receipts lack CDN fields"
    )

    if copy_candidates:
        print(f"\n💡 POTENTIAL ACTION:")
        print(
            f"   You could copy CDN values for {len(copy_candidates)} specific entities"
        )
