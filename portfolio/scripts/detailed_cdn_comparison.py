...
def check_subdirectory_pattern(table: any) -> Dict:
    ...
    try:
        ...
        response = table.scan(
            ...
            ExpressionAttributeValues={":pattern": "/1_"},
            Limit=10,
        )
        items = response.get("Items", [])

        result = {"found": len(items) > 0, "count": len(items), "examples": []}
...
def analyze_missing_cdn_fields(table: any, entity_type: str) -> Dict:
    ...
    try:
        ...
        return {
            "count": len(items),
            "examples": [
                item.get("PK", "").replace(f"{entity_type}#", "") for item in items[:5]
            ],
        }
    except Exception as e:
...
def main():
    ...
    if prod_subdir["found"] and not dev_subdir["found"]:
        ...
        print(
            "✓ PROD has entities missing CDN fields that might benefit from DEV data."
        )
        ...
        print(
            "   Check if specific entities need updating by comparing individual IDs."
        )
