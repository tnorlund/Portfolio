...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "receipt_dynamo"))
)
    ...
    try:
        ...
        for page in pages:
            ...
            for obj in page["Contents"]:
                ...
                if any(
                    pattern in key for pattern in ["IMG_", "IMAGE_", "PHOTO_", "PIC_"]
                ):
        ...
        if image_patterns["large_images"]:
            ...
            for key, size in sorted(
                image_patterns["large_images"], key=lambda x: -x[1]
            )[:10]:
...
def check_dynamo_receipt_entities():
    ...
    dynamo_client = ResilientDynamoClient(
        table_name="portfolioTable", region_name="us-east-1"
    )
