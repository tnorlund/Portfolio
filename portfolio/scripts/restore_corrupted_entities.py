...
class PITRRestorer:
    ...
    def parse_restore_time(self) -> datetime:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid time format: {self.restore_time}. Use ISO format like '2025-07-01T12:00:00'"
                )
    ...
    def identify_corrupted_entities(self):
        ...
        while not done:
            ...
            for item in items:
                ...
                if is_corrupted:
                    self.corrupted_entity_keys.append(
                        {"PK": pk, "SK": sk, "TYPE": entity_type}
                    )
                    corrupted_count += 1
            ...
            # Progress update
            logger.info(
                f"Scanned entities, found {corrupted_count} corrupted so far..."
            )
    ...
    def create_pitr_restore(self) -> bool:
        ...
        logger.info(
            f"Creating PITR restore of {self.source_table} to time: {restore_datetime}"
        )
    ...
    def extract_restored_entities(self) -> List[Dict[str, Any]]:
        ...
        if self.dry_run:
            ...
            logger.info(
                f"[DRY RUN] Would check {len(self.corrupted_entity_keys)} entities in restore table"
            )
        ...
        for entity_key in self.corrupted_entity_keys:
            try:
                response = restore_table.get_item(
                    Key={"PK": entity_key["PK"], "SK": entity_key["SK"]}
                )
                ...
                if "Item" in response:
                    ...
                    logger.warning(
                        f"Entity not found in restore: {entity_key['PK']}, {entity_key['SK']}"
                    )

            except Exception as e:
                logger.error(
                    f"Failed to extract {entity_key['PK']}, {entity_key['SK']}: {e}"
                )
                self.stats["errors"] += 1
        ...
        def copy_entity_batch(entity_batch):
            ...
            for entity in entity_batch:
                try:
                    ...
                    logger.error(
                        f"Failed to copy entity {entity.get('PK')}, {entity.get('SK')}: {e}"
                    )
                    self.stats["errors"] += 1
    ...
    def cleanup_restore_table(self):
        ...
        if self.dry_run:
            logger.info(
                f"[DRY RUN] Would delete restore table: {self.restore_table_name}"
            )
            return
    ...
    def print_summary(self):
        ...
        logger.info("=" * 50)
        logger.info(
            f"Corrupted entities found: {self.stats['corrupted_entities_found']}"
        )
