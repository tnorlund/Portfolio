...
def update_prod_cdn_fields(table_name: str, cdn_bucket: str, dry_run: bool = True):
    ...
    logger.info(f"{'[DRY RUN] ' if dry_run else ''}Updating CDN fields for PROD entities...")
    ...
    while not done:
        ...
        for entity in items:
            ...
            cdn_fields, found_count = generate_cdn_fields(entity_id, s3_client, cdn_bucket)
            ...
            try:
                ...
                logger.info(f"  ✅ {'[DRY RUN] ' if dry_run else ''}Updated {entity_id} with {found_count} CDN fields")
