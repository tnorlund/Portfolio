...
def fix_corrupted_from_restore(prod_table: str, restore_table: str, dry_run: bool = True):
    ...
    while not done:
        ...
        if len(corrupted_entities) % 50 == 0 and len(corrupted_entities) > 0:
            logger.info(f"Found {len(corrupted_entities)} corrupted IMAGE entities so far...")
    ...
    logger.info(f"Found {stats['corrupted_found']} corrupted entities total (Images + Receipts)")
    ...
    def fix_entity(entity_info):
        ...
        try:
            ...
            restore_response = restore_table_resource.get_item(Key={
                'PK': entity_info['PK'], 
                'SK': entity_info['SK']
            })
            
            if 'Item' not in restore_response:
                logger.warning(f"Entity not found in restore table: {entity_info['PK']}, {entity_info['SK']}")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(fix_entity, entity): entity 
        ...
        for future in as_completed(futures):
            try:
                ...
                if completed % 25 == 0:
                    logger.info(f"Progress: {completed}/{len(corrupted_entities)} entities processed")
...
def main():
    ...
    fix_corrupted_from_restore(prod_table, args.restore_table, dry_run=not args.no_dry_run)
