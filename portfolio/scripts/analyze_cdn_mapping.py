...
def generate_cdn_mapping(
    bucket: str, image_id: str, pattern: str = "auto"
) -> Dict[str, str]:
...
def analyze_prod_entities(table_name: str, cdn_bucket: str, sample_size: int = 10):
    ...
    for entity in image_entities:
        ...
        populated_fields = sum(
            1 for field in cdn_fields if entity.get(field) is not None
        )
...
def main():
    ...
    if args.analyze_bucket:
        ...
        logger.info(
            f"Image IDs with flat pattern: {bucket_analysis['image_ids_with_flat']}"
        )
        logger.info(
            f"Image IDs with directory pattern: {bucket_analysis['image_ids_with_dir']}"
        )
        ...
        logger.info(
            f"Images with only directory pattern: {bucket_analysis['only_dir']}"
        )
    ...
    flat_pattern = sum(
        1 for r in entity_results if r["pattern_info"]["recommended_pattern"] == "flat"
    )
    dir_pattern = sum(
        1 for r in entity_results if r["pattern_info"]["recommended_pattern"] == "dir"
    )
    no_pattern = sum(
        1 for r in entity_results if r["pattern_info"]["recommended_pattern"] is None
    )

    logger.info(f"\nSUMMARY:")
    logger.info(f"Total entities analyzed: {total_entities}")
    logger.info(
        f"Can be mapped to CDN: {can_be_mapped} ({can_be_mapped/total_entities*100:.1f}%)"
    )
    ...
    for i, result in enumerate(entity_results[:3]):
        ...
        logger.info(
            f"Recommended pattern: {result['pattern_info']['recommended_pattern']}"
        )
        logger.info(f"Flat files found: {len(result['pattern_info']['flat_files'])}")
        logger.info(
            f"Directory files found: {len(result['pattern_info']['dir_files'])}"
        )

        if result["recommended_mapping"]:
            logger.info("Recommended CDN field mapping:")
            for field, value in list(result["recommended_mapping"].items())[
                :5
            ]:  # Show first 5
                logger.info(f"  {field}: {value}")
            if len(result["recommended_mapping"]) > 5:
                logger.info(
                    f"  ... and {len(result['recommended_mapping']) - 5} more fields"
                )
