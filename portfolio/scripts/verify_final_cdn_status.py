...
def check_entity_cdn_completeness(
    entity: Dict[str, Any], s3_client, cdn_bucket: str
) -> Dict[str, Any]:
...
def main():
    ...
    for entity in images:
        ...
        if (
            result["cdn_fields_populated"] > 0
            and result["cdn_fields_populated"] == result["cdn_files_in_s3"]
        ):
        ...
        if result["missing_files"]:
            ...
            logger.warning(
                f"  Image {result['entity_id']} missing files: {result['missing_files']}"
            )
    ...
    for entity in receipts:
        ...
        if result["missing_files"]:
            stats["entities_with_missing_files"] += 1
            logger.warning(
                f"  Receipt {result['entity_id']} missing files: {result['missing_files']}"
            )
    ...
    logger.info(
        f"Images fully populated with valid S3 files: {stats['images_fully_populated']}"
    )
    logger.info(f"Receipts checked: {stats['receipts_checked']}")
    logger.info(
        f"Receipts fully populated with valid S3 files: {stats['receipts_fully_populated']}"
    )
    logger.info(f"\nPattern Analysis:")
    logger.info(f"Entities using flat pattern: {stats['entities_with_flat_pattern']}")
    logger.info(
        f"Entities with subdirectory pattern available: {stats['entities_with_subdirectory_pattern']}"
    )
    logger.info(f"Entities with BOTH patterns: {stats['entities_with_both_patterns']}")
    logger.info(
        f"Entities with missing S3 files: {stats['entities_with_missing_files']}"
    )
    ...
    total_populated = (
        stats["images_fully_populated"] + stats["receipts_fully_populated"]
    )
