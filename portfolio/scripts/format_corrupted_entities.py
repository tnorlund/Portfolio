...
def format_corrupted_entities(table_name: str, raw_bucket: str, cdn_bucket: str, dry_run: bool = True):
    ...
    def format_image_entity(corrupted_image):
        ...
        try:
            ...
            if 'timestamp_added' not in formatted_item:
                formatted_item['timestamp_added'] = datetime.now(timezone.utc).isoformat()
    ...
    def format_receipt_entity(corrupted_receipt):
        ...
        try:
            ...
            if 'bottom_right' not in formatted_item:
                formatted_item['bottom_right'] = [formatted_item['width'], formatted_item['height']]
            
            logger.info(f"Formatting RECEIPT {image_id}/{receipt_id}: added {missing_fields}")
            ...
            logger.error(f"Failed to format RECEIPT {item.get('PK', 'unknown')}/{item.get('SK', 'unknown')}: {e}")
            stats['errors'] += 1
            return False
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                if completed % 25 == 0:
                    logger.info(f"Progress: {completed}/{len(all_entities)} entities formatted")
...
def main():
    ...
    format_corrupted_entities(prod_table, raw_bucket, cdn_bucket, dry_run=not args.no_dry_run)
