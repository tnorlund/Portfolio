...
def check_remaining_corruption(table_name: str):
    ...
    while not done:
        ...
        if stats["total_images"] % 100 == 0:
            logger.info(
                f"Checked {stats['total_images']} IMAGE entities, found {stats['corrupted_images']} corrupted..."
            )
    ...
    while not done:
        ...
        for item in items:
            ...
            try:
                ...
                stats["parsing_errors"].append(error_info)
                logger.error(
                    f"Corrupted RECEIPT {item.get('PK')}/{item.get('SK')}: {e}"
                )
                if error_info["missing_fields"]:
                    logger.error(f"  Missing fields: {error_info['missing_fields']}")
        ...
        if stats["total_receipts"] % 100 == 0:
            logger.info(
                f"Checked {stats['total_receipts']} RECEIPT entities, found {stats['corrupted_receipts']} corrupted..."
            )
    ...
    logger.info(
        f"Total corrupted entities: {stats['corrupted_images'] + stats['corrupted_receipts']}"
    )
...
def delete_corrupted_entities(
    table_name: str, corrupted_entities: List[Dict], dry_run: bool = True
):
    ...
    logger.info(
        f"{'[DRY RUN] ' if dry_run else ''}Deleting {len(corrupted_entities)} corrupted entities..."
    )
    ...
    for entity in corrupted_entities:
        try:
            ...
            logger.info(
                f"{'[DRY RUN] ' if dry_run else ''}Deleting {entity_type}: {pk}/{sk}"
            )
...
def main():
    ...
    if args.delete_corrupted and corrupted_entities:
        delete_corrupted_entities(
            prod_table, corrupted_entities, dry_run=not args.no_dry_run
        )
