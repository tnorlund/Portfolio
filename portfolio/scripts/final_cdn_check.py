...
def main():
    ...
    # Analyze patterns
    dev_patterns = {'flat': 0, 'subdirectory': 0, 'subdirectory_with_prefix': 0, 'none': 0, 'unknown': 0}
    prod_patterns = {'flat': 0, 'subdirectory': 0, 'subdirectory_with_prefix': 0, 'none': 0, 'unknown': 0}
    ...
    for img in prod_images:
        ...
        if pattern in ['subdirectory', 'subdirectory_with_prefix'] and len(prod_subdirectory_examples) < 5:
    ...
    if common_ids:
        ...
        for entity_id in sorted(list(common_ids))[:5]:
            # Find in lists
            dev_img = next((img for img in dev_images if img.get('PK') == f'IMAGE#{entity_id}'), None)
            prod_img = next((img for img in prod_images if img.get('PK') == f'IMAGE#{entity_id}'), None)
    ...
    if prod_patterns['subdirectory'] > 0 or prod_patterns['subdirectory_with_prefix'] > 0:
        print("\n⚠️  CRITICAL: PROD has subdirectory pattern CDN entries!")
        print(f"   - {prod_patterns['subdirectory'] + prod_patterns['subdirectory_with_prefix']} entities use subdirectory pattern")
