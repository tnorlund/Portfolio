...
def generate_receipt_cdn_fields(
    image_id: str, receipt_id: str, s3_client, cdn_bucket: str
    ...
    logger.info(
        f"  Receipt {image_id}/RECEIPT_{receipt_id}: {found_files} CDN files found"
    )
    if found_files > 0 and cdn_fields.get("cdn_s3_key"):
        logger.info(
            f"    Primary format: {cdn_fields['cdn_s3_key'].split('.')[-1].upper()}"
        )
...
def fix_receipt_cdn_fields(table_name: str, cdn_bucket: str, dry_run: bool = True):
    ...
    logger.info(
        f"{'[DRY RUN] ' if dry_run else ''}Fixing CDN fields for Receipt entities..."
    )
    ...
    while not done:
        ...
        for entity in items:
            ...
            cdn_fields, found_count = generate_receipt_cdn_fields(
                image_id, receipt_id, s3_client, cdn_bucket
            )
