...
// Line-Item Decoder Walkthrough Types
// Static payload under public/line-item-demo/receipts.json generated by
// scripts/export_line_item_viz_data.py: real golden receipts with the
// deterministic decoder's intermediate stages captured for replay.

/** [line_id, word_id] reference into LineItemDemoReceipt.words. */
export type LineItemDemoWordRef = [number, number];

export interface LineItemDemoWord {
  line_id: number;
  word_id: number;
  text: string;
  /** Normalized 0-1, bottom-left origin (same convention as OCR words). */
  bbox: BoundingBox;
  in_zone: boolean;
}
...
export interface LineItemDemoBand {
  band_id: number;
  line_ids: number[];
  word_refs: LineItemDemoWordRef[];
  text: string;
  /** Normalized band y-center, bottom-left origin (larger = higher). */
  y: number;
  amounts: number[];
  role: "PRICE" | "MEMBER" | "OUTSIDE";
  guard: LineItemDemoGuard | null;
  prior: { role: string; support: number; purity: number } | null;
  /** Only PRICE bands carry an outcome. */
  outcome?: "item" | "absorbed" | "quantity_donor" | "unclaimed";
  /** Display-order index of the item an absorbed/donor band feeds. */
  absorbed_into?: number;
}

export interface LineItemDemoItem {
  y: number | null;
  name: string | null;
  price: number | null;
  ...
  line_ids: number[];
  name_word_ids: LineItemDemoWordRef[];
  price_word_id: LineItemDemoWordRef | null;
  qty_word_ids: LineItemDemoWordRef[];
}

export interface LineItemDemoDroppedItem extends LineItemDemoItem {
  reason: string;
}

/** CDN keys mirror utils/imageFormat.ts ImageFormats (full/thumbnail/
 * small/medium tiers in jpeg/webp/avif). */
export interface LineItemDemoImage {
  width: number;
  height: number;
  cdn_s3_key: string;
  cdn_webp_s3_key?: string;
...
export interface LineItemDemoReconcile {
  status: "match" | "near" | "mismatch" | "no-baseline";
  item_sum: number | null;
  baseline: number | null;
  baseline_source: string | null;
  baseline_figures_agreeing: number | null;
}

export interface LineItemDemoReceipt {
  image_id: string;
  receipt_id: number;
  merchant: string;
  ...
  reconcile: LineItemDemoReconcile;
}

export interface LineItemDemoResponse {
  source: string;
  table: string;
  decoder: string;
