"""Script to validate receipts multiple times using Claude API."""
...
if __name__ == "__main__":
    ...
    print("Starting multi-validation script with Claude API...")
    ...
    # Initialize Claude client (using OpenAI-compatible endpoint)
    # Claude provides an OpenAI-compatible API through their proxy
    client = OpenAI(
        api_key=ANTHROPIC_API_KEY,
        base_url="https://api.anthropic.com/v1/messages/openai",
        default_headers={
    ...
    # or newer: claude-3-5-sonnet-20241022
    CLAUDE_MODEL = "claude-3-haiku-20240307"  # Fastest and cheapest
    ...
    for idx, receipt in enumerate(receipts_missing_metadata, 1):
        ...
        receipt = Receipt(**image_dict["receipts"][0])
        receipt_lines = [
        ...
        for run in range(1, VALIDATION_RUNS + 1):
            ...
            if run > 1:
                print("  ⏱️  Waiting 1 second before next run...")
                time.sleep(1)

            # Create handler with Claude client
