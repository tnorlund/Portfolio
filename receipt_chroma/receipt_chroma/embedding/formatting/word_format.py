...
def get_word_neighbors(
    ...
    Uses a simple line-aware approach similar to receipt_label:
    1. First, find words on the same line using vertical span overlap
    ...
    # Use vertical span overlap for same-line detection (like receipt_label)
    ...
    for orig_idx, w in sorted_all:
        ...
        # Check vertical span overlap (same line)
    ...
    # Filter nearby-line candidates by x-proximity (simple threshold), then sort by y-proximity, then x-proximity
    x_proximity_threshold = 0.25  # Simple threshold to avoid words too far horizontally
    ...
    nearby_line_left_sorted = sorted(
        nearby_line_left_filtered,
        key=lambda item: (
            abs(item[1].calculate_centroid()[1] - target_y),  # y-proximity first
            target_x - item[1].calculate_centroid()[0],  # then x_diff (smaller is better)
        ),
    )
    nearby_line_right_sorted = sorted(
        ...
        key=lambda item: (
            abs(item[1].calculate_centroid()[1] - target_y),  # y-proximity first
            item[1].calculate_centroid()[0] - target_x,  # then x_diff (smaller is better)
        ),
    )
    ...
    # If not enough, add nearby-line words to the left (sorted by y-proximity, then x-proximity)
    if len(left_words) < context_size:
    ...
    # If not enough, add nearby-line words to the right (sorted by y-proximity, then x-proximity)
    if len(right_words) < context_size:
