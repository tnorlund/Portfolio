...
class DeltaProcessor:
    ...
    def list_delta_files(
        ...
        if database_name and collection_name:
            search_prefix = f"{self.delta_prefix}{database_name}/{collection_name}/"
                ...
                if 'Contents' in page:
                    for obj in page['Contents']:
                        ...
                        if len(parts) >= 3:  # At minimum: deltas/uuid/file
                            ...
                            if database_name and collection_name:
                                if len(parts) >= 5:  # deltas/db/collection/uuid/file
                                    delta_id = parts[3]
                                    delta_prefix_key = '/'.join(parts[:4]) + '/'
                                else:
                                    continue
                            elif collection_name:
                                if len(parts) >= 4:  # deltas/collection/uuid/file
                                    delta_id = parts[2]
                                    delta_prefix_key = '/'.join(parts[:3]) + '/'
                                else:
                                    continue
                            else:
                                delta_id = parts[1]
                            ...
                            if current_delta is None or current_delta['delta_id'] != delta_id:
                            ...
                            if obj['LastModified'] > current_delta['last_modified']:
                                current_delta['last_modified'] = obj['LastModified']
    ...
    def download_delta(
        ...
        try:
            paginator = self.s3_client.get_paginator('list_objects_v2')
            pages = paginator.paginate(Bucket=self.bucket_name, Prefix=delta_prefix)
            ...
            for page in pages:
                if 'Contents' in page:
                    for obj in page['Contents']:
                        key = obj['Key']
                        if key != delta_prefix:  # Skip the prefix itself if it's a "directory"
        ...
        if not objects_to_download:
            raise RuntimeError(f"No delta files found at s3://{self.bucket_name}/{delta_prefix}")
        ...
        logger.info(
            ...
            len(downloaded_files), total_size, delta_prefix
        )
    ...
    def merge_delta_into_collection(
        ...
        if not os.path.exists(delta_directory):
            raise RuntimeError(f"Delta directory does not exist: {delta_directory}")
        ...
        try:
            if not target_client.collection_exists(collection_name):
                if create_collection_if_missing:
                    ...
                    target_client.get_collection(collection_name, create_if_missing=True)
                    logger.info("Created target collection: %s", collection_name)
                else:
                    raise RuntimeError(f"Target collection '{collection_name}' does not exist")
        except Exception as e:
        ...
        logger.info("Merging delta: %d items into collection with %d items", 
                   delta_count, initial_count)
        ...
        try:
            ...
            all_items = delta_collection.get(include=["metadatas", "documents", "embeddings"])
    ...
    def process_delta_batch(
        ...
        for i, delta_info in enumerate(delta_list):
            ...
            logger.info("Processing delta %d/%d: %s", i+1, len(delta_list), delta_id)
            ...
            try:
                ...
                logger.info("Successfully processed delta %s: %d items merged", 
                          delta_id, merge_result['items_merged'])
        ...
        logger.info(
            ...
            len(processed_deltas), len(failed_deltas), total_items_merged
        )
    ...
    def delete_processed_deltas(
        ...
        for delta_prefix in delta_prefixes:
            try:
                ...
                if objects_to_delete:
                    ...
                    logger.info("Deleted delta %s: %d objects", delta_prefix, objects_deleted)
                else:
                    logger.warning("No objects found to delete for prefix: %s", delta_prefix)
        ...
        logger.info(
            ...
            len(deleted_deltas), total_objects_deleted, len(failed_deletions)
        )
