...
class CompactionEngine:
    ...
    def run_compaction(
        ...
        logger.info("Starting compaction for collection '%s' (database: %s)", 
        ...
        try:
            ...
            if delta_list or force_snapshot:
                ...
                logger.info("No deltas to process and force_snapshot=False, skipping compaction")
            ...
            snapshot_creation_result = self._create_new_snapshot(
                snapshot_restore_result["client"], snapshot_restore_result["local_directory"]
            )
            
            # Step 6: Cleanup processed deltas
            delta_cleanup_result = None
            if delta_processing_result and delta_processing_result.get("processed_deltas"):
                processed_prefixes = [
                    delta["delta_prefix"] for delta in delta_processing_result["processed_deltas"]
                ]
                delta_cleanup_result = self.delta_processor.delete_processed_deltas(processed_prefixes)
            ...
            try:
                ...
                logger.warning("Failed to release lock during error cleanup: %s", lock_e)
    ...
    def _acquire_compaction_lock(self, timeout_seconds: int) -> Dict[str, Any]:
        ...
        if not LOCK_MANAGER_AVAILABLE or not self.lock_manager:
            logger.warning("Lock manager not available, proceeding without locking")
            return {"status": "no_lock_manager", "acquired": False}

        try:
            # Create lock ID based on collection and database
            if self.database_name:
                lock_id = f"compaction-{self.database_name}-{self.collection_name}"
            else:
            ...
            if success:
                ...
                logger.info("Successfully acquired compaction lock: %s", lock_id)
                return {
                    "status": "acquired",
                    "lock_id": lock_id,
                    "timeout_seconds": timeout_seconds,
                ...
                raise RuntimeError(f"Failed to acquire compaction lock: {lock_id}")
    ...
    def _restore_current_snapshot(self) -> Dict[str, Any]:
        ...
        logger.info("Restoring current snapshot for collection '%s'", self.collection_name)
        
        # Create temporary directory for snapshot
        snapshot_temp_dir = tempfile.mkdtemp(prefix=f"snapshot_{self.collection_name}_")
        ...
        try:
            ...
            collection = client.get_collection(
                ...
                metadata={"created_by": "compaction_engine", "created_at": datetime.now(timezone.utc).isoformat()}
            )
    ...
    def _create_new_snapshot(
        ...
        logger.info("Creating new snapshot for collection '%s'", self.collection_name)
    ...
    def _build_result(
        ...
        if delta_processing_result:
            ...
            result["deltas_processed"] = delta_processing_result.get("deltas_processed", 0)
            result["total_items_merged"] = delta_processing_result.get("total_items_merged", 0)
            result["failed_deltas"] = delta_processing_result.get("failed_deltas", 0)
    ...
    def estimate_compaction_size(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "estimated_processing_time_minutes": len(deltas) * 2,  # Rough estimate
            }
