        ...
        if sqs_queue_url:
            try:
                ...
                sqs.send_message(
                    ...
                    MessageGroupId=f"{collection_name}:{batch_id or 'default'}",
                    MessageDeduplicationId=f"{collection_name}:{(batch_id or uuid.uuid4().hex)}:{s3_key}",
                    MessageAttributes={
                        "collection": {
...
def upload_snapshot_with_hash(
    ...
    if not os.path.exists(local_snapshot_path):
        raise RuntimeError(
            f"Local snapshot path does not exist: {local_snapshot_path}"
        )
    ...
    if clear_destination:
        try:
            ...
            response = s3_client.list_objects_v2(
                Bucket=bucket, Prefix=snapshot_key
            )
            ...
            if "Contents" in response:
                ...
                if objects_to_delete:
                    ...
                    logger.info(
                        "Cleared %d existing objects at %s",
    ...
    try:
        ...
        if not files_to_upload:
            raise RuntimeError(
                f"No files found to upload in {local_snapshot_path}"
            )
        ...
        logger.info(
            "Successfully uploaded %d files (%d bytes) to S3 at %s",
            uploaded_count,
            total_size,
            snapshot_key,
        )
