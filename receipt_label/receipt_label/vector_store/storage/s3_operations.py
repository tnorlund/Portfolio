...
class S3Operations:
    ...
    def upload_snapshot(
        ...
        if database_name:
            key_prefix = (
                f"{s3_prefix.rstrip('/')}/{database_name}/{collection_name}/"
            )
        else:
            key_prefix = f"{s3_prefix.rstrip('/')}/{collection_name}/"

        logger.info(
            "Uploading snapshot from %s to s3://%s/%s",
            local_directory,
            self.bucket_name,
            key_prefix,
        )
        ...
        if not files_to_upload:
            raise RuntimeError(
                f"No files found to upload in {local_directory}"
            )
        ...
        if hash_result:
            ...
            try:
                ...
                logger.info(
                    "Uploaded hash metadata to s3://%s/%s",
                    self.bucket_name,
                    hash_key,
                )
            except Exception as e:
    ...
    def download_snapshot(
        ...
        logger.info(
            "Downloading snapshot from s3://%s/%s to %s",
            self.bucket_name,
            key_prefix,
            local_directory,
        )
        ...
        if not objects_to_download:
            raise RuntimeError(
                f"No snapshot files found at s3://{self.bucket_name}/{key_prefix}"
            )
        ...
        if verify_hash and hash_key:
            try:
                ...
                hash_response = self.s3_client.get_object(
                    Bucket=self.bucket_name, Key=hash_key
                )
                hash_content = hash_response["Body"].read().decode("utf-8")
                s3_hash_result = HashCalculator.parse_hash_metadata(
                    hash_content
                )
    ...
    def upload_delta(
        ...
        if database_name and collection_name:
            ...
            key_prefix = (
                f"{delta_prefix.rstrip('/')}/{collection_name}/{unique_id}/"
            )
        else:
            key_prefix = f"{delta_prefix.rstrip('/')}/{unique_id}/"

        logger.info(
            "Uploading delta from %s to s3://%s/%s",
            local_directory,
            self.bucket_name,
            key_prefix,
        )
    ...
    def delete_snapshot(
        ...
        logger.info(
            "Deleting snapshot at s3://%s/%s", self.bucket_name, key_prefix
        )
        ...
        try:
            ...
            if not objects_to_delete:
                logger.warning(
                    "No objects found to delete at prefix: %s", key_prefix
                )
            ...
            logger.info(
                "Successfully deleted %d objects from %s",
                deleted_count,
                key_prefix,
            )
...
def upload_delta_to_s3(
    ...
    return s3_ops.upload_delta(
        local_directory=local_directory, delta_prefix=delta_prefix
    )
