...
def performance_env():
    ...
    env_vars = {
        "DYNAMO_TABLE_NAME": "perf-test-table",
...
class TestAIUsagePerformanceIntegration:
    ...
    def test_high_throughput_tracking(
        ...
        with patch("receipt_label.utils.client_manager.DynamoClient", return_value=mock_high_performance_dynamo):
            with patch("receipt_label.utils.client_manager.OpenAI", return_value=mock_openai):
                ...
                avg_latency = statistics.mean(mock_high_performance_dynamo._write_latencies)
            ...
            with patch("receipt_label.utils.client_manager.OpenAI", return_value=mock_openai):
                ...
                def worker_task(worker_id: int, num_requests: int):
                    ...
                    for i in range(num_requests):
                        ...
                        openai_client.chat.completions.create(
                            model="gpt-3.5-turbo",
                            messages=[{"role": "user", "content": f"Worker {worker_id} msg {i}"}],
                        )
                ...
                for worker_id in range(num_workers):
                    worker_metrics = [
                        m for m in mock_high_performance_dynamo._stored_items
            ...
            with patch("receipt_label.utils.client_manager.OpenAI", return_value=mock_openai):
                ...
                for batch in range(num_batches):
                    for i in range(batch_size):
                        openai_client.chat.completions.create(
                            model="gpt-3.5-turbo",
                            messages=[{"role": "user", "content": f"Batch {batch} msg {i}"}],
                        )
                ...
                assert len(mock_high_performance_dynamo._stored_items) >= total_operations * 0.95  # Allow 5% loss
            ...
            with patch("receipt_label.utils.client_manager.OpenAI", return_value=mock_openai):
                ...
                for phase_name, count, delay in phases:
                    ...
                    for i in range(count):
                        ...
                        openai_client.chat.completions.create(
                            model="gpt-3.5-turbo",
                            messages=[{"role": "user", "content": f"{phase_name} {i}"}],
                        )
                ...
                assert burst_metrics["throughput"] > 50  # Handle at least 50 req/s during burst
    ...
    def test_graceful_degradation_under_stress(
        ...
        with patch("receipt_label.utils.client_manager.DynamoClient", return_value=mock_dynamo):
            with patch("receipt_label.utils.client_manager.OpenAI", return_value=mock_openai):
                manager = ClientManager(config)
                openai_client = manager.openai
                ...
                for window_num in range(20):  # 2000 total requests
                    ...
                    for i in range(window_size):
                        try:
                            openai_client.chat.completions.create(
                                model="gpt-3.5-turbo",
                                messages=[{"role": "user", "content": f"Stress test {window_num}-{i}"}],
                            )
                            successes += 1
                        except:
                            failures += 1
                ...
                early_success = statistics.mean(w["success_rate"] for w in early_windows)
                ...
                early_throughput = statistics.mean(w["throughput"] for w in early_windows)
                late_throughput = statistics.mean(w["throughput"] for w in late_windows)
                
                assert late_throughput > early_throughput * 0.3  # At least 30% of original