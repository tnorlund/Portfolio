...
@pytest.fixture
def sample_receipt_section(sample_spatial_pattern, sample_content_pattern) -> ReceiptSection:
...
class TestStructureAnalysis:
    ...
    def test_to_dynamo_basic(self, sample_structure_analysis):
        ...
        assert dynamo_obj.overall_reasoning == sample_structure_analysis.overall_reasoning
    ...
    def test_from_dynamo_basic(self, sample_dynamo_receipt_structure_analysis):
        ...
        analysis = StructureAnalysis.from_dynamo(sample_dynamo_receipt_structure_analysis)
        
        # Check basic fields
        assert len(analysis.sections) == 1
        assert analysis.overall_reasoning == "Receipt has clear header, body, and footer sections"
        ...
        assert section.name == "header"
        assert section.line_ids == [1, 2, 3]
        assert section.reasoning == "Clear header section with business name and address"
    ...
    def test_roundtrip_conversion(self, sample_structure_analysis):
        ...
        assert reconstructed.overall_reasoning == sample_structure_analysis.overall_reasoning
        ...
        assert new_section.spatial_patterns[0].pattern_type == orig_section.spatial_patterns[0].pattern_type
        assert new_section.spatial_patterns[0].description == orig_section.spatial_patterns[0].description
        
        assert len(new_section.content_patterns) == len(orig_section.content_patterns)
        assert new_section.content_patterns[0].pattern_type == orig_section.content_patterns[0].pattern_type
        assert new_section.content_patterns[0].description == orig_section.content_patterns[0].description
        assert new_section.content_patterns[0].examples == orig_section.content_patterns[0].examples 