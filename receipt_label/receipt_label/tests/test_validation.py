...
class TestValidationUtils:
    def test_validate_business_name(self):
        ...
        # Test similar names
        is_valid, message, confidence = validate_business_name("Walmart", "Walmart Supercenter")
        assert is_valid
    ...
    def test_validate_phone_number(self):
        ...
        # Test exact match
        is_valid, message, confidence = validate_phone_number("(555) 123-4567", "555-123-4567")
        assert is_valid
        assert confidence == 1.0
        
        # Test different formats
        is_valid, message, confidence = validate_phone_number("5551234567", "(555) 123-4567")
        assert is_valid
        assert confidence == 1.0
        
        # Test different numbers
        is_valid, message, confidence = validate_phone_number("(555) 123-4567", "(555) 123-4568")
        assert not is_valid
        assert confidence == 0.0
    ...
    def test_validate_datetime(self):
        ...
        # Test with max age
        old_date = (datetime.now().replace(year=datetime.now().year - 2)).strftime("%Y-%m-%d")
    ...
    def test_validate_amounts(self):
        ...
        # Test with tolerance
        is_valid, message, confidence = validate_amounts(100.00, 10.00, 110.001, tolerance=0.01)
        assert is_valid
        assert confidence == 1.0
    ...
    def test_validate_receipt_format(self):
        ...
        is_valid, violations, confidence = validate_receipt_format(receipt_data, receipt_lines)
        assert is_valid
        ...
        is_valid, violations, confidence = validate_receipt_format(invalid_data, receipt_lines)
        assert not is_valid
        assert len(violations) > 0
        assert confidence < 1.0 