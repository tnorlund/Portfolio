...
class TestValidationAnalysisSerialization:
    ...
    def test_to_dynamo_validation_summary(self, sample_validation_analysis):
        ...
        assert summary.overall_reasoning == "Validation failed with 1 error in business identity"
        ...
        assert summary.field_summary["business_identity"]["has_errors"] is True
        assert summary.field_summary["address_verification"]["status"] == "needs_review"
        assert summary.field_summary["address_verification"]["has_warnings"] is True
    ...
    def test_to_dynamo_validation_categories(self, sample_validation_analysis):
        ...
        business_identity_cat = next(cat for cat in categories if cat.field_name == "business_identity")
        ...
        address_cat = next(cat for cat in categories if cat.field_name == "address_verification")
    ...
    def test_to_dynamo_validation_results(self, sample_validation_analysis):
        ...
        business_result = next(res for res in results if res.field_name == "business_identity")
        ...
        address_result = next(res for res in results if res.field_name == "address_verification")
    ...
    def test_from_dynamo_items(self, sample_validation_summary, sample_validation_categories, sample_validation_results):
        ...
        assert analysis.overall_reasoning == "Validation failed with 1 error in business identity"
    ...
    def test_roundtrip_conversion(
        self, sample_validation_analysis, sample_validation_summary, sample_validation_categories, sample_validation_results
    ):
        ...
        reconstructed = ValidationAnalysis.from_dynamo_items(summary, categories, results)
        ...
        assert reconstructed.overall_reasoning == sample_validation_analysis.overall_reasoning
        
        # Check that ValidationResult objects were properly reconstructed
        assert len(reconstructed.business_identity.results) == len(sample_validation_analysis.business_identity.results)
        ...
        assert reconstructed.metadata["version"] == sample_validation_analysis.metadata["version"]
        assert "processing_metrics" in reconstructed.metadata
        assert reconstructed.metadata["processing_metrics"]["processing_time_ms"] == sample_validation_analysis.metadata["processing_metrics"]["processing_time_ms"]
