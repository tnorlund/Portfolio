...
class ValidationConfig:
    ...
    def from_env(cls) -> "ValidationConfig":
        ...
        return cls(
            ...
            enable_smart_batching=os.getenv("ENABLE_SMART_BATCHING", "true").lower() == "true",
...
class ValidationMetrics:
    ...
    def record_validation(self, duration: float, success: bool, tokens: int = 0, cost: float = 0.0):
    ...
    def average_time(self) -> float:
        ...
        return self.total_time / self.validation_count if self.validation_count > 0 else 0.0
    
    @property
    def success_rate(self) -> float:
        """Calculate success rate"""
        return self.success_count / self.validation_count if self.validation_count > 0 else 0.0
...
class ReceiptValidator:
    ...
    async def validate_single_receipt(
        ...
        try:
            ...
            # Record metrics
            self.metrics.record_validation(duration, success, token_estimate, cost_estimate)
    ...
    async def _smart_batch_validate(
        ...
        if urgent_receipts:
            urgent_results = await self._concurrent_validate(urgent_receipts, max_concurrent)
        ...
        if normal_receipts:
            ...
            for i in range(0, len(normal_receipts), batch_size):
                ...
                batch_results = await self._concurrent_validate(batch, max_concurrent)
    ...
    def _estimate_tokens(self, labels: List[Dict], result: Dict) -> int:
        ...
        input_tokens = len(str(labels)) // 4  # Rough estimate: 4 chars per token
...
async def validate_receipt_labels_v2(
    ...
    return await validator.validate_single_receipt(image_id, receipt_id, labels)
