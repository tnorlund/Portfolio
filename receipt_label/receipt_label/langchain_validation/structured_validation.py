...
class ValidationResult(BaseModel):
    ...
    correct_label: Optional[str] = Field(
        default=None, description="The correct label if is_valid is false"
    )
...
class ValidationResponse(BaseModel):
    ...
    processing_notes: Optional[str] = Field(
        default=None, description="Any notes about the validation process"
    )
...
async def validate_with_structured_output(
    formatted_prompt: str, validation_targets: List[dict]
) -> ValidationResponse:
    """
...
async def validate_with_json_schema(
    ...
    try:
        ...
        response = await llm.ainvoke(
            ...
            config={
                "response_format": {
                    "json_schema": get_validation_json_schema()
                }
            },
        )
        return response
...
async def validate_with_ollama_structured(state: dict) -> dict:
    ...
    try:
        ...
        response = await validate_with_structured_output(
            state["formatted_prompt"], state["validation_targets"]
        )
