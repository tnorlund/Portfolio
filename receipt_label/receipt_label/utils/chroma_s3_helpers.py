...
def upload_delta_to_s3(
    ...
    """
    logger.info(
        "Starting S3 delta upload: local_path=%s, bucket=%s, key=%s",
        local_delta_path,
        bucket,
        delta_key,
    )
    try:
        ...
        logger.info(
            "Created S3 client for upload, region: %s", region or "default"
        )

        delta_path = Path(local_delta_path)
        if not delta_path.exists():
            logger.error(
                "Local delta path does not exist: %s", local_delta_path
            )
            return {
                "status": "failed",
        ...
        # Configure high-performance transfer
...
def clear_s3_directory(
    bucket: str, key_prefix: str, region: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(
        "Clearing S3 directory: bucket=%s, prefix=%s", bucket, key_prefix
    )
    ...
    try:
        ...
        return {"status": "failed", "error": str(e), "deleted_count": 0}
...
def upload_snapshot_with_hash(
    ...
    if not HASH_UTILS_AVAILABLE and calculate_hash:
        logger.warning(
            "Hash calculation requested but hash utilities not available"
        )
    ...
    try:
        ...
        if clear_destination:
            ...
            if clear_result["status"] == "cleared":
                logger.info(
                    "Cleared %d existing objects from %s",
                    clear_result["deleted_count"],
                    snapshot_key,
                )
            else:
                logger.warning(
                    "Failed to clear S3 destination: %s",
        ...
        if calculate_hash:
            try:
                logger.info(
                    "Calculating %s hash for snapshot...",
                ...
                hash_result = calculate_chromadb_hash(
                    ...
                    exclude_patterns=[
                        ...
                        "*.log",
                        ".snapshot_hash",
                    ],  # Exclude temp files and existing hash
                )
                logger.info(
                    "Calculated hash: %s (processed %d files)",
                    hash_result.directory_hash,
                    hash_result.file_count,
                )
            except Exception as e:
        ...
        logger.info(
            "Successfully uploaded snapshot: %d files, %d bytes, hash: %s",
            file_count,
            total_size,
...
def download_snapshot_with_verification(
    ...
    """
    logger.info(
        "Starting snapshot download with verification: bucket=%s, key=%s, local_path=%s",
        bucket,
        snapshot_key,
        local_snapshot_path,
    )
    ...
    if not verify_hash or not HASH_UTILS_AVAILABLE:
        if not verify_hash:
            ...
            logger.warning(
                "Hash verification skipped (hash utilities not available)"
            )
        return result
    ...
    try:
        ...
        if expected_hash is None:
            try:
                ...
                logger.info(
                    "Retrieved expected hash from S3: %s (%s)",
                    expected_hash,
                    hash_algorithm.upper(),
                )
        ...
        logger.info(
            "Local hash calculated: %s", local_hash_result.directory_hash
        )

        # Compare hashes
        if s3_hash_result:
            comparison = compare_hash_results(
                local_hash_result, s3_hash_result
            )
        else:
        ...
        if comparison["is_identical"]:
            logger.info(
                "✅ Hash verification successful - downloaded snapshot is identical to S3"
            )
        else:
            logger.warning(
                "❌ Hash verification failed - %s", comparison["message"]
            )
...
def verify_chromadb_sync(
    ...
    logger.info(
        "Starting ChromaDB sync verification: database=%s, bucket=%s, local_path=%s",
        database,
        bucket,
        local_path,
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "S3 hash retrieved: %s (%s)",
                s3_hash_result.directory_hash,
                s3_hash_result.hash_algorithm,
            )
        ...
        if os.path.exists(local_path):
            try:
                ...
                logger.info(
                    "Local hash calculated: %s",
                    local_hash_result.directory_hash,
                )
            ...
            if download_for_comparison:
                ...
                try:
                    ...
                    if download_result["status"] == "downloaded":
                        local_hash_result = calculate_chromadb_hash(
                            temp_dir, algorithm=s3_hash_result.hash_algorithm
                        )
                        logger.info(
                            "Downloaded and calculated hash: %s",
                            local_hash_result.directory_hash,
                        )
                    else:
                        return {
        ...
        if status == "identical":
            ...
            logger.info(
                "❌ ChromaDB sync verification: DIFFERENT - %s",
                comparison["message"],
            )
...
def upload_snapshot_atomic(
    ...
    try:
        ...
        logger.info(
            "DEBUG: Starting atomic snapshot upload: collection=%s, version=%s, local_path=%s",
            collection,
            version_id,
            local_path,
        )
        logger.info(
            "DEBUG: Atomic upload paths - versioned_key=%s, pointer_key=%s",
            versioned_key,
            pointer_key,
        )
        ...
        logger.info(
            "DEBUG: Step 1 - uploading to versioned location: %s",
            versioned_key,
        )
        ...
        logger.info(
            "DEBUG: Step 1 result - status=%s, hash=%s",
        ...
        if upload_result.get("status") != "uploaded":
            logger.error(
                "DEBUG: Step 1 failed - upload_result=%s", upload_result
            )
            return {
                "status": "error",
        ...
        if lock_manager and not lock_manager.validate_ownership():
            ...
            try:
                ...
                logger.info(
                    "DEBUG: Cleaned up versioned upload after lock loss: %s",
                    versioned_key,
                )
        ...
        try:
            ...
            logger.warning(
                "DEBUG: Step 4 warning - failed to cleanup old versions: %s",
                cleanup_error,
            )

        logger.info(
            "DEBUG: Atomic snapshot upload completed successfully: collection=%s, version=%s",
            collection,
            version_id,
        )
        return {
    ...
    try:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "NoSuchKey":
                ...
                logger.info(
                    "No pointer found, falling back to /latest/ path: %s",
                    collection,
                )
...
def _cleanup_old_snapshot_versions(
    s3_client, bucket: str, collection: str, keep_versions: int
) -> None:
