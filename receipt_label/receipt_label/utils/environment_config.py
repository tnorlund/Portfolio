...
class AIUsageEnvironmentConfig:
    ...
    def _get_auto_tags(environment: Environment) -> Dict[str, Any]:
        ...
        if environment == Environment.CICD:
            ...
            tags.update({
                ...
                'instance_id': os.getenv('AWS_LAMBDA_FUNCTION_NAME', 
    ...
    @staticmethod
    def get_table_name(base_table_name: str, environment: Optional[Environment] = None) -> str:
        """
    ...
    def validate_environment_isolation(
        ...
        """
        expected_suffix = AIUsageEnvironmentConfig._get_table_suffix(expected_environment)
        
        if expected_environment == Environment.PRODUCTION:
            # Production should not have a suffix
            return not any(table_name.endswith(f"-{env.value}") 
                          for env in Environment if env != Environment.PRODUCTION)
        else:
