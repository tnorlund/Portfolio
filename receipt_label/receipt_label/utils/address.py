...
def parse_address(address: str) -> Dict[str, str]:
    ...
    if parts:
        ...
        unit_match = re.search(r"(?:suite|ste|apt|apartment)\s+(\w+)", street_part.lower())
        if unit_match:
            components["unit"] = unit_match.group(1)
            street_part = re.sub(r"(?:suite|ste|apt|apartment)\s+\w+", "", street_part, flags=re.IGNORECASE).strip()
...
def compare_addresses(addr1: str, addr2: str) -> float:
    ...
    for key in ["street_number", "street_name", "street_type", "city", "state", "zip_code"]:
