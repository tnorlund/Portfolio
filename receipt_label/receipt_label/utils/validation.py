...
def validate_business_name(
    ...
    if confidence >= min_confidence:
        ...
        return False, f"Business names don't match: '{receipt_name}' vs '{api_name}'", confidence
...
def validate_phone_number(
    ...
    if receipt_clean == api_clean:
        ...
        return False, f"Phone numbers don't match: '{receipt_phone}' vs '{api_phone}'", 0.0
...
def validate_address(
    ...
    if confidence >= min_confidence:
        ...
        return False, f"Addresses don't match: '{receipt_address}' vs '{api_address}'", confidence
...
def validate_receipt_format(
    ...
    confidence = 1.0 - (len(violations) / (len(receipt_data.get("lines", [])) + len(format_rules["required_sections"])))
