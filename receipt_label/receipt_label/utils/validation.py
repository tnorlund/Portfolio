...
def validate_business_name(receipt_name: str, api_name: str) -> Tuple[bool, str, float]:
    ...
    # Remove common words
    common_words = {"the", "a", "an", "and", "&", "inc", "llc", "ltd", "corp", "corporation"}
    ...
    message = "Business names match" if is_valid else "Business names do not match sufficiently"
    
    return is_valid, message, confidence


def validate_phone_number(receipt_phone: str, api_phone: str) -> Tuple[bool, str, float]:
    ...
    receipt_digits = receipt_digits[-10:] if len(receipt_digits) >= 10 else receipt_digits
...
def validate_datetime(date_str: str, time_str: Optional[str] = None, max_age_days: Optional[int] = None) -> Tuple[bool, str, float]:
...
def validate_amounts(subtotal: float, tax: float, total: float, tolerance: float = 0.01) -> Tuple[bool, str, float]:
...
def validate_receipt_data(data: Dict, required_fields: Optional[List[str]] = None) -> Tuple[bool, List[str], float]:
...
def validate_receipt_format(
    ...
    if "sections" in receipt_data:
        total_checks += 1
        missing_sections = set(format_rules["required_sections"]) - set(receipt_data["sections"])
        if missing_sections:
