...
class ChromaCompactor:
        ...
        try:
            with tempfile.TemporaryDirectory() as workdir:
                ...
                chroma = ChromaDBClient(
                    persist_directory=str(snapshot_dir),
                    mode="snapshot"
                )
                ...
                for delta_key in delta_keys:
                    try:
                        ...
                        if deltas_processed % 10 == 0:
                            self.dynamo_client.update_compaction_lock_heartbeat(
                                self.lock_id, lock.owner
                            )
                            
                    except Exception as e:
                        logger.error(f"Error processing delta {delta_key}: {e}")
                ...
                new_snapshot_key = f"snapshot/{datetime.now(timezone.utc).isoformat()}/"
    ...
    def _acquire_lock(self) -> Optional[CompactionLock]:
        ...
        try:
            self.dynamo_client.acquire_compaction_lock(lock)
            logger.info(f"Acquired compaction lock with owner {lock.owner}")
            return lock
    ...
    def _release_lock(self, lock: CompactionLock) -> None:
        ...
        try:
            self.dynamo_client.release_compaction_lock(self.lock_id, lock.owner)
            logger.info(f"Released compaction lock {lock.owner}")
        except Exception as e:
    ...
    def _merge_collections(self, source: ChromaDBClient, target: ChromaDBClient) -> None:
        """
        ...
        for collection_name in collection_names:
            try:
                ...
                # ChromaDB's get() returns all items when called without filters
                ...
                if data["ids"]:
                    ...
                    logger.info(
                        f"Merged {len(data['ids'])} vectors from {collection_name}"
                    )
                    
            except Exception as e:
                logger.warning(f"Error merging collection {collection_name}: {e}")
