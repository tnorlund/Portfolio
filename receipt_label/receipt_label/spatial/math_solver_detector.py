...
class MathSolverDetector:
    ...
    def solve_receipt_math(
        self, currency_values: List[Tuple[float, PatternMatch]]
    ) -> List[MathSolution]:
        """
        ...
        """
        self.logger.info(
            f"Solving receipt math with {len(currency_values)} values"
        )
        ...
        for i in range(len(values) - 1, -1, -1):
            ...
            self.logger.info(
                f"Testing ${potential_total_value:.2f} as potential total"
            )
            ...
            if len(solutions) >= self.max_solutions:
                self.logger.info(
                    f"Reached global max solutions limit ({self.max_solutions}), stopping search"
                )
                break
    ...
    def _find_solutions_numpy_optimized(
        self,
        ...
        if len(values) <= 10:
            ...
            return self._find_solutions_for_total(
                values, total_amount, total_match
            )
        ...
        for tax_idx, (tax_value, tax_match) in enumerate(values):
            ...
            tax_solutions = self._numpy_subset_sum_dp(
                remaining_price_cents,
                remaining_values,
                subtotal_target_cents,
                tolerance_cents,
                total_match,
                tax_info=(tax_value, tax_match),
            )
            solutions.extend(tax_solutions)

        return solutions[: self.max_solutions]

    def _numpy_subset_sum_dp(
        self,
        ...
        tolerance_cents: int,
        total_match: PatternMatch,
        tax_info: Optional[Tuple[float, PatternMatch]] = None,
    ) -> List[MathSolution]:
        """
        ...
        if max_sum > 100000:  # ~$1000 limit
            self.logger.warning(
                f"Target too large for DP optimization: ${target_cents/100:.2f}, falling back to brute force"
            )
            return self._find_solutions_for_total(
                values, target_cents / 100, total_match
            )
            ...
            if dp[n, target]:
                ...
                for combination in combinations_found:
                    ...
                    if tax_info is None:
                        ...
                        self.logger.info(
                            f"Found direct sum (NumPy): {[v[0] for v in combination]} = ${combo_sum:.2f}"
                        )
                    else:
                        ...
                        self.logger.info(
                            f"Found with tax (NumPy): {[v[0] for v in combination]} + ${tax_value:.2f} = ${grand_total_value:.2f}"
                        )
    ...
    def _backtrack_numpy_solutions(
        self,
        dp: np.ndarray,
        price_cents: np.ndarray,
        values: List[Tuple[float, PatternMatch]],
    ...
    ) -> List[List[Tuple[float, PatternMatch]]]:
        """
        ...
        while stack and len(combinations) < max_combinations:
            ...
            if (
                price <= remaining_sum
                and new_remaining >= 0
                and new_remaining < dp.shape[1]
                and dp[i - 1, new_remaining]
            ):
    ...
    def _find_solutions_for_total(
        ...
        for size in range(1, min(len(values), 10) + 1):
            if len(solutions) >= self.max_solutions:
                self.logger.info(
                    f"Reached max solutions limit ({self.max_solutions}), stopping search"
                )
                break
            ...
            for combo in combinations(values, size):
                ...
                if abs(combo_sum - total_amount) <= self.tolerance:
                    ...
                    self.logger.info(
                        f"Found direct sum: {[v[0] for v in combo]} = ${combo_sum:.2f}"
                    )
            ...
            for size in range(1, min(len(remaining_values) + 1, 11)):
                ...
                for combo in combinations(remaining_values, size):
                    ...
                    if abs(combo_sum - target_subtotal) <= self.tolerance:
                        ...
                        if (
                            abs(combo_sum + tax_value - total_amount)
                            <= self.tolerance
                        ):
                            solutions.append(
                                MathSolution(
                            ...
                            self.logger.info(
                                f"Found with tax: {[v[0] for v in combo]} + ${tax_value:.2f} = ${total_amount:.2f}"
                            )
    ...
    def find_best_solution(
        ...
        price_column_x: Optional[float] = None,
    ) -> Optional[MathSolution]:
        """
            ...
            if price_column_x is not None:
                ...
                for _, match in solution.item_prices:
                    if hasattr(match.word, "bottom_right"):
                        x = (
                            match.word.bottom_right.get("x", 0)
                            if isinstance(match.word.bottom_right, dict)
                            else match.word.bottom_right[0]
                        )
