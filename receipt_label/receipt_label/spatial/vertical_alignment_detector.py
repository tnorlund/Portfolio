...
class PriceColumn:
    ...
    y_span: Optional[float] = (
        None  # Y-coordinate span of the column (for debugging)
    )
    x_alignment_tightness: Optional[float] = (
        None  # How tightly aligned are the X-coordinates
    )
    font_consistency: Optional[FontMetrics] = (
        None  # Font size analysis for the column
    )
...
class VerticalAlignmentDetector:
    ...
    def __init__(
        ...
        use_enhanced_clustering: bool = True,
    ):
        """
        Initialize detector.
    ...
    def detect_price_columns(
        self, currency_patterns: List[PatternMatch]
    ) -> List[PriceColumn]:
        """
        ...
        for pattern in currency_patterns:
            ...
            if (
                not x_coords
                and hasattr(word, "bounding_box")
                and word.bounding_box
            ):
        ...
        if self.use_enhanced_clustering:
            x_groups = self._enhanced_x_coordinate_clustering(
                pattern_positions
            )
        else:
        ...
        for group_x, group_data in x_groups.items():
            ...
            if self.use_enhanced_clustering:
                y_clustered_groups = self._enhanced_y_coordinate_clustering(
                    group_data
                )
            else:
            ...
            for cluster in y_clustered_groups:
                if len(cluster) >= 2:  # Each Y-cluster needs at least 2 prices
                    ...
                    if self.use_enhanced_clustering:
                        column = self._create_enhanced_price_column(
                            cluster, len(price_columns), group_x
                        )
                    else:
                        column = self._create_price_column_from_cluster(
                            cluster, len(price_columns)
                        )
    ...
    def _cluster_by_y_coordinate(
        ...
        if len(position_data) <= 3:
            self.logger.info(
                f"Y-clustering: Small X-group ({len(position_data)} items), keeping as single cluster"
            )
            return [position_data]
        ...
        self.logger.info(
            f"Y-clustering: {len(sorted_data)} items in X-group, using line-based clustering (max gap: {max_line_gap})"
        )
        ...
        for i in range(1, len(sorted_data)):
            ...
            self.logger.info(
                f"Y-clustering step {i}: line {current_line} vs prev line {previous_line}, gap={line_gap}"
            )
            ...
            if line_gap <= max_line_gap:
                ...
                self.logger.info(
                    f"  → Added to current cluster (size now {len(current_cluster)})"
                )
            else:
                ...
                if len(current_cluster) >= 1:
                    ...
                    self.logger.info(
                        f"  → Finished cluster with {len(current_cluster)} items"
                    )
        ...
        self.logger.info(
            f"Y-clustering result: {len(filtered_clusters)} clusters after filtering"
        )

        return filtered_clusters

    def _enhanced_y_coordinate_clustering(
        self, position_data: List[Dict]
    ) -> List[List[Dict]]:
        """
        ...
        self.logger.info(
            f"Enhanced Y-clustering: Processing {len(position_data)} items"
        )
        ...
        section_breaks = self._detect_section_breaks(
            position_data, spacing_analysis
        )
        ...
        for section_id, section_items in enumerate(sections):
            if len(section_items) >= 2:
                ...
                section_clusters = self._cluster_within_section(
                ...
                if font_metrics and (
                    font_metrics.is_larger_than_normal
                    or font_metrics.confidence > 0.7
                ):
                    final_clusters.append(section_items)
                    self.logger.info(
                        f"Enhanced Y-clustering: Kept single-item section due to distinctive font"
                    )
        ...
        filtered_clusters = [
            ...
            or (
                len(cluster) == 1
                and cluster[0]
                .get("font_metrics", FontMetrics(0, 0, False, False, 0))
                .is_larger_than_normal
            )
        ]

        self.logger.info(
            f"Enhanced Y-clustering: {len(sections)} sections → {len(filtered_clusters)} final clusters"
        )
        return filtered_clusters
    ...
    def _analyze_line_spacing(self, position_data: List[Dict]) -> Dict:
        ...
        self.logger.info(
            f"Line spacing analysis: avg={avg_spacing:.1f}, variance={spacing_variance:.1f}, "
    ...
    def _detect_section_breaks(
        self, position_data: List[Dict], spacing_analysis: Dict
    ) -> List[int]:
        """
        ...
        for gap_info in spacing_analysis["large_gaps"]:
            ...
            self.logger.info(
                f"Section break detected at line {gap_info['line_after']} "
        ...
        for i in range(1, len(sorted_data)):
            ...
            if current_metrics and previous_metrics:
                ...
                height_ratio = current_metrics.avg_height / (
                    previous_metrics.avg_height + 0.001
                )

                if (
                    height_ratio > 1.5 or height_ratio < 0.67
                ):  # 50% size change
                    line_id = sorted_data[i]["line_id"]
                    if line_id not in section_breaks:
                        section_breaks.append(line_id)
                        self.logger.info(
                            f"Section break detected at line {line_id} "
        ...
        for i in range(1, len(sorted_data)):
            ...
            if previous_pattern.pattern_type.name in [
                "CURRENCY",
                "UNIT_PRICE",
                "LINE_TOTAL",
            ] and current_pattern.pattern_type.name in [
                "GRAND_TOTAL",
                "SUBTOTAL",
                "TAX",
            ]:
    ...
    def _group_by_sections(
        self, position_data: List[Dict], section_breaks: List[int]
    ) -> List[List[Dict]]:
        """
        ...
        self.logger.info(
            f"Grouped into {len(sections)} sections with sizes: {[len(s) for s in sections]}"
        )
        return sections

    def _cluster_within_section(
        self, section_items: List[Dict], section_id: int
    ) -> List[List[Dict]]:
        """
        ...
        has_totals = any(
            item["pattern"].pattern_type.name
            in ["GRAND_TOTAL", "SUBTOTAL", "TAX"]
            for item in section_items
        )
        has_large_fonts = any(
            item.get(
                "font_metrics", FontMetrics(0, 0, False, False, 0)
            ).is_larger_than_normal
            for item in section_items
        )
    ...
    def _cluster_totals_section(
        ...
        """
        self.logger.info(
            f"Clustering totals section {section_id} with {len(section_items)} items"
        )
        ...
        for i in range(1, len(sorted_items)):
            line_gap = (
                sorted_items[i]["line_id"] - sorted_items[i - 1]["line_id"]
            )
        ...
        self.logger.info(
            f"Totals section {section_id}: {len(clusters)} clusters"
        )
        return clusters

    def _cluster_items_section(
        self, section_items: List[Dict], section_id: int
    ) -> List[List[Dict]]:
        """
        ...
        """
        self.logger.info(
            f"Clustering items section {section_id} with {len(section_items)} items"
        )
        ...
        self.logger.info(
            f"Items section {section_id}: {len(filtered_clusters)} clusters (filtered from {len(clusters)})"
        )
        return filtered_clusters

    def _enhanced_x_coordinate_clustering(
        self, pattern_positions: List[Dict]
        ...
        """
        self.logger.info(
            f"Enhanced X-clustering: Processing {len(pattern_positions)} patterns"
        )

        # Add font metrics to each position
        for pos_data in pattern_positions:
            pos_data["font_metrics"] = self._analyze_font_metrics(
                pos_data["pattern"]
            )
        ...
        self._cluster_by_edge_alignment(
            pattern_positions, x_groups, "x_right", "right-edge"
        )
        ...
        unclustered = [
            ...
            if not self._is_already_clustered(p, x_groups)
        ]
        if unclustered:
            self._cluster_by_edge_alignment(
                unclustered, x_groups, "x_left", "left-edge"
            )
        ...
        if unclustered:
            ...
            for pos_data in unclustered:
                ...
                for group_x, group_data in center_groups.items():
                    if (
                        abs(group_x - x_center)
                        < self.alignment_tolerance * 1.5
                    ):  # More permissive for centers
                        group_data.append(pos_data)
                        found_group = True
                        break
            ...
            for center_x, center_data in center_groups.items():
                if (
                    len(center_data) >= 1
                ):  # Even single centered items can be important (totals)
                    unique_key = center_x + 1000  # Offset to avoid conflicts
                    x_groups[unique_key] = center_data
                    self.logger.info(
                        f"Enhanced X-clustering: Created center-aligned group at {center_x:.3f} with {len(center_data)} items"
                    )

        self.logger.info(
            f"Enhanced X-clustering: Created {len(x_groups)} total groups"
        )
        return dict(x_groups)

    def _cluster_by_edge_alignment(
        self,
        patterns: List[Dict],
        x_groups: Dict,
        edge_key: str,
        ...
        for pos_data in patterns:
            ...
            for group_x, group_data in x_groups.items():
                if (
                    group_data
                    and abs(group_data[0][edge_key] - edge_coord)
                    < edge_tolerance
                ):
                    group_data.append(pos_data)
                    found_group = True
        ...
        for group_x, group_data in list(x_groups.items()):
            if len(group_data) == 1:
                ...
                if not (
                    ...
                    and (
                        font_metrics.is_larger_than_normal
                        or font_metrics.confidence > 0.8
                    )
                ):
                    ...
                    groups_created += 1
                    self.logger.info(
                        f"Enhanced X-clustering: Kept single {edge_type} item due to distinctive font"
                    )
            else:
                groups_created += 1

        self.logger.info(
            f"Enhanced X-clustering: Created {groups_created} {edge_type} groups"
        )
    ...
    def _compute_adaptive_font_thresholds(
                    ...
                    if hasattr(word, corner_attr):
                        ...
                        if corner:
                            if isinstance(corner, dict):
                                ...
                                y_coords.append(
                                    corner[1] if len(corner) > 1 else 0
                                )
        ...
        self.logger.debug(
            f"Adaptive font thresholds: small={small_threshold:.3f}, normal={normal_threshold:.3f}, large={large_threshold:.3f}"
        )
        self.logger.debug(
            f"Based on {n} words with heights - p20={p20:.3f}, p50={p50:.3f}, p80={p80:.3f}"
        )
    ...
    def _analyze_font_metrics(self, pattern: PatternMatch) -> FontMetrics:
        ...
        if hasattr(word, "bounding_box") and word.bounding_box:
            if isinstance(word.bounding_box, dict):
                ...
                # List format: [left, top, width, height]
                height = (
                    word.bounding_box[3] if len(word.bounding_box) > 3 else 0.0
                )
        ...
        if hasattr(self, "_font_thresholds"):
            small_threshold, normal_threshold, large_threshold = (
                self._font_thresholds
            )
        else:
        ...
        if is_larger:
            confidence = (
                0.9  # High confidence for large text (likely totals/headers)
            )
        elif is_smaller:
            confidence = (
                0.3  # Lower confidence for small text (might be noise)
            )
        elif small_threshold <= height <= large_threshold:
            confidence = 0.7  # Good confidence for normal text

        self.logger.debug(
            f"Font analysis for '{word.text}': height={height:.1f}, large={is_larger}, small={is_smaller}, confidence={confidence:.2f}"
        )
    ...
    def _create_enhanced_price_column(
        self, cluster: List[Dict], column_id: int, group_x: float
    ) -> PriceColumn:
        """
        ...
        x_variance = sum((x - avg_x_right) ** 2 for x in x_rights) / len(
            x_rights
        )
        x_alignment_tightness = 1.0 - min(
            x_variance / (self.alignment_tolerance**2), 1.0
        )

        # Calculate Y-proximity confidence
        y_span = max(y_positions) - min(y_positions)
        y_density = len(cluster) / (
            y_span + 0.001
        )  # Items per unit Y-distance
        ...
        if font_consistency and font_consistency.is_larger_than_normal:
            base_confidence *= 1.4
            self.logger.info(
                f"Column {column_id}: Font boost for large text (confidence +40%)"
            )

        # Boost confidence if column has totals
        has_total = any(
            p.pattern_type in [PatternType.GRAND_TOTAL, PatternType.SUBTOTAL]
            for p in patterns
        )
        if has_total:
            base_confidence *= 1.3
            self.logger.info(
                f"Column {column_id}: Total boost (confidence +30%)"
            )
        ...
        if x_alignment_tightness > 0.9:
            base_confidence *= 1.2
            self.logger.info(
                f"Column {column_id}: Tight alignment boost (confidence +20%)"
            )

        confidence = min(base_confidence, 1.0)

        self.logger.info(
            f"Enhanced column {column_id} at X={x_center:.3f}, Y-span={y_span:.3f} "
    ...
    def _analyze_font_consistency(
        self, font_metrics_list: List[Optional[FontMetrics]]
        ...
        height_variance = (
            sum((h - avg_height) ** 2 for h in heights) / len(heights)
            if len(heights) > 1
            else 0.0
        )
        ...
        large_count = sum(
            1 for fm in valid_metrics if fm.is_larger_than_normal
        )
        small_count = sum(
            1 for fm in valid_metrics if fm.is_smaller_than_normal
        )
        ...
        base_confidence = sum(fm.confidence for fm in valid_metrics) / len(
            valid_metrics
        )
    ...
    def _create_price_column_from_cluster(
        self, cluster: List[Dict], column_id: int
    ) -> PriceColumn:
        """
        ...
        self.logger.info(
            f"Created price column {column_id} at X={x_center:.3f}, Y-span={y_span:.3f} "
    ...
    def match_products_to_column(
        self, words: List[ReceiptWord], price_column: PriceColumn
        ...
        for price_pattern in price_column.prices:
            ...
            for offset in range(
                ...
                for word in line_words:
                    ...
                    if hasattr(word, "bottom_right") and word.bottom_right:
                        ...
                        if isinstance(word.bounding_box, dict):
                            ...
                            word_x_right = (
                                word.bounding_box[0] + word.bounding_box[2]
                            )
                    else:
                        continue
                ...
                score = self._score_product_match(
                    product_words, price_pattern, abs(offset)
                )
            ...
            if best_product and best_score > 0.3:
                product_text = " ".join(
                    w.text
                    for w in sorted(best_product, key=lambda w: w.word_id)
                )
                            ...
                            if (
                                ...
                                if not has_price:
                                    description_text = " ".join(
                                        w.text
                                        for w in sorted(
                                            line_words, key=lambda w: w.word_id
                                        )
                                    )
                                    description_lines.append(description_text)
                                    has_indented_description = True
                                    self.logger.info(
                                        f"   Found indented description: '{description_text}'"
                                    )
                                else:
                ...
                log_msg = (
                    f"Matched '{product_text}' → {price_pattern.word.text} "
                )
                log_msg += (
    ...
    def detect_line_items_with_alignment(
        self, words: List[ReceiptWord], pattern_matches: List[PatternMatch]
    ) -> Dict:
        """
        ...
        self.logger.info(
            f"Processing {len(currency_patterns)} currency patterns for alignment"
        )
        ...
        for item in sorted(all_line_items, key=lambda i: i.product_line):
            ...
            if (
                item.product_text not in seen_products
                and price_id not in seen_prices
            ):
        ...
        enhanced_metadata = {
            ...
            "best_column_confidence": (
                max(c.confidence for c in price_columns)
                if price_columns
                else 0
            ),
            ...
            enhanced_metadata.update(
                {
                    ...
                    "font_consistency_confidence": (
                        best_column.font_consistency.confidence
                        if best_column.font_consistency
                        else 0
                    ),
                    "has_large_font_patterns": (
                        best_column.font_consistency.is_larger_than_normal
                        if best_column.font_consistency
                        else False
                    ),
    ...
    def _score_product_match(
        ...
        if any(
            keyword in product_text.upper() for keyword in metadata_keywords
        ):
            score -= 0.3

        return max(0.0, min(score, 1.0))

    def _analyze_line_indentations(
        self, words_by_line: Dict[int, List[ReceiptWord]]
    ) -> Dict[int, float]:
        """
    ...
    def _get_word_left_x(self, word: ReceiptWord) -> float:
        ...
        if (
            ...
            if isinstance(word.bounding_box, dict):
                ...
                x_coords.append(
                    word.bounding_box[0]
                    if len(word.bounding_box) > 0
                    else float("inf")
                )

        return min(x_coords) if x_coords else float("inf")

    def _is_word_in_column(
        self, word: ReceiptWord, column: PriceColumn
    ) -> bool:
        """
        ...
        if hasattr(word, "bottom_right") and word.bottom_right:
            if isinstance(word.bottom_right, dict):
                ...
                x_coords.append(
                    word.bottom_right[0] if len(word.bottom_right) > 0 else 0
                )
        ...
        if hasattr(word, "bottom_left") and word.bottom_left:
            if isinstance(word.bottom_left, dict):
                ...
                x_coords.append(
                    word.bottom_left[0] if len(word.bottom_left) > 0 else 0
                )
        ...
        return (
            column.x_min - tolerance
            <= word_center_x
            <= column.x_max + tolerance
        )

    def _detect_multi_column_layout(
        self, price_columns: List[PriceColumn]
    ) -> List[PriceColumn]:
        """
        ...
        page_width = max(col.x_max for col in price_columns) - min(
            col.x_min for col in price_columns
        )
            ...
            for left_col in left_columns:
                for right_col in right_columns:
                    ...
                    if y_overlap > 0.5:  # Significant Y-overlap
                        ...
                        self.logger.info(
                            f"Multi-column layout detected: Column {left_col.column_id} (left) "
        ...
        for i, col1 in enumerate(price_columns):
            for col2 in price_columns[i + 1 :]:
                if (
                    abs(len(col1.prices) - len(col2.prices)) <= 2
                ):  # Similar item counts
                    x_separation = abs(col1.x_center - col2.x_center)

                    if (
                        0.3 < x_separation < 0.7
                    ):  # Columns are well-separated horizontally
                        self.logger.info(
                            f"Potential multi-column pair: Columns {col1.column_id} and {col2.column_id} "
    ...
    def _calculate_y_overlap(
        self, col1: PriceColumn, col2: PriceColumn
    ) -> float:
        """
        ...
        total_lines = max(
            col1_y_max - col1_y_min + 1, col2_y_max - col2_y_min + 1
        )
