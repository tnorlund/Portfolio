...
""",
    ...
    def _create_search_nearby_tool(self):
        ...
        @function_tool
        def search_nearby(lat: float, lng: float, radius: float) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    try:
                        ...
                        metadata, partial = self._extract_agent_results(run_result, attempt)
        ...
        for item in run_result.new_items:
            ...
            if hasattr(raw, "name") and raw.name == "tool_return_metadata":
                try:
                    ...
                    logger.error(f"Unexpected error parsing metadata on attempt {attempt}: {e}")
                    metadata = getattr(item, "output", None)
                break
            
            # Collect partial results
            elif hasattr(raw, "name") and raw.name in [
                "search_by_phone", "search_by_address", "search_by_text"
            ]:
                try:
                    partial_result = {
                        "function": raw.name,
