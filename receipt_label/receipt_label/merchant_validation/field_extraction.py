...
def extract_candidate_merchant_fields(words: List[ReceiptWord]) -> dict:
    ...
    if not result["name"] and words:
        ...
        top_words = [word.word for word in words[:5] if word.word.strip()]
        if top_words:
