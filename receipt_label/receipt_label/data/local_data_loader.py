...
class LocalDataLoader:
    ...
    def load_receipt_by_id(
        ...
        try:
            ...
            logger.error(f"Error loading receipt {image_id}/{receipt_id}: {e}")
            return None
    ...
    def load_receipt_with_labels(
        ...
        image_id: str,
        receipt_id: str
    ) -> Optional[Tuple[Receipt, List[ReceiptWord], List[ReceiptLine], List[ReceiptWordLabel]]]:
        """
    ...
    def _load_words(self, receipt_dir: Path) -> List[ReceiptWord]:
        ...
        if not words_file.exists():
            logger.warning(f"Words file not found: {words_file}")
            return []
    ...
    def _load_metadata(self, receipt_dir: Path) -> Optional[ReceiptMetadata]:
        ...
        try:
            with open(metadata_file) as f:
                data = json.load(f)
            return ReceiptMetadata.from_dict(data)
        except Exception as e:
