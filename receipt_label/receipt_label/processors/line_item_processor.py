...
from ..data.gpt import gpt_request_line_item_analysis, gpt_request_spatial_currency_analysis
...
SUBTOTAL_PATTERNS = [
    r'(?i)sub\s*total', r'(?i)subtotal', r'(?i)sub\-total', r'(?i)sub total', 
    r'(?i)net\s*total', r'(?i)net amount', r'(?i)amount before tax', r'(?i)merchandise'
]
TAX_PATTERNS = [
    r'(?i)tax(?!\s*exempt)', r'(?i)vat', r'(?i)gst', r'(?i)hst', r'(?i)sales tax', r'(?i)tax amount', 
    r'(?i)state tax', r'(?i)local tax', r'(?i)city tax', r'(?i)t\s*a\s*x'
]
TOTAL_PATTERNS = [
    r'(?i)total\b(?!.*subtotal)', r'(?i)amount due', r'(?i)amount paid', r'(?i)grand total', 
    r'(?i)balance due', r'(?i)balance', r'(?i)due\s*amount', r'(?i)amount\s*due',
    r'(?i)payment', r'(?i)pay total', r'(?i)order total', r'(?i)total payment', 
    r'(?i)to pay', r'(?i)please pay', r'(?i)payment due', r'(?i)due', r'(?i)final amount',
    r'(?i)final total', r'(?i)total amount', r'(?i)total to pay', r'(?i)total due'
]
DISCOUNT_PATTERNS = [
    r'(?i)discount', r'(?i)coupon', r'(?i)sale', r'(?i)markdown', r'(?i)promo', r'(?i)savings', 
    r'(?i)price adjustment', r'(?i)% off'
]
...
class FastPatternMatcher:
    ...
    def find_currency_amounts(self, text: str, line_id: str, position: Dict[str, int], word_id: Optional[int] = None) -> List[CurrencyMatch]:
    ...
    def process(
        ...
        for line in receipt_lines:
            ...
            if line_words:
                ...
                y_pos = (first_word.bounding_box['y'] if isinstance(first_word.bounding_box, dict) 
        ...
        sorted_line_ids = sorted(line_positions.keys(), key=lambda line_id: line_positions[line_id])
        ...
        for line_id, matches in amounts_by_line.items():
            ...
            lines_above = [l_id for l_id in sorted_line_ids 
                          if line_positions.get(l_id, 0) < curr_y_pos]
            if lines_above:
                # Get the closest line above by y-position
                closest_above = max(lines_above, key=lambda l_id: line_positions.get(l_id, 0))
                prev_line_text = line_texts.get(closest_above, "")
                
            # Find the closest line below
            lines_below = [l_id for l_id in sorted_line_ids 
                          if line_positions.get(l_id, 0) > curr_y_pos]
            if lines_below:
                # Get the closest line below by y-position
                closest_below = min(lines_below, key=lambda l_id: line_positions.get(l_id, 0))
                next_line_text = line_texts.get(closest_below, "")
            
            # For each currency amount in this line
            for match in matches:
                # Get position information
                x_pos = match.position.get('x', 0) if isinstance(match.position, dict) else getattr(match.position, 'x', 0)
                
                # Get all words to the left of this currency amount on the same line
                words_to_left = [w for w in receipt_words if w.line_id == line_id and 
                                (w.bounding_box['x'] if isinstance(w.bounding_box, dict) else w.bounding_box.x) < x_pos]
                
                # Get the description to the left (all words joined)
                if words_to_left:
                    # Sort words by x-position to maintain correct reading order
                    sorted_left_words = sorted(words_to_left, 
                                               key=lambda w: w.bounding_box['x'] if isinstance(w.bounding_box, dict) else w.bounding_box.x)
                ...
                if not left_text:
                    ...
                    line_start_words = [w.text for w in sorted([w for w in receipt_words if w.line_id == line_id], 
                                                           key=lambda w: w.bounding_box['x'] if isinstance(w.bounding_box, dict) else w.bounding_box.x)
                                     if (w.bounding_box['x'] if isinstance(w.bounding_box, dict) else w.bounding_box.x) < x_pos]
                    left_text = " ".join(line_start_words) if line_start_words else f"Item on line {line_id}"
                ...
                currency_contexts.append({
                    ...
                    "x_position": x_pos,
                    "y_position": match.position.get('y', 0) if isinstance(match.position, dict) else getattr(match.position, 'y', 0),
                    "full_line": line_text,
                ...
                if len(matches) > 1:
                    uncertain_items.append(MultipleAmountsUncertainty(
                        line=next(l for l in receipt_lines if 
                                 (hasattr(l, 'line_id') and l.line_id == line_id) or 
                                 (isinstance(l, dict) and l.get('line_id', '') == line_id)),
        ...
        for context in currency_contexts:
            ...
            if any(pattern in line_text for pattern in ['subtotal', 'sub total', 'sub-total', 'net']):
                subtotal = amount
                continue
            ...
            if any(pattern in line_text for pattern in ['total', 'balance due', 'amount due', 'pay']):
                # Skip "subtotal" lines that also contain "total"
                if 'subtotal' not in line_text and 'sub total' not in line_text and 'sub-total' not in line_text:
                    total = amount
                    continue
            
            # If we get here, this might be a line item
            # Create a basic description from text to the left
            description = left_text.strip() if left_text else f"Item on line {context['line_id']}"
...
class LineItemProcessor:
    ...
    def _find_description_words(self, receipt_words: List[ReceiptWord], description: str, line_ids: List[int]) -> List[ReceiptWord]:
        """
        ...
        def is_price_like(text):
            ...
            return bool(re.match(r'^\$?\d+\.\d{2}$', text) or  # $XX.XX or XX.XX
                       re.match(r'^\d+$', text) or  # Just a number
                       text in ["$", "usd", "total", "subtotal", "tax"])
        ...
        if re.match(r'item on line \d+', clean_description):
            ...
            if len(clean_description.split()) <= 3:
                for word in line_words:
                    ...
                    if (word_text in clean_description or 
                        clean_description in word_text or
                        any(w in word_text for w in clean_description.split()) or
                        any(word_text in d for d in clean_description.split())):
                        matched_words.append(word)
    ...
    def _find_quantity_words(self, receipt_words: List[ReceiptWord], quantity: Quantity, line_ids: List[int]) -> Dict[str, List[ReceiptWord]]:
        """
        ...
        for word in line_words:
            ...
            elif any(word_text_lower.startswith(amt + unit_lower) 
    ...
    def _find_price_words(self, receipt_words: List[ReceiptWord], price: Decimal, line_ids: List[int]) -> List[ReceiptWord]:
        """
    ...
    def _create_word_labels(
        ...
        for item_idx, item in enumerate(initial_results.line_items):
            ...
            label_summary[item_key] = {
                "labels": [],
                "price": str(item.price.extended_price) if item.price and item.price.extended_price else "N/A",
                "quantity": str(item.quantity.amount) + " " + item.quantity.unit if item.quantity else "N/A"
            }
            ...
            if item.quantity:
                ...
                for word in quantity_words['unit']:
                    ...
                    if (word.line_id, word.word_id) in word_labels and word_labels[(word.line_id, word.word_id)]["label"] == ITEM_QUANTITY_LABEL:
                        # Update the reasoning to indicate it's both
                        word_labels[(word.line_id, word.word_id)]["reasoning"] += f" with unit: {item.quantity.unit}"
                    else:
                        word_labels[(word.line_id, word.word_id)] = {
            ...
            if item.price:
                ...
                if (item.price.extended_price and 
                    (not item.price.unit_price or item.price.extended_price != item.price.unit_price)):
        ...
        if initial_results.subtotal:
            for word in receipt_words:
                ...
                if clean_text == str(initial_results.subtotal) or clean_text == str(float(initial_results.subtotal)):
                    # Check if this word is near "subtotal" text
                    is_subtotal = False
                    nearby_words = [w for w in receipt_words if abs(w.line_id - word.line_id) <= 1]
        ...
        if initial_results.tax:
            for word in receipt_words:
                clean_text = re.sub(r'[$€£¥\s,]', '', word.text)
                if clean_text == str(initial_results.tax) or clean_text == str(float(initial_results.tax)):
        ...
        if initial_results.total:
            for word in receipt_words:
                clean_text = re.sub(r'[$€£¥\s,]', '', word.text)
                if clean_text == str(initial_results.total) or clean_text == str(float(initial_results.total)):
        ...
        for label_type, words_list in summary_labels.items():
            if label_type.lower() == "subtotal" and words_list:
                for word_info in words_list:
                    ...
                    word = type('', (), {})()
                    word.line_id = int(word_info['position'].split('L')[1].split('W')[0])
    ...
    def _log_label_summary(
        ...
        for i, item in enumerate(results.line_items):
            ...
            if word_labels:
                item_labels = [(key, label) for key, label in word_labels.items() 
                ...
                if item_labels:
                    ...
                    for (line_id, word_id), label in item_labels:
                        ...
                        logger.info(f"        • '{word_text}' → {label['label']} (L{line_id}W{word_id})")
                else:
        ...
        if word_labels:
            ...
            subtotal_labels = [(key, label) for key, label in word_labels.items() 
            ...
            tax_labels = [(key, label) for key, label in word_labels.items() 
            ...
            total_labels = [(key, label) for key, label in word_labels.items() 
    ...
    def _find_financial_summary_fields(self, receipt_lines: List[ReceiptLine], receipt_words: List[ReceiptWord]) -> Dict:
        """
        ...
        for line_id in candidate_line_ids:
            ...
            line_text = ' '.join([word.text for word in receipt_words if word.line_id == line_id])
        ...
        for line_id in candidate_line_ids:
            ...
            if any(re.search(pattern, line_text) for pattern in TAX_PATTERNS):
                ...
                if currency_amounts:
                    ...
                    right_aligned = sorted(currency_amounts, key=lambda x: x[1].bounding_box['x'] if hasattr(x[1], 'bounding_box') else 0, reverse=True)
        ...
        if not found_total:
            ...
            total_phrases = [
                ...
                "amount", "payment", "charge", "sum", "final"
            ]
            ...
            for line_id in candidate_line_ids:
                line = receipt_lines_by_id[line_id]
                line_text = ' '.join([word.text for word in receipt_words if word.line_id == line_id]).lower()
                ...
                if any(phrase in line_text for phrase in total_phrases):
                    ...
                    for word in [w for w in receipt_words if w.line_id == line_id]:
                        try:
                            ...
                            if re.match(r'\d+\.\d{2}', clean_text):
                                ...
                                score = sum(1 for phrase in total_phrases if phrase in line_text.lower())
                    ...
                    if currency_amounts:
                        amount, word, score = max(currency_amounts, key=lambda x: (x[2], x[0]))
            ...
            if potential_totals:
                ...
                result['total']['value'] = amount
                result['total']['words'].append(word)
                logger.debug(f"Found implicit total: {amount} from line item: {line_text}")
    ...
    def _post_process_line_items(self, result: ProcessingResult) -> ProcessingResult:
        ...
        if not updated_result.total:
            ...
            for item in updated_result.line_items:
                if hasattr(item, 'description') and item.description:
                    ...
                    if any(phrase in desc_lower for phrase in total_phrases):
                        ...
                        score = sum(1 for phrase in total_phrases if phrase in desc_lower)
            ...
            if potential_total_items:
                ...
                if hasattr(best_match.price, 'extended_price') and best_match.price.extended_price:
                    logger.info(f"Using line item '{best_match.description}' as total: {best_match.price.extended_price}")
                    updated_result.total = best_match.price.extended_price
                    
                    # Keep all remaining items
                    updated_result.line_items = [
                        item for item in updated_result.line_items 
                        if item != best_match
                    ]
            ...
            if not updated_result.total and updated_result.line_items:
                line_items_sum = sum(item.price.extended_price or Decimal('0') for item in updated_result.line_items)
                if line_items_sum > Decimal('0'):
                    logger.info(f"Creating synthetic total from line items sum: {line_items_sum}")
                    updated_result.total = line_items_sum
                    
                    if updated_result.reasoning:
                        updated_result.reasoning += " Created total from sum of line items."
                    else:
                        updated_result.reasoning = "Created total from sum of line items."
    ...
    def _process_line_items(self, line_items: List[LineItem]) -> List[LineItem]:
        ...
        for item in line_items:
            ...
            if not hasattr(item, 'description') or not item.description:
                continue
            if not hasattr(item, 'price') or not item.price or not hasattr(item.price, 'extended_price'):
                continue
            ...
            is_total_item = any(term in desc_lower for term in [
                "total", "balance due", "amount due", "grand total", "payment total",
                "order total", "total amount", "total due", "payment due"
            ])
            
            # Check for exact subtotal-like items
            is_subtotal_item = any(term in desc_lower for term in [
                "subtotal", "sub total", "sub-total", "net total"
            ])
            
            # Check for exact tax-like items
            is_tax_item = any(term in desc_lower for term in [
                "tax", "vat", "gst", "hst", "sales tax", "tax amount"
            ])
            ...
            if is_total_item and item.price.extended_price > Decimal(0):
                ...
                logger.debug(f"Identified potential total: {item.description} - {item.price.extended_price}")
                continue
            
            if is_subtotal_item and item.price.extended_price > Decimal(0):
                # This is likely a subtotal item, not a regular line item
                logger.debug(f"Identified potential subtotal: {item.description} - {item.price.extended_price}")
                continue
            
            if is_tax_item and item.price.extended_price > Decimal(0):
                # This is likely a tax item, not a regular line item
                logger.debug(f"Identified potential tax: {item.description} - {item.price.extended_price}")
                continue
    ...
    async def process(
        ...
        try:
            ...
            logger.info("Stage 1: Running pattern analysis to detect all currency amounts")
            ...
            logger.info(f"Stage 2: Using GPT to classify {len(initial_result.currency_amounts) if hasattr(initial_result, 'currency_amounts') else 0} currency amounts")
            ...
            if not use_llm or not self.openai_api_key or not hasattr(initial_result, 'currency_amounts') or not initial_result.currency_amounts:
            ...
            logger.info(f"Stage 3: Using GPT to classify {len(currency_contexts)} currency amounts")
            
            # Process with GPT...
            try:
                # Process with GPT API (using the centralized function in gpt.py)
                if self.openai_api_key:
                    gpt_result, prompt, response_text = await gpt_request_spatial_currency_analysis(
                    receipt=receipt,
                    ...
                    if gpt_result:
                        ...
                        for item in gpt_result.get("line_items", []):
                            ...
                            try:
                                ...
                                logger.warning(f"Invalid amount in GPT response: {amount}")
                                continue
                            ...
                            if "quantity" in item and item["quantity"] is not None:
                                try:
                                    ...
                                    logger.warning(f"Invalid quantity in GPT response: {item['quantity']}")
                        ...
                        if "financial_summary" in gpt_result and isinstance(gpt_result["financial_summary"], dict):
                            # Extract values from financial_summary
                            if "subtotal" in gpt_result["financial_summary"] and gpt_result["financial_summary"]["subtotal"] is not None:
                                try:
                                    subtotal = Decimal(str(gpt_result["financial_summary"]["subtotal"]))
                                except (ValueError, InvalidOperation):
                                    logger.warning(f"Invalid subtotal in financial_summary: {gpt_result['financial_summary']['subtotal']}")
                            
                            if "tax" in gpt_result["financial_summary"] and gpt_result["financial_summary"]["tax"] is not None:
                                try:
                                    tax = Decimal(str(gpt_result["financial_summary"]["tax"]))
                                except (ValueError, InvalidOperation):
                                    logger.warning(f"Invalid tax in financial_summary: {gpt_result['financial_summary']['tax']}")
                            
                            if "total" in gpt_result["financial_summary"] and gpt_result["financial_summary"]["total"] is not None:
                                try:
                                    total = Decimal(str(gpt_result["financial_summary"]["total"]))
                                except (ValueError, InvalidOperation):
                                    logger.warning(f"Invalid total in financial_summary: {gpt_result['financial_summary']['total']}")
                        ...
                        if subtotal is None or tax is None or total is None:
                            ...
                            for item in gpt_result.get("classification", []):
                                ...
                                try:
                                    ...
                                    logger.warning(f"Invalid amount in GPT classification: {amount}")
                                    continue
                        ...
                        if subtotal is None or tax is None or total is None:
                            logger.info("Some financial fields missing, attempting to infer them...")
                            ...
                            if subtotal is None and classified_items:
                                potential_subtotal = sum(
                                    ...
                                    if hasattr(item, 'price') and item.price and item.price.extended_price
                                )
                                if potential_subtotal > Decimal('0'):
                                    logger.info(f"Inferred subtotal from sum of line items: {potential_subtotal}")
                            ...
                            if tax is None and subtotal is not None:
                                ...
                                for item in classified_items:
                                    if (hasattr(item, 'price') and item.price and item.price.extended_price and
                                        item.price.extended_price > Decimal('0')):
                                        tax_ratio = item.price.extended_price / subtotal
                                        if Decimal('0.03') < tax_ratio < Decimal('0.15'):  # Common tax rates
                                            logger.info(f"Potential tax found: {item.price.extended_price} ({tax_ratio:.2%} of subtotal)")
                                            if tax is None or item.price.extended_price > tax:
                                                tax = item.price.extended_price
                                                # Remove this item from line items as it's likely tax
                                                classified_items = [i for i in classified_items if i != item]
                            ...
                            if total is None:
                                ...
                                if subtotal is not None and tax is not None:
                                    total = subtotal + tax
                                    logger.info(f"Inferred total from subtotal + tax: {total}")
                                # Otherwise, look for the largest amount near the end of the receipt
                                    ...
                                    items_by_line = sorted(
                                        [(int(item.line_ids[0]) if item.line_ids else 0, item) for item in classified_items],
                                        key=lambda x: x[0],
                                        reverse=True
                                    )
                                    
                                    # Take the top few items and find the largest amount
                                    potential_totals = items_by_line[:5]  # Look at the last 5 lines
                                    if potential_totals:
                                        largest_amount_item = max(
                                            [item for _, item in potential_totals if hasattr(item, 'price') and item.price and item.price.extended_price],
                                            key=lambda x: x.price.extended_price,
                                            default=None
                                        )
                                        
                                        if largest_amount_item and largest_amount_item.price.extended_price > Decimal('0'):
                                            logger.info(f"Identified potential total from position and value: {largest_amount_item.price.extended_price}")
                                            total = largest_amount_item.price.extended_price
                                            # Remove this item from line items as it's likely the total
                                            classified_items = [i for i in classified_items if i != largest_amount_item]
                        ...
                        initial_result = ProcessingResult(
                            ...
                            tax=tax,
                            total=total,
                            reasoning=initial_result.reasoning + " Refined through GPT spatial analysis.",
            ...
            try:
                return LineItemAnalysis(
                    ...
                    tax=initial_result.tax,
                    total=initial_result.total,
                    discrepancies=[f"{item}" for item in validation_results.get("warnings", [])] + 
