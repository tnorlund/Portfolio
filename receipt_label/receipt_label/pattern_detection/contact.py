...
class ContactPatternDetector(PatternDetector):
        ...
        for word in words:
            ...
            if phone_match:
                ...
                if "is_international" in phone_match:
                    metadata["is_international"] = phone_match[
                        "is_international"
                    ]
    ...
    def _match_phone_pattern(self, text: str) -> Optional[Dict]:
        ...
        if match := self._compiled_patterns["international"].search(
            ...
            # Validate country code for international numbers
            if matched_text.startswith('+'):
                country_code_match = re.match(r'\+(\d{1,3})', matched_text)
                if country_code_match:
                    country_code = int(country_code_match.group(1))
                    # List of valid country codes (partial - major ones)
                    valid_codes = {1, 7, 20, 27, 30, 31, 32, 33, 34, 36, 39, 40, 41, 43, 44, 45, 46, 47, 48, 49,
                                  ...
                                  670, 672, 673, 674, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685, 686,
            ...
            if (
                10 <= len(digits) <= 15 and has_structure
            ):  # Valid phone length with structure
                return {
        ...
        if match := self._compiled_patterns["us_standard"].search(
            ...
            if len(digits) == 10 or (len(digits) == 11 and digits[0] == "1"):
                ...
                if match_in_original:
                    ...
                    if remaining_text and re.match(
                        r"[-.\s]*\d", remaining_text
                    ):
        ...
        if match := self._compiled_patterns["short"].search(text):
            ...
            if "." in matched_text and not re.search(
                r"[\d\-\(\)\s]", matched_text
            ):
                return None
            ...
            if re.search(r"[A-Za-z]", matched_text):
                ...
                if re.search(r"\d", matched_text) and (
                    re.search(r"1-?800|1-?888|1-?877|1-?866", matched_text)
                    or len(re.sub(r"[^\d]", "", matched_text)) >= 7
                ):
                    return {
                        "matched_text": matched_text,
                        "normalized": self._normalize_phone(matched_text),
