...
from receipt_label.pattern_detection.patterns_config import PatternConfig, classify_word_type, is_noise_word
...
class PatternOptimizer:
    ...
    def should_run_detector(detector_type: str, words: List[ReceiptWord]) -> bool:
        ...
        for word in words:
            ...
            if detector_type == "currency" and ("currency_like" in types or "numeric" in types):
    ...
    @staticmethod
    def filter_words_for_detector(detector_type: str, words: List[ReceiptWord]) -> List[ReceiptWord]:
        """
        ...
        if detector_type == "currency":
            return [w for w in words if not w.is_noise and 
                   any(c in w.text for c in "0123456789$€£¥₹¢.,")]
        
        elif detector_type == "contact":
            return [w for w in words if not w.is_noise and 
                   ("@" in w.text or any(f".{tld}" in w.text.lower() 
                                       for tld in PatternConfig.COMMON_TLDS) or
                    any(c in w.text for c in "0123456789()-. +"))]
            ...
            return [w for w in words if not w.is_noise and 
                   (any(c in w.text for c in "0123456789@x") or 
                    any(unit in w.text.lower() for unit in 
                        ["qty", "each", "lbs", "oz", "kg", "pack", "box"]))]
...
class BatchPatternMatcher:
    ...
    @staticmethod
    def combine_patterns(patterns: Dict[str, str], group_names: bool = True) -> Tuple[re.Pattern, Dict[int, str]]:
        """
        ...
        if group_names:
            ...
            combined_pattern = "|".join(combined_parts)
            group_mapping = {i + 1: name for i, name in enumerate(patterns.keys())}
        
        return re.compile(combined_pattern, re.IGNORECASE), group_mapping
    
    @staticmethod
    def batch_match_text(combined_pattern: re.Pattern, group_mapping: Dict[int, str], 
                        text_list: List[str]) -> Dict[str, List[Tuple[str, re.Match]]]:
        """
...
class ContextAnalyzer:
    ...
    @staticmethod
    def get_line_context(target_word: ReceiptWord, all_words: List[ReceiptWord], 
                        y_tolerance: float = 10.0) -> List[ReceiptWord]:
        """
    ...
    @staticmethod
    def get_nearby_words(target_word: ReceiptWord, all_words: List[ReceiptWord], 
                        max_distance: float = 50.0, max_count: int = 5) -> List[Tuple[ReceiptWord, float]]:
        """
    ...
    @staticmethod
    def calculate_position_percentile(word: ReceiptWord, all_words: List[ReceiptWord]) -> Dict[str, float]:
        """
...
class KeywordMatcher:
    ...
    def find_keywords(self, text: str, categories: Optional[List[str]] = None) -> Dict[str, List[str]]:
        """
