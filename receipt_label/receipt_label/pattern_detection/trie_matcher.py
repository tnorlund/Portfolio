...
class TrieNode:
    ...
    def __init__(self):
        ...
        self.patterns: List[Tuple[str, PatternType]] = (
            []
        )  # (pattern, type) pairs
    ...
    def add_pattern(
        self, words: List[str], pattern: str, pattern_type: PatternType
    ):
...
class AhoCorasickMatcher:
    ...
    def compile(self):
        ...
        while queue:
            ...
            for char, child in current.children.items():
                ...
                if (
                    char in failure.children
                    and failure.children[char] != child
                ):
    ...
    def find_matches(self, words: List[ReceiptWord]) -> List[MultiWordMatch]:
        ...
        for i, word in enumerate(words):
            ...
            while (
                current_node != self.root
                and word_text not in current_node.children
            ):
            ...
            matches.extend(
                self._collect_matches_at_position(
    ...
    def _collect_matches_at_position(
        self,
        node: TrieNode,
        words: List[ReceiptWord],
        end_position: int,
        non_noise_positions: List[int],
    ) -> List[MultiWordMatch]:
        ...
        while current:
            if current.is_end_of_pattern:
                for pattern, pattern_type in current.patterns:
                    ...
                    current_non_noise_idx = non_noise_positions.index(
                        end_position
                    )

                    # Calculate the start index in the non-noise sequence
                    start_non_noise_idx = max(
                        0, current_non_noise_idx - pattern_length + 1
                    )

                    # Extract matching words using actual positions
                    matched_words = []
                    for idx in range(
                        start_non_noise_idx, current_non_noise_idx + 1
                    ):
                    ...
                    if matched_words:
                        ...
                        if start_non_noise_idx < len(non_noise_positions):
                            start_position = non_noise_positions[
                                start_non_noise_idx
                            ]
                        else:
                            start_position = 0
    ...
    def _calculate_match_confidence(
        self, pattern: str, words: List[ReceiptWord], match_type: MatchType
    ) -> float:
...
class MultiWordPatternDetector:
    ...
    def detect_multi_word_patterns(
    ...
    def _resolve_overlapping_matches(
        self, matches: List[MultiWordMatch]
    ) -> List[MultiWordMatch]:
        ...
        for match in matches:
            ...
            match_positions = set(
                range(match.start_position, match.end_position + 1)
            )
    ...
    def add_merchant_patterns(
        self, merchant_name: str, patterns: Dict[str, PatternType]
    ):
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_patterns": self.exact_matcher.pattern_count
            + self.fuzzy_matcher.pattern_count,
            "algorithm": "aho_corasick",
            "capabilities": [
