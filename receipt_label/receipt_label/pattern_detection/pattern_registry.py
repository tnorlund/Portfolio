...
class PatternDetectorRegistry:
    ...
    def get_detectors_by_category(self, category: DetectorCategory) -> List[DetectorMetadata]:
    ...
    def get_detectors_for_pattern(self, pattern_type: PatternType) -> List[DetectorMetadata]:
        """Get detectors that can handle a specific pattern type."""
        detector_names = self._pattern_to_detectors.get(pattern_type, [])
        return [self._detectors[name] for name in detector_names]
    
    def get_all_detectors(self, sort_by_priority: bool = True) -> List[DetectorMetadata]:
    ...
    def create_selective_detector_list(self, words: List[ReceiptWord], 
                                     categories: Optional[List[DetectorCategory]] = None) -> List[PatternDetector]:
        """
        ...
        if categories:
            ...
            for category in categories:
                candidate_detectors.extend(self.get_detectors_by_category(category))
        else:
    ...
    def _register_default_detectors(self) -> None:
        ...
        self.register_detector(DetectorMetadata(
            name="currency",
            ...
            supported_patterns=[
                ...
                PatternType.TAX, PatternType.DISCOUNT, PatternType.UNIT_PRICE, PatternType.LINE_TOTAL
            ],
        ...
        self.register_detector(DetectorMetadata(
            ...
            supported_patterns=[PatternType.PHONE_NUMBER, PatternType.EMAIL, PatternType.WEBSITE],
        ...
        self.register_detector(DetectorMetadata(
            ...
            supported_patterns=[PatternType.DATE, PatternType.TIME, PatternType.DATETIME],
        ...
        self.register_detector(DetectorMetadata(
            ...
            supported_patterns=[PatternType.QUANTITY, PatternType.QUANTITY_AT, PatternType.QUANTITY_TIMES, PatternType.QUANTITY_FOR],
...
class PatternDetectorFactory:
    ...
    def create_full_detectors() -> List[PatternDetector]:
        ...
        return [detector for detector in 
               [PATTERN_REGISTRY.create_detector(name) for name in PATTERN_REGISTRY._detectors.keys()]
               if detector is not None]
    
    @staticmethod
    def create_category_detectors(categories: List[DetectorCategory]) -> List[PatternDetector]:
        """Create detectors for specific categories only."""
        detectors = []
        for category in categories:
            for metadata in PATTERN_REGISTRY.get_detectors_by_category(category):
    ...
    @staticmethod
    def create_adaptive_detectors(words: List[ReceiptWord]) -> List[PatternDetector]:
        """
...
def get_detector_summary() -> Dict:
    ...
    # Pattern coverage
    for pattern_type, detector_names in PATTERN_REGISTRY._pattern_to_detectors.items():
