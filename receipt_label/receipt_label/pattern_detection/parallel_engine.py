...
class TrueParallelismEngine:
    ...
    async def execute_pattern_detection(
        self,
        words: List[ReceiptWord],
        detector_categories: Optional[List[str]] = None,
        ...
        try:
            ...
            if self._should_use_threading(words):
                result = await self._execute_with_threads(
                    words, detector_categories
                )
                used_threading = True
            else:
                result = await self._execute_with_asyncio(
                    words, detector_categories
                )
                used_threading = False
    ...
    def _has_complex_patterns(self, words: List[ReceiptWord]) -> bool:
        ...
        has_currency = any(
            "$" in w.text or any(c.isdigit() for c in w.text) for w in words
        )
    ...
    async def _execute_with_threads(
        self,
        words: List[ReceiptWord],
        detector_categories: Optional[List[str]],
    ) -> Dict[str, List[Any]]:
        ...
        if self.batch_processor and self.config.enable_batch_processing:
            ...
            for category, word_chunk in work_chunks.items():
                future = self._thread_pool.submit(
                    self._process_individual_category_sync,
                    category,
                    word_chunk,
                )
                futures_to_category[future] = category
        ...
        completed_futures = as_completed(
            futures_to_category.keys(), timeout=self.config.timeout_seconds
        )
    ...
    def _select_adaptive_categories(
        ...
        return (
            categories if categories else ["currency_and_quantity"]
        )  # Fallback

    def _create_work_chunks(
        self, words: List[ReceiptWord], categories: List[str]
    ...
    def _process_batch_category_sync(
        self, category: str, words: List[ReceiptWord]
    ) -> List[Any]:
        ...
        try:
            batch_results = self.batch_processor.process_words_batch(
                words, [category]
            )
    ...
    def _process_individual_category_sync(
        self, category: str, words: List[ReceiptWord]
    ) -> List[Any]:
    ...
    def _calculate_performance_gain(
            ...
            self._performance_history.append(
                {
                    ...
                    "used_threading": current_stats.get(
                        "used_threading", False
                    ),
                }
            )
        ...
        similar_executions = [
            exec_data
            for exec_data in self._performance_history
            if abs(exec_data["word_count"] - word_count) <= word_count * 0.2
        ]

        if similar_executions:
            avg_time = sum(e["time_ms"] for e in similar_executions) / len(
                similar_executions
            )
            performance_gain = (
                avg_time / current_time if current_time > 0 else 1.0
            )
...
OPTIMIZED_PATTERN_DETECTOR = ParallelPatternDetector(
