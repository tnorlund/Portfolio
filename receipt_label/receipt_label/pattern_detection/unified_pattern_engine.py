...
class MerchantPatternSet:
    ...
    transaction_patterns: Dict[str, str] = field(
        default_factory=dict
    )  # pattern -> description
    keyword_categories: Dict[str, Set[str]] = field(
        default_factory=dict
    )  # category -> keywords
...
class MerchantPatternManager:
    ...
    def add_merchant_patterns(self, merchant_set: MerchantPatternSet):
        ...
        self.merchant_patterns[merchant_set.merchant_name.lower()] = (
            merchant_set
        )
    ...
    def _detect_merchant_products(
        self,
        merchant_patterns: MerchantPatternSet,
        words: List[ReceiptWord],
        text: str,
    ) -> List[UnifiedMatch]:
    ...
    def _detect_transaction_patterns(
        ...
        for (
            pattern,
            description,
        ) in merchant_patterns.transaction_patterns.items():
    ...
    def _find_matching_words(
        self, words: List[ReceiptWord], pattern: str
    ) -> List[ReceiptWord]:
...
class UnifiedPatternEngine:
    ...
    async def detect_all_patterns(
        self, words: List[ReceiptWord], merchant_name: Optional[str] = None
    ) -> Dict[str, List[UnifiedMatch]]:
        """
        ...
        multi_word_matches = (
            self.multi_word_detector.detect_multi_word_patterns(words)
        )
        unified_multi_word = self._convert_multi_word_matches(
            multi_word_matches
        )
        ...
        unified_keywords = self._convert_keyword_matches(
        ...
        if merchant_name:
            merchant_matches = self.merchant_manager.detect_merchant_patterns(
                merchant_name, words
            )
        ...
        grouped_matches["_metadata"] = {
            ...
            "engines_used": ["trie", "automaton"]
            + (["merchant"] if merchant_name else []),
            "performance_stats": self.performance_stats.copy(),
        }

        return grouped_matches

    def _convert_multi_word_matches(
        self, multi_word_matches: List[MultiWordMatch]
    ) -> List[UnifiedMatch]:
    ...
    def _convert_keyword_matches(
        self, keyword_matches: List[AutomatonMatch], words: List[ReceiptWord]
    ) -> List[UnifiedMatch]:
        ...
        for match in keyword_matches:
            ...
            matching_words = self._find_words_for_text_range(
                words, match.text, match.start_pos, match.end_pos
            )
    ...
    def _find_words_for_text_range(
        self,
        words: List[ReceiptWord],
        matched_text: str,
        start_pos: int,
    ...
    def _resolve_and_optimize_matches(
        self, matches: List[UnifiedMatch]
    ) -> List[UnifiedMatch]:
    ...
    def add_merchant_patterns(
        self, merchant_name: str, patterns: Dict[str, Any]
    ):
        ...
        if "transaction_patterns" in patterns:
            merchant_set.transaction_patterns.update(
                patterns["transaction_patterns"]
            )

        if "keyword_categories" in patterns:
            merchant_set.keyword_categories.update(
                patterns["keyword_categories"]
            )
        ...
        if "keyword_categories" in patterns:
            self.keyword_matcher.add_merchant_keywords(
                merchant_name, patterns["keyword_categories"]
            )
        ...
        return {
            "unified_engine": {
                ...
                "engines": {
                    ...
                    "merchant_patterns": len(
                        self.merchant_manager.merchant_patterns
                    ),
                },
                "capabilities": [
