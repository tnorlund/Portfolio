...
class KeywordAutomaton:
    ...
    def add_keywords(self, category: str, keywords: Set[str]):
        ...
        for keyword in keywords:
            normalized = keyword.lower() if not self._case_sensitive else keyword
    ...
    def compile(self) -> re.Pattern:
        ...
        if not self.patterns:
            self.compiled_automaton = re.compile(r'(?!)', re.IGNORECASE)  # Never matches
            return self.compiled_automaton
...
class PhraseAutomaton:
    ...
    def _create_flexible_pattern(self, phrase: str) -> str:
        ...
        for word in words:
            ...
            # Allow o/0 confusion
            flexible_word = flexible_word.replace('o', '[o0]').replace('0', '[o0]')
            
            # Allow i/l confusion  
            flexible_word = flexible_word.replace('i', '[il]').replace('l', '[il]')
            
            # Allow s/5 confusion
            flexible_word = flexible_word.replace('s', '[s5]').replace('5', '[s5]')
            
            # Allow rn/m confusion
            flexible_word = flexible_word.replace('rn', '(?:rn|m)').replace('m', '(?:rn|m)')
    ...
    def find_matches(self, text: str) -> List[AutomatonMatch]:
        ...
        for category, pattern in self.compiled_patterns.items():
            for match in pattern.finditer(text):
                ...
                confidence = self._calculate_match_confidence(match.group(0), category)
                
                automaton_match = AutomatonMatch(
                    text=match.group(0),
                    pattern=self._find_closest_phrase(match.group(0), category),
                    category=category,
                    start_pos=match.start(),
                    end_pos=match.end(),
    ...
    def _calculate_match_confidence(self, matched_text: str, category: str) -> float:
        """Calculate confidence for a phrase match."""
        # Find the closest phrase in this category
        category_phrases = [phrase for phrase, cat in self.phrases.items() if cat == category]
...
class OptimizedKeywordMatcher:
    ...
    def _initialize_from_config(self):
        ...
        currency_automaton.add_keywords_from_config(PatternConfig.CURRENCY_KEYWORDS)
        ...
        financial_phrases.add_phrases("total", {
            "grand total", "final total", "total amount", "amount due", "balance due"
        })
        financial_phrases.add_phrases("subtotal", {
            "sub total", "subtotal", "merchandise total", "items total"
        })
        financial_phrases.add_phrases("tax", {
            "sales tax", "tax amount", "state tax", "city tax", "local tax"
        })
        financial_phrases.add_phrases("discount", {
            "store credit", "store discount", "member discount", "coupon savings"
        })
    ...
    def _resolve_overlapping_matches(self, matches: List[AutomatonMatch]) -> List[AutomatonMatch]:
    ...
    def add_merchant_keywords(self, merchant_name: str, categories: Dict[str, Set[str]]):
        """Add merchant-specific keywords."""
        merchant_automaton = KeywordAutomaton(f"merchant_{merchant_name.lower()}")
        
        for category, keywords in categories.items():
            merchant_automaton.add_keywords(category, keywords)
        
        merchant_automaton.compile()
        self.keyword_automata[f"merchant_{merchant_name.lower()}"] = merchant_automaton
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_automata": len(self.keyword_automata) + len(self.phrase_automata),
