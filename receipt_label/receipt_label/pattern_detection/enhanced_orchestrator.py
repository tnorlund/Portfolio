...
class OptimizationLevel(Enum):
    ...
    ADVANCED = "advanced"    # Phase 3: + Trie matching + Automata + Merchant patterns
...
class EnhancedPatternOrchestrator:
    ...
    async def detect_patterns(self, words: List[ReceiptWord], 
                            merchant_name: Optional[str] = None,
                            compare_performance: bool = False) -> Dict[str, Any]:
        """
        ...
        metrics = self._calculate_performance_metrics(results, processing_time, words)
    ...
    async def _run_legacy_detection(self, words: List[ReceiptWord]) -> Dict[str, Any]:
        ...
        legacy_orchestrator = ParallelPatternOrchestrator(use_adaptive_selection=False)
    ...
    async def _run_basic_detection(self, words: List[ReceiptWord]) -> Dict[str, Any]:
        """Run detection with Phase 1 optimizations (centralized patterns)."""
        from receipt_label.pattern_detection.orchestrator import ParallelPatternOrchestrator
        
        basic_orchestrator = ParallelPatternOrchestrator(use_adaptive_selection=False)
    ...
    async def _run_optimized_detection(self, words: List[ReceiptWord]) -> Dict[str, Any]:
    ...
    async def _run_advanced_detection(self, words: List[ReceiptWord], 
                                    merchant_name: Optional[str]) -> Dict[str, Any]:
        """Run detection with Phase 3 enhancements."""
        results = await UNIFIED_PATTERN_ENGINE.detect_all_patterns(words, merchant_name)
        ...
        return {
            ...
            "results": results,
            "engines_used": results.get("_metadata", {}).get("engines_used", []),
            "performance_data": results.get("_metadata", {}).get("performance_stats", {})
        }
    
    async def _run_performance_comparison(self, words: List[ReceiptWord], 
                                        merchant_name: Optional[str]) -> Dict[str, Any]:
        ...
        advanced_results = await self._run_advanced_detection(words, merchant_name)
        advanced_time = (time.time() - advanced_start) * 1000
        
        # Calculate performance gains
        time_improvement = (legacy_time - advanced_time) / legacy_time if legacy_time > 0 else 0
        ...
        return {
            "comparison": {
                "legacy": {
                    ...
                    "patterns_found": self._count_patterns(legacy_results["results"])
                },
                "advanced": {
                    "results": advanced_results,
                    "processing_time_ms": advanced_time,
                    "patterns_found": self._count_patterns(advanced_results["results"])
                },
                "performance_improvement": {
                    "time_reduction_percent": time_improvement * 100,
                    "speedup_factor": legacy_time / advanced_time if advanced_time > 0 else 1.0,
                    "optimizations_enabled": len(advanced_results["optimizations"])
                }
            }
        }
    ...
    def _calculate_performance_metrics(self, results: Dict[str, Any], 
                                     ...
                                     words: List[ReceiptWord]) -> PerformanceMetrics:
        ...
        if "results" in results:
            ...
            patterns_detected = self._count_patterns(results["pattern_results"])
        
        # Determine optimizations used
        optimizations_used = results.get("optimizations", [])
        
        # Estimate efficiency gains based on optimizations
        cpu_efficiency_gain = self._estimate_cpu_efficiency_gain(optimizations_used)
        memory_efficiency_gain = self._estimate_memory_efficiency_gain(optimizations_used)
        cost_reduction_estimate = self._estimate_cost_reduction(optimizations_used, patterns_detected)
    ...
    def _estimate_memory_efficiency_gain(self, optimizations: List[str]) -> float:
    ...
    def _estimate_cost_reduction(self, optimizations: List[str], patterns_found: int) -> float:
        ...
        if "trie_based_multi_word_detection" in optimizations:
            base_reduction += 0.3  # 30% fewer GPT calls for multi-word entities
        
        if "merchant_specific_patterns" in optimizations:
            base_reduction += 0.2  # 20% fewer GPT calls for known merchant items
        
        if "optimized_keyword_lookups" in optimizations:
            base_reduction += 0.1  # 10% fewer GPT calls for better classification
        ...
        return min(base_reduction, 0.84)  # Cap at the 84% target from the roadmap
...
async def detect_patterns_optimized(words: List[ReceiptWord], 
...
async def compare_optimization_performance(words: List[ReceiptWord],
                                         merchant_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    """
    orchestrator = EnhancedPatternOrchestrator(OptimizationLevel.ADVANCED)
    return await orchestrator.detect_patterns(words, merchant_name, compare_performance=True)
