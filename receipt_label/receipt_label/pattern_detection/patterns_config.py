...
class PatternConfig:
    ...
    CURRENCY_NUMBER = rf"(?:{FORMATTED_NUMBER}(?:\.\d{{1,2}})?|{DECIMAL_NUMBER})"
    ...
    CURRENCY_KEYWORDS = {
        "total": {
            "total", "grand total", "amount due", "balance due", "due", "pay",
            "total amount", "total due", "total price", "final total"
        },
        "subtotal": {
            "subtotal", "sub total", "sub-total", "merchandise", "net total",
            "items total", "goods total", "product total"
        },
        "tax": {
            "tax", "sales tax", "vat", "gst", "hst", "pst", "taxes",
            "tax amount", "total tax", "city tax", "state tax"
        },
        "discount": {
            "discount", "coupon", "savings", "save", "off", "reduction",
            "promo", "promotion", "special", "deal", "markdown"
        }
    }
    ...
    MERCHANT_PATTERNS = {
        ...
        "mcdonalds": {
            "products": {"Big Mac", "McFlurry", "Quarter Pounder", "McNuggets", "McChicken"},
        },
        "gas_station": {
            "gallons": r"(\d+\.\d+)\s*GAL",
    ...
    @classmethod
    def compile_patterns(cls, pattern_dict: Dict[str, str], flags: int = re.IGNORECASE) -> Dict[str, re.Pattern]:
...
def is_noise_word(text: str) -> bool:
    ...
    return any(pattern.match(text.strip()) for pattern in noise_patterns.values())
...
def classify_word_type(text: str) -> Set[str]:
    ...
    if "@" in text or any(f".{tld}" in text.lower() for tld in PatternConfig.COMMON_TLDS):
