...
class DevelopmentConfig:
    ...
    local_data_dir: str = field(
        default_factory=lambda: os.getenv(
            "RECEIPT_LOCAL_DATA_DIR", "./receipt_data"
        )
    )

    # API stubbing settings
    use_stub_apis: bool = field(
        default_factory=lambda: os.getenv("USE_STUB_APIS", "false").lower()
        == "true"
    )

    stub_openai: bool = field(
        default_factory=lambda: os.getenv("STUB_OPENAI", "false").lower()
        == "true"
    )

    stub_pinecone: bool = field(
        default_factory=lambda: os.getenv("STUB_PINECONE", "false").lower()
        == "true"
    )

    stub_places_api: bool = field(
        default_factory=lambda: os.getenv("STUB_PLACES_API", "false").lower()
        == "true"
    )

    stub_dynamodb: bool = field(
        default_factory=lambda: os.getenv("STUB_DYNAMODB", "false").lower()
        == "true"
    )

    # Local caching settings
    enable_local_cache: bool = field(
        default_factory=lambda: os.getenv("ENABLE_LOCAL_CACHE", "true").lower()
        == "true"
    )

    cache_dir: str = field(
        default_factory=lambda: os.getenv(
            "RECEIPT_CACHE_DIR", "./.cache/receipt_label"
        )
    )
    ...
    track_api_costs: bool = field(
        default_factory=lambda: os.getenv("TRACK_API_COSTS", "true").lower()
        == "true"
    )

    cost_log_file: str = field(
        default_factory=lambda: os.getenv(
            "COST_LOG_FILE", "./logs/api_costs.log"
        )
    )

    # Development mode settings
    verbose_logging: bool = field(
        default_factory=lambda: os.getenv("VERBOSE_LOGGING", "false").lower()
        == "true"
    )

    log_api_requests: bool = field(
        default_factory=lambda: os.getenv("LOG_API_REQUESTS", "false").lower()
        == "true"
    )

    # Performance settings
    max_parallel_patterns: int = field(
        default_factory=lambda: int(os.getenv("MAX_PARALLEL_PATTERNS", "4"))
    )

    pattern_timeout_seconds: int = field(
        default_factory=lambda: int(os.getenv("PATTERN_TIMEOUT_SECONDS", "30"))
    )

    # Sample data settings
    sample_data_size: int = field(
        default_factory=lambda: int(os.getenv("SAMPLE_DATA_SIZE", "20"))
    )

    download_images: bool = field(
        default_factory=lambda: os.getenv("DOWNLOAD_IMAGES", "false").lower()
        == "true"
    )
