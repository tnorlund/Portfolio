...
async def test_simplified_confidence():
    ...
    for i, file_path in enumerate(all_files):
        if i % 20 == 0:
            ...
            if i > 0:
                ...
                print(f"   Progress: {i}/{len(all_files)} ({i/len(all_files)*100:.1f}%) - ETA: {eta:.1f}s")
        ...
        try:
            ...
            results.append({
                ...
                'best_column_confidence': spatial_analysis['best_column_confidence'],
                'x_alignment_tightness': spatial_analysis['x_alignment_tightness'],
                'font_consistency': spatial_analysis['font_consistency_confidence'],
                'has_large_fonts': spatial_analysis['has_large_font_patterns'],
    ...
    if successful:
        ...
        for confidence_level in ['high_confidence', 'medium_confidence', 'low_confidence', 'no_solution']:
            count = confidence_dist.get(confidence_level, 0)
            print(f"   {confidence_level}: {count} ({count/len(successful)*100:.1f}%)")
        ...
        new_cost = (high_conf * 0.0) + (medium_conf * 0.3) + (low_conf * 1.0) + (no_solution * 1.0)
        cost_reduction = ((current_cost - new_cost) / current_cost) * 100
        
        print(f"\n💰 COST REDUCTION ANALYSIS:")
        print(f"   High confidence (no Pinecone): {high_conf} ({high_conf/len(successful)*100:.1f}%)")
        print(f"   Medium confidence (light Pinecone): {medium_conf} ({medium_conf/len(successful)*100:.1f}%)")
        print(f"   🎯 COST REDUCTION: {cost_reduction:.1f}%")
        
        # Phase 2 feature usage
        phase2_features = {
            'tight_alignment': sum(1 for r in successful if r.get('x_alignment_tightness', 0) > 0.9),
            'font_analysis': sum(1 for r in successful if r.get('font_consistency', 0) > 0),
            'large_fonts': sum(1 for r in successful if r.get('has_large_fonts', False)),
            'indented_items': sum(1 for r in successful if r.get('has_indented_items', False))
        }
        
        print(f"\n✨ PHASE 2 FEATURE USAGE:")
        print(f"   Tight X-alignment (>0.9): {phase2_features['tight_alignment']} ({phase2_features['tight_alignment']/len(successful)*100:.1f}%)")
        print(f"   Font analysis available: {phase2_features['font_analysis']} ({phase2_features['font_analysis']/len(successful)*100:.1f}%)")
        print(f"   Large font detection: {phase2_features['large_fonts']} ({phase2_features['large_fonts']/len(successful)*100:.1f}%)")
        print(f"   Indented descriptions: {phase2_features['indented_items']} ({phase2_features['indented_items']/len(successful)*100:.1f}%)")
        ...
        for merchant, count in merchant_stats.most_common():
            ...
            high_conf_count = sum(1 for c in confidences if c == 'high_confidence')
            print(f"   {merchant}: {count} receipts, {high_conf_count} high confidence ({high_conf_count/count*100:.1f}%)")
