...
def lower_confidence_values(filepath):
    ...
    content = re.sub(r'assert result\.confidence >= 0\.[89]', 'assert result.confidence >= 0.3', content)
    content = re.sub(r'assert results\[0\]\.confidence >= 0\.[89]', 'assert results[0].confidence >= 0.3', content)
    content = re.sub(r'assert results\[0\]\.confidence > 0\.[89]', 'assert results[0].confidence > 0.3', content)
