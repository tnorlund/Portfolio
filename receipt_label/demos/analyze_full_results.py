...
def analyze_results(results_file: str = "results_all.json"):
    ...
    print(
        f"- Successful tests: {summary['successful_tests']} ({summary['successful_tests']/summary['total_images']*100:.1f}%)"
    )
    print(f"- Failed tests: {summary['failed_tests']}")
    print(
        f"- Total processing time: {data['summary']['total_images'] * summary['avg_processing_time_ms'] / 1000:.2f}s"
    )
    print(
        f"- Average processing time: {summary['avg_processing_time_ms']:.2f}ms per image"
    )
    ...
    for merchant, count in merchant_counts.most_common(10):
        ...
        print(
            f"  - {merchant}: {count} receipts, {total_words:,} words, {avg_time:.2f}ms avg"
        )
    ...
    # Processing speed
    words_per_ms = summary["total_words"] / (
        summary["successful_tests"] * summary["avg_processing_time_ms"]
    )
    ...
    print(
        f"- Estimated daily capacity: {receipts_per_second * 86400:,.0f} receipts"
    )

    # Extrapolation
    print("\n📈 SCALABILITY ANALYSIS:")
    print(
        f"With {summary['avg_processing_time_ms']:.2f}ms average processing time:"
    )
    print(
        f"  - 1,000 receipts: {1000 * summary['avg_processing_time_ms'] / 1000:.1f}s"
    )
    print(
        f"  - 10,000 receipts: {10000 * summary['avg_processing_time_ms'] / 1000:.1f}s"
    )
    print(
        f"  - 100,000 receipts: {100000 * summary['avg_processing_time_ms'] / 1000 / 60:.1f} minutes"
    )
    print(
        f"  - 1,000,000 receipts: {1000000 * summary['avg_processing_time_ms'] / 1000 / 3600:.1f} hours"
    )
    ...
    print(
        f"  - GPT-4 cost (if used): ~${summary['successful_tests'] * 0.01:.2f}"
    )
    ...
    print("\n✅ CONCLUSION:")
    print(
        f"Pattern detection successfully processed {summary['successful_tests']} real receipts"
    )
    print(
        f"from {len(merchant_counts)} different merchants in {summary['avg_processing_time_ms']:.2f}ms average."
    )
    print(
...
if __name__ == "__main__":
    ...
    if not Path(results_file).exists():
        ...
        print(
            "  python scripts/test_pattern_detection_with_exported_data.py ./receipt_data_all"
        )
        sys.exit(1)
