...
class PatternDetectionTester:
    ...
    async def run_comprehensive_test(
        self, max_receipts: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        test_results["performance_comparison"] = (
            await self._run_performance_comparison(available_receipts)
        )

        # Run optimization analysis
        logging.info("Running optimization analysis...")
        test_results["optimization_analysis"] = (
            await self._run_optimization_analysis(available_receipts)
        )

        # Run pattern coverage analysis
        logging.info("Running pattern coverage analysis...")
        test_results["pattern_coverage"] = (
            await self._run_pattern_coverage_analysis(available_receipts)
        )

        # Run merchant-specific analysis
        logging.info("Running merchant-specific analysis...")
        test_results["merchant_analysis"] = await self._run_merchant_analysis(
            available_receipts
        )

        # Save results
        test_results["metadata"]["test_completed"] = time.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        self._save_results(test_results)

        logging.info(
            f"Comprehensive test completed. Results saved to: {self.output_dir}"
        )
        return test_results
    ...
    async def _run_performance_comparison(
        ...
        for level in optimization_levels:
            ...
            level_results = await self._test_optimization_level(
                test_receipts, level
            )
    ...
    async def _test_optimization_level(
        self, receipts: List[tuple], level: OptimizationLevel
    ) -> Dict[str, Any]:
        ...
        for image_id, receipt_id in receipts:
            try:
                ...
                # Record stats
                patterns_found = detection_result.get(
                    "performance_metrics", {}
                ).get("patterns_detected", 0)
                ...
                results["per_receipt_stats"].append(
                    {
                        ...
                        "words_processed": len(
                            [w for w in words if not w.is_noise]
                        ),
                    }
                )
                ...
                error_msg = (
                    f"Error processing receipt {image_id}/{receipt_id}: {e}"
                )
        ...
        if results["receipts_processed"] > 0:
            results["average_processing_time_ms"] = (
                total_time / results["receipts_processed"]
            )

        return results

    def _calculate_performance_summary(
        self, level_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if (
            legacy["receipts_processed"] == 0
            or advanced["receipts_processed"] == 0
        ):
        ...
        time_improvement = (
            ((legacy_avg - advanced_avg) / legacy_avg) * 100
            if legacy_avg > 0
            else 0
        )
        ...
        return {
            ...
            "patterns_improvement": {
                ...
                "improvement": advanced["total_patterns_detected"]
                - legacy["total_patterns_detected"],
            },
        }
    ...
    async def _run_optimization_analysis(
        ...
        for image_id, receipt_id in test_receipts:
            try:
                ...
                comparison_result = await compare_optimization_performance(
                    words
                )

                if "comparison" in comparison_result:
                    comparison = comparison_result["comparison"]
                    optimization_analysis["optimizations_tested"][
                        f"{image_id}_{receipt_id}"
                    ] = {
                        "legacy_time_ms": comparison["legacy"][
                            "processing_time_ms"
                        ],
                        "advanced_time_ms": comparison["advanced"][
                            "processing_time_ms"
                        ],
                        "improvement": comparison["performance_improvement"],
                        "legacy_patterns": comparison["legacy"][
                            "patterns_found"
                        ],
                        "advanced_patterns": comparison["advanced"][
                            "patterns_found"
                        ],
                    }
                ...
                logging.error(
                    f"Error in optimization analysis for {image_id}/{receipt_id}: {e}"
                )

        # Analyze specific features
        optimization_analysis["feature_analysis"] = (
            await self._analyze_specific_features(test_receipts)
        )

        return optimization_analysis

    async def _analyze_specific_features(
        self, receipts: List[tuple]
    ) -> Dict[str, Any]:
        """Analyze specific enhanced features."""
        feature_analysis = {
            "trie_based_detection": {
                "total_matches": 0,
        ...
        for image_id, receipt_id in receipts:
            try:
                        ...
                        for match in matches:
                            if hasattr(match, "source_engine"):
                                if match.source_engine == "trie":
                                    feature_analysis["trie_based_detection"][
                                        "total_matches"
                                    ] += 1
                                    ...
                                    feature_analysis["merchant_patterns"][
                                        "total_matches"
                                    ] += 1
                                    ...
                                    feature_analysis["automaton_keywords"][
                                        "total_matches"
                                    ] += 1
                                    if match.is_multi_word:
                                        feature_analysis["automaton_keywords"][
                ...
                logging.error(
                    f"Error in feature analysis for {image_id}/{receipt_id}: {e}"
                )

        return feature_analysis

    async def _run_pattern_coverage_analysis(
        self, receipts: List[tuple]
    ) -> Dict[str, Any]:
        ...
        for image_id, receipt_id in receipts:
            try:
                ...
                logging.error(
                    f"Error in coverage analysis for {image_id}/{receipt_id}: {e}"
                )
        ...
        coverage_analysis["coverage_summary"] = {
            ...
            "average_patterns_per_receipt": total_patterns
            / max(coverage_analysis["total_receipts_analyzed"], 1),
    ...
    async def _run_merchant_analysis(
        self, receipts: List[tuple]
    ) -> Dict[str, Any]:
            ...
            for image_id, receipt_id in test_receipts:
                try:
                    ...
                    for category, matches in results.items():
                        ...
                        for match in matches:
                            if (
                                hasattr(match, "is_merchant_specific")
                                and match.is_merchant_specific
                            ):
                                merchant_results["total_matches"] += 1
                                merchant_results[
                                    "merchant_specific_matches"
                                ] += 1

                                # Check if it's a product match
                                if (
                                    hasattr(match, "pattern_type")
                                    and "PRODUCT" in match.pattern_type.value
                                ):
                                    merchant_results["product_matches"] += 1

                                # Check metadata for transaction patterns
                                if (
                                    hasattr(match, "metadata")
                                    and match.metadata.get("pattern_source")
                                    == "transaction_patterns"
                                ):
                                    merchant_results[
                                        "transaction_pattern_matches"
                                    ] += 1

                except Exception as e:
                    logging.error(
                        f"Error in merchant analysis for {merchant} {image_id}/{receipt_id}: {e}"
                    )
        ...
        merchant_analysis["merchant_comparison"] = {
            ...
            "merchants_with_matches": len(
                [
                    merchant
                    for merchant, result in merchant_analysis[
                        "merchant_results"
                    ].items()
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        if (
            "performance_comparison" in results
            and "summary" in results["performance_comparison"]
        ):
    ...
    def _generate_readable_report(self, results: Dict[str, Any]):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Test Date:** {metadata.get('test_started', 'Unknown')}\n"
            )
            f.write(
                f"**Receipts Tested:** {metadata.get('total_receipts', 'Unknown')}\n"
            )
            f.write(
                f"**Data Directory:** {metadata.get('data_directory', 'Unknown')}\n\n"
            )
            ...
            if "performance_comparison" in results:
                ...
                if "summary" in perf and "error" not in perf["summary"]:
                    ...
                    f.write(
                        f"**Time Improvement:** {summary.get('time_improvement_percent', 0):.1f}%\n"
                    )
                    f.write(
                        f"**Speedup Factor:** {summary.get('speedup_factor', 1.0):.2f}x\n"
                    )
                    f.write(
                        f"**Legacy Average:** {summary.get('legacy_avg_ms', 0):.2f}ms\n"
                    )
                    f.write(
                        f"**Advanced Average:** {summary.get('advanced_avg_ms', 0):.2f}ms\n\n"
                    )

                f.write("### Optimization Levels Tested\n\n")
                for level, data in perf.get("optimization_levels", {}).items():
                    f.write(
                        f"- **{level.title()}:** {data.get('receipts_processed', 0)} receipts, "
                    )
                    f.write(
                        f"{data.get('average_processing_time_ms', 0):.2f}ms avg, "
                    )
                    f.write(
                        f"{data.get('total_patterns_detected', 0)} patterns\n"
                    )
                f.write("\n")
            ...
            if "pattern_coverage" in results:
                ...
                f.write(
                    f"**Total Patterns Detected:** {summary.get('total_patterns_detected', 0)}\n"
                )
                f.write(
                    f"**Average per Receipt:** {summary.get('average_patterns_per_receipt', 0):.1f}\n"
                )
                f.write(
                    f"**Unique Pattern Types:** {summary.get('unique_pattern_types', 0)}\n\n"
                )
            ...
            if "merchant_analysis" in results:
                ...
                for merchant_name, data in merchant.get(
                    "merchant_results", {}
                ).items():
                    ...
                    f.write(
                        f"- **Merchant-Specific:** {data.get('merchant_specific_matches', 0)}\n"
                    )
                    f.write(
                        f"- **Product Matches:** {data.get('product_matches', 0)}\n"
                    )
                    f.write(
                        f"- **Transaction Patterns:** {data.get('transaction_pattern_matches', 0)}\n\n"
                    )

            # Enhancement Features
            if (
                "optimization_analysis" in results
                and "feature_analysis" in results["optimization_analysis"]
            ):
            ...
            f.write("---\n")
            f.write(
                f"*Report generated on {metadata.get('test_completed', 'Unknown')}*\n"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test pattern detection enhancements"
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--max-receipts", type=int, help="Maximum number of receipts to test"
    )
    parser.add_argument(
    ...
    try:
        ...
        if (
            ...
            if "error" not in summary:
                print(
                    f"Performance Improvement: {summary.get('time_improvement_percent', 0):.1f}%"
                )
                print(
                    f"Speedup Factor: {summary.get('speedup_factor', 1.0):.2f}x"
                )
        ...
        if "pattern_coverage" in results:
            coverage_summary = results["pattern_coverage"].get(
                "coverage_summary", {}
            )
            print(
                f"Total Patterns Detected: {coverage_summary.get('total_patterns_detected', 0)}"
            )
            print(
                f"Average Patterns per Receipt: {coverage_summary.get('average_patterns_per_receipt', 0):.1f}"
            )

        print(f"\nDetailed results saved to: {args.output_dir}")
        print(
            f"Report available at: {args.output_dir}/pattern_detection_report.md"
        )
