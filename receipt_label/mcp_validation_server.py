...
This server provides tools for validating receipt labels using DynamoDB and
Pinecone, automatically fetching configuration from the specified Pulumi stack.

This server is used to validate labels for receipts that are not in the
Pulumi stack.
"""
...
class ReceiptValidationServer:
    ...
    async def initialize_clients(self, stack_name: str):
        ...
        if "portfolio:OPENAI_API_KEY" in self.pulumi_config:
            config_values["OPENAI_API_KEY"] = self.pulumi_config[
                "portfolio:OPENAI_API_KEY"
            ]["value"]

        if "portfolio:PINECONE_API_KEY" in self.pulumi_config:
            config_values["PINECONE_API_KEY"] = self.pulumi_config[
                "portfolio:PINECONE_API_KEY"
            ]["value"]

        if "portfolio:PINECONE_INDEX_NAME" in self.pulumi_config:
            config_values["PINECONE_INDEX_NAME"] = self.pulumi_config[
                "portfolio:PINECONE_INDEX_NAME"
            ]["value"]

        if "portfolio:PINECONE_HOST" in self.pulumi_config:
            config_values["PINECONE_HOST"] = self.pulumi_config[
                "portfolio:PINECONE_HOST"
            ]["value"]

        # Get DynamoDB table name from outputs
        if (
            "outputs" in self.pulumi_config
            and "dynamoTableName" in self.pulumi_config["outputs"]
        ):
            config_values["DYNAMO_TABLE_NAME"] = self.pulumi_config["outputs"][
                "dynamoTableName"
            ]
    ...
    async def call_tool(
        ...
        if name == "initialize_stack":
            try:
                ...
                return [
                    TextContent(
                        type="text", text=f"Error initializing stack: {e}"
                    )
                ]
    ...
    async def _validate_labels(
        ...
        for label_info in labels:
            ...
            if label not in CORE_LABELS:
                results.append(
                    f"Word {word_id} ('{text}'): Invalid label '{label}' - not in CORE_LABELS"
                )
            else:
                results.append(
                    f"Word {word_id} ('{text}'): Valid label '{label}'"
                )

        return [TextContent(type="text", text="\n".join(results))]

    async def _query_similar_labels(
        self, args: Dict[str, Any]
    ) -> List[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error querying Pinecone: {e}")
            ]

    async def _persist_validated_labels(
        self, args: Dict[str, Any]
    ) -> List[TextContent]:
        ...
        try:
            ...
            for label_info in validated_labels:
                try:
                    label_entity = ReceiptWordLabel(
                        ...
                        word_id=label_info["word_id"],
                        label=label_info["label"],
                        reasoning=label_info.get(
                            "reasoning", "MCP validation"
                        ),
            ...
            result = (
                f"Persisted {success_count}/{len(validated_labels)} labels"
            )
            if errors:
                result += f"\nErrors:\n" + "\n".join(errors)
            ...
            return [
                TextContent(type="text", text=f"Error persisting labels: {e}")
            ]

    async def _get_receipt_labels(
        self, args: Dict[str, Any]
    ) -> List[TextContent]:
        ...
        try:
                    ...
                    labels.append(
                        {
                            ...
                            "reasoning": item.get("reasoning", {}).get(
                                "S", ""
                            ),
                        }
                    )
            ...
            return [
                TextContent(type="text", text=f"Error getting labels: {e}")
            ]
