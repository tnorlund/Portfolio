...
async def test_pattern_detection_on_receipt(
    ...
    if not receipt_data:
        logging.warning(f"No data found for {image_id}/{receipt_id}")
        return {
            'image_id': image_id,
    ...
    receipt, receipt_words, receipt_lines = receipt_data
    ...
    logging.info(f"Testing {image_id}/{receipt_id} - {merchant_name} ({len(receipt_words)} words)")
    ...
    try:
        ...
        logging.error(f"Error testing {image_id}/{receipt_id}: {e}")
        return {
            'image_id': image_id,
            'receipt_id': receipt_id,
            'error': str(e)
...
async def test_all_receipts(
    ...
    logging.info(f"Testing {len(all_receipts)} receipts with {optimization_level.value} optimization")
    ...
    for i, (image_id, receipt_id) in enumerate(all_receipts):
        ...
        if (i + 1) % 5 == 0:
            logging.info(f"Processed {i + 1}/{len(all_receipts)} receipts")
...
async def compare_optimization_levels(data_dir: str, limit: int = None) -> Dict[str, Any]:
    ...
    logging.info(f"Comparing optimization levels on {len(all_receipts)} receipts")
    ...
    for i, (image_id, receipt_id) in enumerate(all_receipts):
        ...
        receipt, receipt_words, _ = receipt_data
        metadata = loader.load_metadata(image_id, receipt_id)
        merchant_name = metadata.merchant_name if metadata else "Unknown"
        ...
        try:
            ...
            logging.error(f"Error comparing {image_id}/{receipt_id}: {e}")
        
        if (i + 1) % 5 == 0:
            logging.info(f"Compared {i + 1}/{len(all_receipts)} receipts")
    
    return {
        'total_receipts': len(comparisons),
...
def main():
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Limit number of receipts to test"
    )
    parser.add_argument(
        "--compare-all",
        action="store_true",
    ...
    try:
        if args.compare_all:
            results = asyncio.run(compare_optimization_levels(
                args.data_dir,
                args.limit
            ))
            ...
            results = asyncio.run(test_all_receipts(
                args.data_dir,
                optimization_level,
                args.limit
            ))
