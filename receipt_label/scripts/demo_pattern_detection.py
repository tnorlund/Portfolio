...
class PatternDetectionDemo:
    ...
    def _create_mock_receipts(self) -> List[List[ReceiptWord]]:
        ...
        generic_words = [
            ...
            self._create_mock_word(
                45, "Fotal", 3, generic_id
            ),  # OCR error: "Total"
            self._create_mock_word(46, "$20.75", 3, generic_id),
            self._create_mock_word(
                47, "Sa1es", 3, generic_id
            ),  # OCR error: "Sales"
    ...
    async def run_comprehensive_demo(self) -> Dict[str, Any]:
        ...
        print("=" * 80)
        print(
            f"Testing {len(self.mock_receipts)} mock receipts with various patterns"
        )
        print()
        ...
        print("-" * 40)
        demo_results["performance_comparison"] = (
        ...
        print("-" * 40)
        demo_results["feature_demonstration"] = (
            await self._demo_enhancement_features()
        )
    ...
    async def _demo_performance_comparison(self) -> Dict[str, Any]:
        ...
        for level in levels:
            ...
            print(
                f"{level.value.upper():>12}: {processing_time:6.2f}ms | {patterns_found:2d} patterns"
            )
        ...
        if "legacy" in results and "advanced" in results:
            ...
            if legacy_time > 0:
                ...
                speedup = (
                    legacy_time / advanced_time if advanced_time > 0 else 1.0
                )
    ...
    async def _demo_enhancement_features(self) -> Dict[str, Any]:
        ...
        for category, matches in pattern_results.items():
            ...
            for match in matches:
                if hasattr(match, "is_multi_word") and match.is_multi_word:
                    multi_word_found.append(
                        f"{category}: '{match.matched_text}'"
                    )
        ...
        mcdonalds_result = await detect_patterns_optimized(
            mcdonalds_words, merchant_name="McDonald's"
        )
        ...
        if merchant_specific:
            ...
            print(
                "  ✓ McDonald's product names: Big Mac, McNuggets, Apple Pie"
            )
                ...
                if hasattr(match, "matched_text"):
                    ...
                    if "fotal" in text or "sa1es" in text:
                        fuzzy_matches.append(
                            f"{category}: '{match.matched_text}' (corrected OCR error)"
                        )
    ...
    async def _demo_pattern_analysis(self) -> Dict[str, Any]:
        ...
        for i, words in enumerate(self.mock_receipts):
            ...
            merchant_name = (
                receipt_name if receipt_name in ["McDonald's"] else None
            )

            result = await detect_patterns_optimized(
                words, merchant_name=merchant_name
            )
                ...
                if matches:
                    ...
                    for match in matches[:3]:  # Show first 3 matches
                        ...
                        matched_text = getattr(
                            match, "matched_text", str(match)
                        )

                        print(
                            f"    - '{matched_text}' (confidence: {confidence:.2f})"
                        )
            ...
            pattern_analysis["total_patterns_by_receipt"].append(
                {"receipt": receipt_name, "total_patterns": receipt_patterns}
            )
        ...
        pattern_analysis["pattern_type_distribution"] = dict(
        ...
        print(
            f"  Average confidence: {pattern_analysis['confidence_analysis'].get('average', 0):.2f}"
        )
    ...
    def save_demo_results(
        self, results: Dict[str, Any], output_file: str = "demo_results.json"
    ):
...
async def main():
    ...
    try:
        ...
        print(
            f"1. Export data: python scripts/export_receipt_data.py sample --size 10"
        )
        print(
            f"2. Run tests: python scripts/test_pattern_detection.py --max-receipts 10"
        )
