...
class TestProcessPartialResults:
    ...
    def test_process_partial_results_with_output_and_errors(self) -> None:
        ...
        client_manager.openai.files.content.side_effect = (
            file_content_side_effect
        )

        # Process partial results
        successful, failed = process_partial_results(
            "batch_123", client_manager
        )

        # Verify results
        assert len(successful) == 1  # Only one with valid embedding
        assert (
            successful[0]["custom_id"] == "IMAGE#123#RECEIPT#00001#LINE#00001"
        )
        assert successful[0]["embedding"] == [0.1, 0.2, 0.3]

        assert (
            len(failed) == 2
        )  # One from output with None, one from error file
...
class TestStatusHandlers:
    ...
    def test_handle_completed_status(self) -> None:
        ...
        result = handle_completed_status(
            "batch_123", "openai_batch_456", client_manager
        )
    ...
    def test_handle_failed_status(self) -> None:
        ...
        with patch(
            ...
            mock_process_error.return_value = {
                ...
                "error_details": [],
                "sample_errors": [
                    {"custom_id": "test", "error_type": "rate_limit"}
                ],
            }
    ...
    def test_handle_expired_status(self) -> None:
        ...
        with patch(
            ...
            mock_process_partial.return_value = (
                [
                    {"custom_id": "test1", "embedding": [0.1, 0.2]}
                ],  # successful
                ["test2", "test3"],  # failed
            )
    ...
    def test_handle_in_progress_status(self) -> None:
        ...
        result = handle_in_progress_status(
            "batch_123", "openai_batch_456", "in_progress", client_manager
        )

        assert result["action"] == "wait"
        assert result["status"] == "in_progress"
        assert (
            9.9 < result["hours_elapsed"] < 10.1
        )  # Allow for small time diff
    ...
    def test_handle_in_progress_status_near_timeout(self) -> None:
        ...
        with patch(
            "receipt_label.embedding.common.batch_status_handler.logger"
        ) as mock_logger:
            result = handle_in_progress_status(
                "batch_123", "openai_batch_456", "in_progress", client_manager
            )
    ...
    def test_handle_cancelled_status(self) -> None:
        ...
        result = handle_cancelled_status(
            "batch_123", "openai_batch_456", "cancelled", client_manager
        )
        ...
        with patch(
            ...
            with patch(
                ...
                result = handle_batch_status(
                    "batch_123", "openai_batch_456", status, client_manager
                )
