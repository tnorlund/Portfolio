...
def get_full_test_markers() -> Set[str]:
    ...
    return {
        "not unused_in_production"  # Test everything except deprecated
    }
...
def should_skip_test_in_environment(test_markers: Set[str], environment: str = None) -> bool:
...
def get_pytest_args_for_environment(environment: str = None) -> list[str]:
    ...
    environment_configs = {
        ...
        "pre_commit": [
            "-m", "unit and fast and not aws and not openai",
            "--timeout=10",  # Even faster for pre-commit
            "-x",
            "--tb=line"  # Minimal tracebacks
        ],
        "development": [
            "-m", "not unused_in_production",
            "--timeout=60",  # More time for dev debugging
            "--tb=long",  # Detailed tracebacks
            "-v"  # Verbose output
        ],
        "integration": [
            "-m", "integration and not unused_in_production",
            "--timeout=120",  # Integration tests can be slower
            "--tb=short"
        ],
        "staging": [
            "-m", "not unused_in_production and not slow",
            "--timeout=90",
            "--tb=short"
        ],
        "production_validation": [
            "-m", "end_to_end and pattern_detection and cost_optimization",
...
class ExecutionStrategy:
    ...
    def should_run_test(self, test_markers: Set[str]) -> bool:
        ...
        return not should_skip_test_in_environment(test_markers, self.environment)
