...
def test_blank_rows_and_words_are_skipped_with_vectors_zipped_first() -> None:
    """The engine writer refuses empty text; the builder drops blank rows
    and words WITH their supplied vectors (strict zip before the skip),
    so alignment never drifts — the corpus contract from teardown PR #4."""
    requests = build_embedding_write_requests(
        image_id=IMAGE_ID,
        receipt_id=1,
        ...
        word_embeddings=[_vector(0.3), _vector(0.4)],
        include_embedding_input=False,
        missing_row="raise",
    )
    lines = [r for r in requests if r.kind == "line"]
    words = [r for r in requests if r.kind == "word"]
    assert len(lines) == 1 and lines[0].vector == _vector(0.1)
    assert len(words) == 1 and words[0].vector == _vector(0.3)


def test_vector_count_mismatch_raises_strict() -> None:
...
def test_section_type_blanks_when_row_spans_sections() -> None:
    ...
    # INVALID-only words stay in the validated population (#1513 class).
