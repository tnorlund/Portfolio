...
- captured: 2026-08-31 from the dev vector store that preceded the DynamoDB indexes, 7m43s
...
- capture cmd: scripts/similarity_harness/capture_golden.py --extra-receipts <extras15> --canonical
  (the capture script was removed with the vector-store teardown; the set is frozen)
