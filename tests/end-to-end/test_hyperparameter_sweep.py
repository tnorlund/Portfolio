...
def test_run_hyperparameter_sweep(validate_env_vars, cleanup_wandb, log_capture):
    ...
    try:
        ...
        try:
            ...
            assert found_metrics, f"None of the expected metrics {expected_metrics} were found in W&B"
        ...
        found_files = [f for f in expected_files if any(f in os.path.join(output_dir, filename) for filename in file_list)]
