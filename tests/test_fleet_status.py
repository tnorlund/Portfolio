...
def test_legacy_enumerations_union_to_seventeen_merchants() -> None:
...
def test_empty_fleet_renders_zero_active_seventeen_missing_exit_zero(
    ...
    assert "**0 ACTIVE / 0 SEALED pending activation / 17 missing**" in output
...
def test_populated_fleet_renders_row_with_short_hash_and_staleness(
    ...
    assert exit_code == 0
    output = capsys.readouterr().out
    assert "**1 ACTIVE / 0 SEALED pending activation / 16 missing**" in output
...
def test_active_slug_outside_legacy_is_surfaced_not_fatal(
    ...
    assert exit_code == 0
    report = json.loads(capsys.readouterr().out)
    assert report["missing_count"] == 17
...
def test_sealed_without_active_is_listed_pending(
    ...
    output = capsys.readouterr().out
    assert "**0 ACTIVE / 1 SEALED pending activation / 17 missing**" in output
