...
## V7 RE-SCORE (after PR3 content fixes a+b+c+d) — 5 DEMO-READY
- Mean 4.63 -> **5.08** (PR3 +0.45; total 2.71->5.08 = +87%). Floor moved up (worst now 3.0-3.5). 5/20 demo-ready (from 0): amazon replace_field, costco replace_field, gelsons add, vons add, vons compose_store_header. Best 6.5: vons compose-header, costco/amazon replace_field.
- Tells: content 16/20 (still dominant), layout 2, graphics 2. PR3 committed: (a) multi-price gutter+truncation, (b) currency 3-dec->2, (c) EMV canonical (PIN/AID/Seq#), (d) in-body barcode + adaptive QR. Render cache warm = full matrix re-render 16s.
- REGRESSION introduced by PR3a: literal '…' ellipsis truncation clips LABELS too (SUBTOT…) — flagged as a new tell. Fix in PR4 (ASCII '...', only long product names, width-fit, never labels).
- PR4 SCOPE (text-pipeline, ranked): T1 (1) clean-string serializer kill OCR corruption (Ontotless/Hendersan/n0W/glued dates) ~6+ renders; (2) eliminate ellipsis/right-edge clipping, width-fit ~9; (3) arithmetic+field-binding validator (no blank/unreconciled totals) ~8. T2 op-fixes: remove_line_item sub-line deletion+rollup, replace_field value-scoping. T3 defer: barcode-over-text (2 smiths, graphics), paper texture (caps 0). Projected: T1 alone ~5.08->~6.5, demo-ready 5->~10.
