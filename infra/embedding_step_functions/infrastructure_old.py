...
class EmbeddingInfrastructure(ComponentResource):
        ...
        for name, lambda_config in zip_configs.items():
            ...
            lambda_func = Function(
                ...
                opts=ResourceOptions(
                    parent=self, ignore_changes=["layers"]
                ),
            )
    ...
    def _create_container_lambda_functions(self):
        ...
        for name, lambda_config in container_configs.items():
            env_vars = {
                "HANDLER_TYPE": lambda_config["handler_type"],
                "DYNAMODB_TABLE_NAME": dynamodb_table.name,
            ...
            if lambda_config["handler_type"] == "compaction":
                env_vars.update(
                    {
                        "CHUNK_SIZE": "10",
            ...
            lambda_func = Function(
                ...
                ephemeral_storage=(
                    FunctionEphemeralStorageArgs(
                        size=lambda_config.get("ephemeral_storage", 512)
                    )
                    if lambda_config.get("ephemeral_storage", 512) > 512
                    else None
                ),
                tags={
                    "environment": stack
