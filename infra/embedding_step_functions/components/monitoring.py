...
class MonitoringComponent(ComponentResource):
    ...
    def _create_lambda_alarms(self):
        ...
        for name, lambda_func in self.lambda_functions.items():
            ...
            timeout_alarm = MetricAlarm(
                ...
                threshold=lambda_func.timeout.apply(
                    lambda t: t * 1000 * 0.8
                ),  # 80% of timeout in ms
    ...
    def _build_dashboard_with_resolved_values(self, resolved):
        ...
        step_function_widgets = self._create_step_function_widgets_resolved(
            step_func_arns
        )
    ...
    def _create_lambda_widgets_resolved(self, lambda_names) -> list:
        ...
        log_sources = " | ".join(
            [f"SOURCE '/aws/lambda/{name}'" for name in lambda_names]
        )
        memory_widget = {
            "type": "log",
            "x": 0,
        ...
        # Lambda memory utilization over time
        memory_timeline_widget = {
            "type": "log",
            "x": 12,
            "y": 12,
            "width": 12,
        ...
        widgets.append(memory_timeline_widget)
