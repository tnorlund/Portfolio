...
def process_chunk_handler(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        ...
        chunk_index, len(chunk_deltas), batch_id
    )
    
    try:
        # Process chunk deltas
        chunk_result = process_chunk_deltas(batch_id, chunk_index, chunk_deltas)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for delta_key in delta_keys:
            ...
            if delta_data["ids"]:
                ...
                total_embeddings_processed += embeddings_count
                logger.info(
                    "Added %d embeddings from delta to chunk %d (total in chunk: %d)",
                    embeddings_count, chunk_index, total_embeddings_processed
                )
        ...
        logger.info(
            ...
            chunk_index, bucket_name, intermediate_key
        )
    
    elapsed_time = time.time() - start_time
    logger.info(
        "Chunk %d processing completed in %.2f seconds with %d embeddings",
        chunk_index, elapsed_time, total_embeddings_processed
    )
...
def merge_intermediate_chunks(batch_id: str, total_chunks: int) -> Dict[str, Any]:
    """
    ...
    elapsed_time = time.time() - start_time
    logger.info(
        "Final merge completed in %.2f seconds. Merged %d chunks with %d total embeddings",
        elapsed_time, total_chunks, total_embeddings_processed
    )
