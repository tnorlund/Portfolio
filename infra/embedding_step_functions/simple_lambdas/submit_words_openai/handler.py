...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        local_path = download_serialized_words(
            {
                "s3_bucket": s3_bucket,
                "s3_key": s3_key,
                # Include the original ndjson path so helper can write
        ...
        logger.info("Deserialized %d words", len(deserialized_words))
        ...
        logger.info("Formatted %d words with context", len(formatted_words))
        ...
        logger.info(
            "Updated embedding status for %d words", len(deserialized_words)
        )
