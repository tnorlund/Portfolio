"""Lambda handler for normalizing poll batches data - S3-only approach.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if (
        ...
        logger.info(
            "poll_results contains S3 references, downloading and combining: batch_id=%s, result_count=%d",
            batch_id,
            len(poll_results),
        )
        ...
        for result_ref in poll_results:
            ...
            if not result_bucket or not result_key:
                logger.warning("Skipping invalid S3 reference: %s", result_ref)
                continue
            ...
            try:
                ...
                logger.error(
                    "Failed to download individual result from S3: result_bucket=%s, result_key=%s, error=%s",
                    result_bucket,
                    result_key,
                    str(e),
                )
        ...
        logger.info(
            "Downloaded and combined individual results: batch_id=%s, combined_count=%d",
            batch_id,
        ...
        try:
            ...
            logger.info(
                "Uploaded combined poll_results to S3: s3_key=%s, bucket=%s, poll_results_count=%d",
                poll_results_s3_key,
                bucket,
                len(combined_results),
            )
    ...
    logger.info(
        "poll_results is in legacy format (full results), uploading to S3: batch_id=%s, poll_results_count=%d",
        batch_id,
    ...
    try:
        ...
        logger.info(
            "Uploaded poll_results to S3: s3_key=%s, bucket=%s, poll_results_count=%d",
            poll_results_s3_key,
            bucket,
            len(poll_results) if isinstance(poll_results, list) else 0,
        )
