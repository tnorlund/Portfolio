        ...
        for collection_name, collection_deltas in deltas_by_collection.items():
            ...
            try:
                ...
                collection = chroma_client.create_collection(
                    collection_name,
                    metadata={"chunk_index": chunk_index, "batch_id": batch_id}
                )
        ...
        for chunk_index in range(total_chunks):
            ...
            try:
                ...
                for collection_meta in chunk_client.list_collections():
                    chunk_collection = chunk_client.get_collection(collection_meta.name)
                    
                    # Get or create collection in main snapshot
                    try:
                        main_collection = chroma_client.get_collection(collection_meta.name)
                    except Exception:
...
def cleanup_intermediate_chunks(batch_id: str, total_chunks: int):
    ...
    for chunk_index in range(total_chunks):
        ...
        try:
            ...
            logger.warning("Failed to delete chunk %d: %s", chunk_index, str(e))
