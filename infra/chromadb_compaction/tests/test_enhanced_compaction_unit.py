...
class TestCoreCompactionLogic:
    ...
    def test_update_receipt_metadata_logic(self):
        ...
        changes = {
            "canonical_merchant_name": {"old": "Old Store", "new": "New Store"},
    ...
    def test_sqs_message_categorization(self):
        ...
        messages = [
            {"body": '{"entity_type": "RECEIPT_METADATA"}', "messageAttributes": {"source": {"stringValue": "dynamodb_stream"}}},
            {"body": '{"entity_type": "RECEIPT_WORD_LABEL"}', "messageAttributes": {"source": {"stringValue": "dynamodb_stream"}}},
            {"body": '{"delta_file": "some_delta.json"}', "messageAttributes": {"source": {"stringValue": "delta_processor"}}},
    ...
    def test_metadata_field_filtering(self):
        ...
        mock_changes = {
            ...
            "created_at": {"old": "2023-01-01", "new": "2023-01-02"},  # Should be ignored
            "address": {"old": "123 Main St", "new": "456 Oak St"},
        }
        
        # Filter to only relevant fields (simulating the actual filtering logic)
        filtered_changes = {
            field: change for field, change in mock_changes.items() 
