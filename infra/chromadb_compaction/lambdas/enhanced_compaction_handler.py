...
def _get_entity_key(msg: StreamMessage) -> tuple[str, ...]:
    ...
    - For RECEIPT: (image_id, receipt_id)
    - For RECEIPT_LINE: (image_id, receipt_id, line_id)
    - For RECEIPT_WORD: (image_id, receipt_id, line_id, word_id)
    - For others: (entity_type, str(entity_data))
    """
    entity_data = msg.entity_data
    entity_type = msg.entity_type

    if entity_type == "RECEIPT" or entity_type == "RECEIPT_PLACE":
        return (
...
def sort_and_deduplicate_messages(
    ...
    """
    # Sort: REMOVE first, then INSERT/MODIFY
