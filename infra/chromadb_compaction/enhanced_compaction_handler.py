...
def process_stream_messages(stream_messages: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def process_metadata_updates(metadata_updates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def process_label_updates(label_updates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    bucket = os.environ["CHROMADB_BUCKET"]
    snapshot_key = "words/snapshot/latest/"  # Labels only affect word embeddings
...
def update_receipt_metadata(
    ...
    for i, record_id in enumerate(all_results["ids"]):
        if record_id.startswith(id_prefix):
            ...
            updated_metadata["last_metadata_update"] = datetime.now(timezone.utc).isoformat()
...
def remove_receipt_metadata(collection: Any, image_id: str, receipt_id: int) -> int:
...
def remove_word_labels(collection: Any, chromadb_id: str) -> int:
    ...
    try:
        ...
        label_fields = [key for key in updated_metadata.keys() if key.startswith("label_")]
...
def process_delta_messages(delta_messages: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def traditional_compaction_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
