...
from dynamo.data._corner_process import (
    normalize,
...
def test_normalize():
    ...
    assert result2 == (0.0, 0.0), f"Expected (0,0) for near-zero vector, got {result2}"
...
def test_offset_corner_inward():
    ...
    and verifying the result is inward by 'offset_distance' along sum of directions.
    """
    # Suppose we have a corner at (10,10), and two adjacent corners at (0,10) and (10,0).
    corner = (10.0, 10.0)
    adj1 = (0.0, 10.0)
    adj2 = (10.0, 0.0)
    offset_distance = 5.0
...
def test_window_rectangle_for_corner():
    ...
    We'll do TWO sub-scenarios:
      ...
      2) corner_position="bottom" (to ensure coverage of the bottom-flip branch).
    """
    ...
    # so that the code triggers: "elif corner_position == 'bottom' and r[1] < 0: r = (-r[0], -r[1])".
...
def test_crop_polygon_region():
    ...
    # Check a pixel inside => (0,0) in cropped => (2,2) in original => color=blue
...
def test_extract_and_save_corner_windows():
    ...
    # 2) Suppose the receipt corners are basically near each corner of the image:
    top_left = (0.0, 0.0)
    top_right = (199.0, 0.0)
    ...
    # 4) Check structure => keys = top_left, top_right, bottom_right, bottom_left
    for k in ["top_left", "top_right", "bottom_right", "bottom_left"]:
        assert k in windows, f"Missing corner key {k}"
        corner_data = windows[k]
        # corner_data => { 'image': PIL.Image, 'width': int, 'height': int, 'inner_corner': (x,y) }
    ...
    # Optionally, you can also check if windows['top_left']['image'] is not empty, etc.
