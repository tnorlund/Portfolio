...
from dynamo.data._ocr import apple_vision_ocr
...
from dynamo.data._corner_process import extract_and_save_corner_windows
from dynamo.data._cluster import dbscan_lines
...
def process_picture(
    ...
    for image_path in image_paths:
        ...
        for i, receipt_image in enumerate(receipt_images, start=1):
            ...
            receipt_box = next(
                (box for box in receipt_boxes if box["receipt_id"] == i), None
            )
            ...
            top_left = {
                "x": receipt_box["receipt_box_corners"][0][0],
                "y": receipt_box["receipt_box_corners"][0][1],
            }
            top_right = {
                "x": receipt_box["receipt_box_corners"][1][0],
                "y": receipt_box["receipt_box_corners"][1][1],
            }
            bottom_right = {
                "x": receipt_box["receipt_box_corners"][2][0],
                "y": receipt_box["receipt_box_corners"][2][1],
            }
            bottom_left = {
                "x": receipt_box["receipt_box_corners"][3][0],
                "y": receipt_box["receipt_box_corners"][3][1],
            }
            dynamo_receipts.append(
                Receipt(
                    receipt_id=i,
                    image_id=image_id,
                    width=image.width,
...
def _get_ocr_and_windows(image_path: Path) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for cluster_id, cluster in clusters.items():
        ...
        for w in cluster_words:
            corners_image_coordinates = w.calculate_corners(
                width=image.width,
                height=image.height,
        ...
        receipt_box_corners = compute_receipt_box_from_skewed_extents(
            hull, cx, cy, -avg_angle_degrees
        )
        receipt_boxes.append(
            {
                "image_id": image_id,
        ...
        a_i, b_i, c_i, d_i, e_i, f_i, g_i, h_i = find_perspective_coeffs(
        ...
        refined_lines, refined_words, refined_letters = list(
            apple_vision_ocr([warp_image_path]).values()
        )[0]
