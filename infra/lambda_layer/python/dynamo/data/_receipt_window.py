...
class _ReceiptWindow:
    ...
    def addReceiptWindows(self, receipt_windows: List[ReceiptWindow]):
        ...
        if receipt_windows is None:
            raise ValueError(
                "receipt_windows parameter is required and cannot be None."
            )
        ...
        if not all(isinstance(rw, ReceiptWindow) for rw in receipt_windows):
            raise ValueError(
                "All items in the receipt_windows list must be instances of the ReceiptWindow class."
            )
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                raise Exception(
                    f"Could not add receipt windows to DynamoDB: {e}"
                ) from e
    ...
    def deleteReceiptWindows(self, receipt_windows: List[ReceiptWindow]) -> None:
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                raise Exception(
                    f"Could not delete receipt windows from DynamoDB: {e}"
                ) from e
            if error_code == "ProvisionedThroughputExceededException":
                raise Exception(f"Provisioned throughput exceeded: {e}") from e
            elif error_code == "ValidationException":
    ...
    def listReceiptWindows(
        self, limit: int = None, lastEvaluatedKey: dict = None
    ) -> tuple[list[ReceiptWindow], dict | None]:
        """
        ...
        try:
            ...
            if error_code == "ResourceNotFoundException":
                raise Exception(
                    f"Could not list receipt windows from DynamoDB: {e}"
                ) from e
            elif error_code == "ProvisionedThroughputExceededException":
                raise Exception(f"Provisioned throughput exceeded: {e}") from e
            elif error_code == "ValidationException":
