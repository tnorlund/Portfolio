...
class MetadataFinderProcessor:
    ...
    async def find_missing_metadata(
        ...
        try:
            ...
            if existing_metadata is None:
                try:
                    existing_metadata = self.dynamo.get_receipt_metadata(image_id, receipt_id)
                except Exception:
                    # Metadata doesn't exist yet, skip finder (should be created by initial workflow)
                    _log(f"ReceiptMetadata doesn't exist yet for {image_id}#{receipt_id}, skipping finder")
                    return {
                        "success": False,
            ...
            has_all_fields = (
                existing_metadata.place_id and
                existing_metadata.merchant_name and
                existing_metadata.address and
                existing_metadata.phone_number
            )

            if has_all_fields:
                _log(f"All metadata fields already present for {image_id}#{receipt_id}, skipping finder")
                return {
                    "success": True,
            ...
            if self.google_places_api_key:
                try:
                    places_client = PlacesClient(api_key=self.google_places_api_key)
            ...
            _log(f"Running metadata finder agent for {image_id}#{receipt_id}...")
            ...
            if not agent_result or not agent_result.get("found"):
                _log(f"Agent did not find metadata for {image_id}#{receipt_id}")
                return {
                    "success": False,
                    "reason": "agent_no_results",
                    "fields_found": [],
                    "reasoning": agent_result.get("reasoning") if agent_result else None,
                }
            ...
            updated_metadata = self.dynamo.get_receipt_metadata(image_id, receipt_id)
            ...
            if "merchant_name" in fields_found and agent_result.get("merchant_name"):
                if not updated_metadata.merchant_name:
                    updated_metadata.merchant_name = agent_result["merchant_name"]
            ...
            if "phone_number" in fields_found and agent_result.get("phone_number"):
                if not updated_metadata.phone_number:
                    updated_metadata.phone_number = agent_result["phone_number"]
    ...
    async def _setup_chromadb(self) -> tuple[Optional[Any], Optional[Any]]:
        ...
        try:
            ...
            if storage_mode == "efs" or (storage_mode == "auto" and efs_root and efs_root != "/tmp/chroma"):
                try:
            ...
            if not chroma_client and self.chroma_http_endpoint:
                try:
                    ...
                    _log(f"Using HTTP ChromaDB endpoint: {self.chroma_http_endpoint}")
                except Exception as e:
