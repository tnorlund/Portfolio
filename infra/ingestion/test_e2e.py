...
from .utils import backup_raw_s3, backup_cdn_s3, delete_raw_s3, delete_cdn_s3, restore_s3
...
def setup_and_cleanup(pulumi_outputs):
    ...
    # ----------------------------------------------------------------------
    # In real code, you'd iterate over objects in S3 and store them locally or in memory
    # This might be large, so consider carefully how you handle backups.
    # Similarly for DynamoDB items.
    
    raw_keys = backup_raw_s3(raw_bucket)
    cdn_keys = backup_cdn_s3(cdn_bucket)
    # dynamo_backup = backup_dynamo_items(dynamo_client, dynamo_table)
    
    # ----------------------------------------------------------------------
    # 2. DELETE them from the test environment
    # ----------------------------------------------------------------------
    delete_raw_s3(raw_bucket)
    delete_cdn_s3(cdn_bucket)
    # delete_dynamo_items(dynamo_client, dynamo_table)
    ...
    # restore_dynamo_items(dynamo_client, dynamo_table, dynamo_backup)
