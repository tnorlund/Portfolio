        ...
        *,
                        ...
                        "Statement": [
                            {
                                ...
                                "Resource": [
                                    ...
                                    [args[4], f"{args[4]}/*"] if len(args) > 4 else []  # artifacts bucket if provided
                                ),
                            }
                        ],
                    }
                            ...
                            "Statement": [
                                {
                                    ...
                                    "Resource": args[0],  # Specific queue ARN, no wildcards
                                }
                            ],
                        }
                    )
        ...
        combine_lambda_config = {
            ...
            "environment": {
                ...
                "ARTIFACTS_BUCKET": artifacts_bucket_name or "",  # Required for NDJSON export - must be provided
                "EMBED_NDJSON_QUEUE_URL": embed_ndjson_queue_url or "",  # Empty string if not provided
                "DYNAMODB_TABLE_NAME": dynamodb_table_name,
                ...
                "LANGCHAIN_PROJECT": pulumi.Config("portfolio").get("langchain_project") or "combine-receipts",
            },
        }
            ...
            "States": {
                "Initialize": {
                    ...
                    "Parameters": {
                        ...
                        "llm_analysis_s3_key.$": "$.llm_analysis_s3_key",  # Optional: S3 key to LLM analysis JSON
                        "limit.$": "$.limit",  # Optional: limit number of images
                    },
                    "ResultPath": "$.init",
                    "Next": "ListImages",
                },
