"""List unique merchants with trace propagation.

This handler starts the root trace for the Step Function workflow.
All subsequent handlers will resume this trace as children.
"""
...
from tracing import flush_langsmith_traces, start_trace
...
def handler(event: dict[str, Any], _context: Any) -> "ListMerchantsOutput":
    ...
    This handler starts the LangSmith trace for the workflow.
    ...
    Output:
    {
        "merchants": [...],
        "total_merchants": 18,
        "langsmith_headers": {...}  # For trace propagation
    }
    """
    ...
    # Start the root trace for this workflow
    with start_trace(
        "label_evaluator_workflow",
        event,
        metadata={
        ...
        tags=["label-evaluator", "workflow-root"],
    ) as trace_ctx:
