...
from pulumi_docker import DockerBuildArgs, Image, RegistryArgs, CacheFromArgs
...
class BaseImages(ComponentResource):
    ...
    def get_image_tag(self, package_name: str, package_dir: Path) -> str:
        ...
        if use_static is None:
            use_static = os.environ.get("USE_STATIC_BASE_IMAGE", "").lower() in ("true", "1", "yes")
        
        if use_static:
            # Use a stable tag for local development
            pulumi.log.info(f"Using stable tag for {package_name} (static mode enabled)")
            return "stable"
        
        # Use content-based hash for production
        hash_tag = self.get_content_hash(package_dir)
        pulumi.log.info(f"Using content-based tag for {package_name}: {hash_tag}")
        return hash_tag
    ...
    def __init__(
        ...
        self.dynamo_base_image = Image(
            ...
            build=DockerBuildArgs(
                ...
                # The issue: Pulumi's type serialization expects a list but gets CacheFromArgs
                # Workaround: Use manual cache warming with ./pull_base_images.sh
            ),
