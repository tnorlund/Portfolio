...
sys.path.insert(
    0,
    "/Users/tnorlund/portfolio_agent_evaluator_visualization/Portfolio/receipt_dynamo",
)
...
def main():
    ...
    print("\nInitializing Spark...")
    spark = (
        SparkSession.builder.appName("CacheGenerator")
        .master("local[*]")
        ...
        for label_str in labels_raw:
            if isinstance(label_str, str):
                ...
                if "line_id" in parsed and "word_id" in parsed:
                    labels_lookup[(parsed["line_id"], parsed["word_id"])] = (
                        parsed.get("label")
                    )
    ...
    for r in selected[:5]:
        print(
            f"  {r['merchant_name']}: {r['issues_found']} issues, {len(r['words'])} words"
        )
