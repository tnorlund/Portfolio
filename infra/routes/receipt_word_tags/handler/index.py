...
def handler(event, context):
    ...
    production_origins = ["https://tylernorlund.com", "https://dev.tylernorlund.com"]
    ...
    if http_method == "GET":
        try:
            ...
            if "tag" in query_params:
                ...
                return {
                    "statusCode": 400,
                    "body": "Missing required query parameter 'tag'",
                }
        ...
        try:
            ...
            if "selected_tag" not in body or "selected_words" not in body:
                return {
                    ...
                    "body": json.dumps(
                        {
                            "error": "Request must include 'selected_tag' and 'selected_words'"
                        }
                    ),
                }
            ...
            for word_data in selected_words:
                if "word" not in word_data or "tags" not in word_data:
                    return {
                        ...
                        "headers": cors_headers,
                        "body": json.dumps(
                            {"error": "Each item must contain 'word' and 'tags'"}
                        ),
                    }
                ...
                word_params = {
                    k: v for k, v in word_data["word"].items() if k != "receipt_id"
                }
                ...
                receipt_word_tag = ReceiptWordTag(
                    ...
                    tag=selected_tag,
                    timestamp_added=(
                        old_receipt_word_tag.timestamp_added
                        if matching_tag
                        else timestamp_str
                    ),
                    validated=old_receipt_word_tag.validated if matching_tag else None,
                    timestamp_validated=(
                        old_receipt_word_tag.timestamp_validated
                        if matching_tag
                        else None
                    ),
                    gpt_confidence=(
                        old_receipt_word_tag.gpt_confidence if matching_tag else None
                    ),
                    flag=old_receipt_word_tag.flag if matching_tag else None,
                    revised_tag=(
                        old_receipt_word_tag.revised_tag if matching_tag else None
                    ),
                ...
                word_tag = WordTag(
                    ...
                    tag=selected_tag,
                    timestamp_added=(
                        old_word_tag.timestamp_added if matching_tag else timestamp_str
                    ),
                    validated=old_word_tag.validated if matching_tag else None,
                    timestamp_validated=(
                        old_word_tag.timestamp_validated if matching_tag else None
                    ),
                    gpt_confidence=(
                        old_word_tag.gpt_confidence if matching_tag else None
                    ),
