...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        start_date = query_params.get("start_date", (datetime.utcnow() - timedelta(days=30)).strftime("%Y-%m-%d"))
...
def aggregate_by_service(metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for service in by_service:
        ...
        by_service[service]["average_cost_per_call"] = round(
            by_service[service]["cost_usd"] / by_service[service]["api_calls"], 6
        ) if by_service[service]["api_calls"] > 0 else 0
...
def aggregate_by_model(metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for model in by_model:
        ...
        by_model[model]["average_tokens_per_call"] = round(
            by_model[model]["tokens"] / by_model[model]["api_calls"], 2
        ) if by_model[model]["api_calls"] > 0 else 0
...
def aggregate_by_operation(metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for operation in by_operation:
        ...
        by_operation[operation]["cost_usd"] = round(by_operation[operation]["cost_usd"], 4)
