"""Pulumi infrastructure for validating PENDING ReceiptWordLabel entities."""
...
class ValidatePendingLabelsStepFunction(ComponentResource):
    ...
    - 2 Lambda Functions:
        * list_pending_labels: Queries DynamoDB for PENDING labels, creates manifest
        * validate_receipt: Validates PENDING labels for a single receipt (container-based)
    ...
    - DynamoDB access for label storage
    """
    ...
    def __init__(
        ...
        *,
        ...
        manifest_bucket = aws.s3.Bucket(
            ...
            force_destroy=True,
            tags={"environment": stack, "purpose": "pending-labels-manifest"},
        ...
        # Create zip-based list_pending_labels Lambda
        list_pending_labels_lambda = Function(
            f"{name}-list-pending-labels",
            name=f"{name}-list-pending-labels",
            role=lambda_exec_role.arn,
            runtime="python3.12",
            architectures=["arm64"],
            handler="list_pending_labels.handler",
            code=AssetArchive(
                {
                    "list_pending_labels.py": FileAsset(
                        os.path.join(
                            os.path.dirname(__file__),
                            "handlers",
                            "list_pending_labels.py",
                        )
                    ),
        ...
        # Create container-based validate_receipt Lambda
        ...
        # Bucket name will be: {name}-validate-receipt-img-artifacts (55 chars) + 7 random = 62 chars
        validate_receipt_docker_image = CodeBuildDockerImage(
            f"{name}-val-receipt-img",  # Further shortened to keep bucket name under 63 chars
        ...
        # Note: Lambda functions automatically create CloudWatch Log Groups when they first run
        # We don't create them explicitly here (unlike some other routes) to avoid conflicts
        # The log groups will be created automatically with the pattern: /aws/lambda/{function-name}
        # Retention policies can be set later via AWS Console or separate Pulumi resources if needed
        ...
        RolePolicy(
            ...
            role=sfn_role.id,
            policy=Output.all(
                list_pending_labels_lambda.arn, validate_receipt_lambda.arn
            ).apply(
        ...
        self.state_machine = StateMachine(
            ...
            # Express has 5-minute limit, but we process 393 receipts taking 60-100+ seconds each
            tags={"environment": stack},
        ...
        self.list_pending_labels_lambda_arn = list_pending_labels_lambda.arn
        ...
        self.register_outputs(
            {
                ...
                "list_pending_labels_lambda_arn": list_pending_labels_lambda.arn,
    ...
    def _create_step_function_definition(self, arns: list) -> str:
        ...
        definition = {
            "Comment": "Validate ReceiptWordLabel entities (PENDING or INVALID status)",
            ...
            "States": {
                ...
                "NoReceipts": {
                    "Type": "Pass",
                    "Result": "No receipts with labels to validate",
                    "End": True,
                },
            },
        }
