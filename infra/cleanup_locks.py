    ...
    for i, lock in enumerate(locks, 1):
        try:
            ...
            if i % 50 == 0:  # Progress update every 50 deletions
                print(f"  Progress: {i}/{len(locks)} ({success_count} successful, {error_count} errors)")
        except Exception as e:
            error_count += 1
            if "ConditionalCheckFailedException" not in str(e):  # Don't log ownership errors
...
if __name__ == "__main__":
    ...
    response = input(f"Delete all {len(locks)} locks individually? This will take some time. (y/N): ")
    if response.lower() == 'y':
