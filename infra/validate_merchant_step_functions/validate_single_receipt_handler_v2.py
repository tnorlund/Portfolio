...
def validate_handler(event, context):
    ...
    # Validate merchant
    metadata, status_info = handler.validate_receipt_merchant(
        image_id=image_id,
    ...
    logger.info(f"Got metadata for {image_id} {receipt_id}: {dict(metadata)}")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: python validate_single_receipt_handler_v2.py <image_id> <receipt_id>")
        sys.exit(1)
