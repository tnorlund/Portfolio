...
class LayerVersionUpdater:
    ...
    def __init__(self, config_path: str, stack_name: str = "dev"):
        ...
        try:
            ...
            self.aws_account_id = (
                cfg_account or boto3.client("sts").get_caller_identity()["Account"]
            )
    ...
    def update_layer_versions_in_state(
        ...
        for resource in lambda_resources:
            ...
            for layer_arn in layers:
                if not isinstance(layer_arn, str) or not self._validate_layer_arn(
                    layer_arn
                ):
    ...
    def run(self, dry_run: bool = True, sync_aws: bool = False) -> None:
        ...
        try:
            ...
            if not dry_run:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                updated_file = (
                    self.backup_dir / f"{self.stack_name}_updated_{timestamp}.json"
                )
