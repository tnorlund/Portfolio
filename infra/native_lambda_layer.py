    ...
    def _calculate_package_hash(self) -> str:
        ...
        for root, dirs, files in os.walk(self.package_path):
            ...
            for file in files:
                ...
                if file_path.suffix in ['.pyc', '.pyo'] or file_path.name in ['.DS_Store']:
                    continue
                    
                try:
                    with open(file_path, 'rb') as f:
                        # Hash file path and content
                        hash_md5.update(str(file_path.relative_to(self.package_path)).encode())
...
class NativeLambdaLayerWithCodeBuild(NativeLambdaLayer):
    ...
    def _create_layer_with_codebuild(self):
        ...
        build_trigger = self._create_build_trigger(codebuild_project, source_object)
        ...
        self.layer_version = aws.lambda_.LayerVersion(
            ...
            code=aws.lambda_.LayerVersionCodeArgs(
                s3_bucket=source_bucket.bucket,
    ...
    def _upload_source_to_s3(self, bucket: aws.s3.Bucket) -> aws.s3.BucketObject:
    ...
    def _create_codebuild_project(self, bucket: aws.s3.Bucket) -> aws.codebuild.Project:
        ...
        return aws.codebuild.Project(
            ...
            source=aws.codebuild.ProjectSourceArgs(
                type="S3",
                location=bucket.bucket.apply(lambda b: f"{b}/{self.name}/source.zip")
            ),
            # Build specification embedded directly (no external files)
            buildspec="version: 0.2\nphases:\n  build:\n    commands:\n      - pip install -r requirements.txt -t python/\n      - zip -r layer.zip python/"
        )
    
    def _create_build_trigger(self, project: aws.codebuild.Project, 
                            source_object: aws.s3.BucketObject) -> pulumi.CustomResource:
