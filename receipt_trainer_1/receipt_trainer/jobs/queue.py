...
class JobQueue:
    ...
    def submit_job(self, job: Job) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"Job {job.job_id} submitted successfully with SQS message ID: {response.get('MessageId')}"
            )
            return job.job_id
        ...
        for i in range(0, len(jobs), 10):
                ...
                entries.append(
                    {
                        "Id": str(
                            j
                        ),  # A unique ID for the batch operation, not the job ID
                        "MessageBody": job.to_json(),
                        "MessageAttributes": job.get_sqs_message_attributes(),
                        "MessageGroupId": job.get_sqs_message_group_id(),
                        "MessageDeduplicationId": job.get_sqs_deduplication_id(),
            ...
            try:
                ...
                if "Failed" in response and response["Failed"]:
                    ...
                    self.logger.error(
                        f"Failed to submit {len(failed_messages)} jobs in batch: {failed_messages}"
                    )
    ...
    def retry_job(self, job: Job) -> Optional[str]:
        ...
        if not job.can_retry():
            self.logger.warning(
                f"Job {job.job_id} cannot be retried (attempts: {job.attempt_count}/{job.max_attempts})"
            )
            return None
        ...
        try:
            ...
            self.logger.info(
                f"Job {job.job_id} scheduled for retry (attempt {job.attempt_count}) with delay of {delay_seconds} seconds"
            )
            return job.job_id
    ...
    def process_jobs(
        self, handler: Callable[[Job], bool], interval_seconds: float = 5.0
    ) -> None:
        """
                    ...
                    try:
                        ...
                        if job_service:
                            ...
                            if not dependencies_satisfied:
                                self.logger.info(
                                    f"Job {job.job_id} has unsatisfied dependencies. Returning to queue with delay."
                                )
                                ...
                                self.logger.debug(
                                    f"Unsatisfied dependencies for job {job.job_id}: {unsatisfied}"
                                )
                                continue
                        ...
                        if self.config.visibility_timeout_seconds > 0:
                            ...
                            heartbeat_interval = max(
                                30, self.config.visibility_timeout_seconds // 2
                            )
                        ...
                        if success:
                            ...
                            if job.retry_count < self.config.max_retries:
                                self.logger.info(
                                    f"Retrying job {job.job_id} ({job.retry_count + 1}/{self.config.max_retries})"
                                )
                                self.delete_job(receipt_handle)
                                job.retry_count += 1
                                self.retry_job(job)
                                ...
                                self.logger.warning(
                                    f"Job {job.job_id} exceeded max retries, sending to DLQ"
                                )
                                self.delete_job(receipt_handle)
                        ...
                        self.logger.error(
                            f"Error processing job {job.job_id}: {str(e)}"
                        )
    ...
    def _get_job_service(self):
        ...
        try:
            ...
            self.logger.warning(
                "Could not import JobService, dependency checking disabled"
            )
            return None
        except Exception as e:
    ...
    def start_processing(
        ...
        thread = threading.Thread(
            target=self.process_jobs, args=(handler, interval_seconds), daemon=True
        )
    ...
    def get_queue_attributes(self) -> Dict[str, Any]:
        ...
        try:
            response = self.sqs.get_queue_attributes(
                QueueUrl=self.config.queue_url, AttributeNames=["All"]
            )
