...
class Job:
    ...
    config: Dict[str, Any]  # Configuration for the job (includes model config, training params, etc.)
    ...
    dependencies: List[str] = dataclasses.field(default_factory=list)  # List of job IDs this job depends on
    timeout_seconds: Optional[int] = None  # Maximum runtime in seconds before job is considered failed
    ...
    def get_sqs_message_attributes(self) -> Dict[str, Dict[str, Union[str, Dict[str, str]]]]:
        """
    ...
    def can_retry(self) -> bool:
        ...
        return (self.status in (JobStatus.FAILED, JobStatus.INTERRUPTED) and 
