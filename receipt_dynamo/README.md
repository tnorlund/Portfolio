...
The ReceiptWord entity includes an `is_noise` field to identify words that don't need labeling:

### What are Noise Words?
- Single punctuation characters: `.`, `,`, `:`, `-`
- Separators: `---`, `===`, `***`
- OCR artifacts and scan noise
- Non-alphanumeric strings without semantic value

### Storage Strategy
- **DynamoDB**: Stores ALL words including noise (complete OCR preservation)
- **Pinecone**: Only meaningful words are embedded (cost optimization)
- **Labels**: Noise words never receive labels

### Benefits
- Complete audit trail of OCR output
- Reduced API costs by skipping noise in expensive operations
- Ability to reprocess with improved noise detection
- Analytics on OCR quality and noise levels

See [NOISE_WORD_IMPLEMENTATION.md](NOISE_WORD_IMPLEMENTATION.md) for implementation details.
