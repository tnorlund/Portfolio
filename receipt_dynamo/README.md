...
| **Receipt Validation Analysis** | `IMAGE#<image_id>`         | `RECEIPT#<receipt_id>#ANALYSIS#VALIDATION`                              | `ANALYSIS_TYPE`         | `VALIDATION#<timestamp>`                                                        | `RECEIPT`              | `IMAGE#<image_id>#RECEIPT#<receipt_id>`                                         | `VALIDATION_STATUS#<status>` | `TIMESTAMP#<timestamp>`                     | - `validations: {business_identity: [{type: "error\|warning\|info\|success", message, reasoning, field, expected_value, actual_value}], address_verification: [{type, message, reasoning, field, expected_value, actual_value}], phone_validation: [{type, message, reasoning, field, expected_value, actual_value}], hours_verification: [{type, message, reasoning, field, expected_value, actual_value}], cross_field_consistency: [{type, message, reasoning, field, expected_value, actual_value}], line_item_validation: [{type, message, reasoning, field, expected_value, actual_value}]}` <br>- `overall_valid: bool` <br>- `validation_status: "VALID\|INVALID_BUSINESS\|INVALID_ADDRESS\|INVALID_PHONE\|INVALID_HOURS\|INVALID_TOTALS\|INVALID_LINE_ITEMS\|NEEDS_REVIEW\|INCOMPLETE"` <br>- `prompt_template: str` <br>- `response_template: str` <br>- `timestamp_added: str` <br>- `version: str` <br>- `metadata: {processing_metrics: {processing_time_ms, api_calls}, processing_history: [{event_type, timestamp, description, model_version}], source_information: {model_name, model_version, algorithm, configuration}}` |
