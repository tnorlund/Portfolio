...
def find_methods_needing_decorators(content: str) -> List[Tuple[int, str, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        method_match = re.match(r'^\s{4}def (add|update|delete|get|list)_(\w+)\(', line)
        if method_match:
            # Check if previous line has decorator
            if i > 0 and '@handle_dynamodb_errors' not in lines[i-1]:
                method_name = method_match.group(0).strip()[4:-1]  # Extract method name
                operation_name = method_name
                methods_to_decorate.append((i, method_name, operation_name))
    
    return methods_to_decorate

def add_decorators_to_content(content: str, methods: List[Tuple[int, str, str]]) -> str:
    ...
    for line_num, method_name, operation_name in sorted(methods, reverse=True):
        ...
        decorator = ' ' * indent + f'@handle_dynamodb_errors("{operation_name}")'
        ...
        if not import_added:
            ...
            for i, line in enumerate(lines):
                if line.startswith('from receipt_dynamo.data'):
                    lines.insert(i+1, 'from receipt_dynamo.data.base_operations import handle_dynamodb_errors')
                    break
