...
def get_required_imports(file_content: str) -> List[str]:
    ...
    if "raise ValueError" in file_content:
        ...
        for match in re.finditer(
            r"raise ValueError\((.*?)\)", file_content, re.DOTALL
        ):
    ...
    if "raise RuntimeError" in file_content:
        runtime_errors = re.findall(
            r"raise RuntimeError\((.*?)\)", file_content, re.DOTALL
        )
...
def fix_valueerror(file_content: str) -> Tuple[str, int]:
    ...
    file_content = re.sub(
        pattern_multiline, replace_error_multiline, file_content
    )
...
def process_file(filepath: Path) -> Dict[str, int]:
    ...
    return {"ValueError": valueerror_count, "RuntimeError": runtimeerror_count}
...
def main():
    ...
    for filepath in sorted(files):
        ...
        if stats["ValueError"] > 0 or stats["RuntimeError"] > 0:
            print(
                f"  Fixed: {stats['ValueError']} ValueError, {stats['RuntimeError']} RuntimeError"
            )
    ...
    print(f"\n{'='*50}")
    print(
        f"Total fixes: {total_stats['ValueError']} ValueError, {total_stats['RuntimeError']} RuntimeError"
    )
