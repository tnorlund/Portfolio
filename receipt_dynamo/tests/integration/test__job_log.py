...
def test_addJobLog_success(job_log_dynamo, sample_job_log):
    ...
    retrieved_log = job_log_dynamo.getJobLog(job_id=sample_job_log.job_id, timestamp=sample_job_log.timestamp)
...
def test_addJobLog_raises_value_error_job_not_instance(job_log_dynamo):
    """Test that addJobLog raises ValueError when job_log is not a JobLog instance."""
...
def test_addJobLog_raises_conditional_check_failed(job_log_dynamo, sample_job_log):
    """Test that addJobLog raises ValueError when trying to add a duplicate job log."""
    # Add the job log
    job_log_dynamo.addJobLog(sample_job_log)
...
def test_addJobLogs_success(job_log_dynamo, multiple_job_logs):
    ...
    for log in multiple_job_logs:
        retrieved_log = job_log_dynamo.getJobLog(job_id=log.job_id, timestamp=log.timestamp)
...
def test_getJobLog_raises_value_error_log_not_found(job_log_dynamo):
    ...
    with pytest.raises(ValueError, match="not found"):
        job_log_dynamo.getJobLog(job_id="non-existent-job", timestamp="2021-01-01T12:00:00")
...
def test_listJobLogs_with_limit(job_log_dynamo, multiple_job_logs):
    ...
    next_logs, next_last_key = job_log_dynamo.listJobLogs(job_id=job_id, limit=limit, lastEvaluatedKey=last_key)
...
def test_deleteJobLog_success(job_log_dynamo, sample_job_log):
    ...
    with pytest.raises(ValueError, match="not found"):
        job_log_dynamo.getJobLog(job_id=sample_job_log.job_id, timestamp=sample_job_log.timestamp)
...
def test_deleteJobLog_raises_value_error_log_not_instance(job_log_dynamo):
    """Test that deleteJobLog raises ValueError when job_log is not a JobLog instance."""
    with pytest.raises(ValueError, match="job_log must be a JobLog instance"):
