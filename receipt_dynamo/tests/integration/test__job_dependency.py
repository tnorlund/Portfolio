...
@pytest.mark.integration
def test_addJobDependency_raises_conditional_check_failed(job_dependency_dynamo, sample_job_dependency):
...
@pytest.mark.integration
def test_addJobDependency_raises_resource_not_found(job_dependency_dynamo, sample_job_dependency, mocker):
...
def test_listDependencies_with_limit(job_dependency_dynamo, multiple_job_dependencies):
    ...
    dependent_job_id = multiple_job_dependencies[0].dependent_job_id  # All have the same dependent_job_id
    limit = 2
    dependencies, last_key = job_dependency_dynamo.listDependencies(dependent_job_id=dependent_job_id, limit=limit)
    ...
    if last_key is not None:
        ...
        assert all(dep1.dependency_job_id != dep2.dependency_job_id
            for dep1 in dependencies
            for dep2 in next_dependencies), "Should get different dependencies"
...
def test_listDependents_success(job_dependency_dynamo):
    ...
    for i in range(3):
        dependent_job_id = str(uuid.uuid4())
        dependency = JobDependency(dependent_job_id=dependent_job_id,
            dependency_job_id=dependency_job_id,
            type="SUCCESS",
            created_at=datetime.now().isoformat(),
            condition=f"Test condition {i}", )
...
def test_listDependents_with_limit(job_dependency_dynamo):
    ...
    limit = 2
    dependents, last_key = job_dependency_dynamo.listDependents(dependency_job_id=dependency_job_id, limit=limit)
    ...
    if last_key is not None:
        ...
        assert all(dep1.dependent_job_id != dep2.dependent_job_id
            for dep1 in dependents
            for dep2 in next_dependents), "Should get different dependents"
...
@pytest.mark.integration
def test_deleteJobDependency_raises_conditional_check_failed(job_dependency_dynamo, sample_job_dependency):
