...
def test_batch_add_and_list_gpt_initial_tagging(
    ...
    """
    Tests adding multiple GPTInitialTagging records in batch and then listing
    them.
    """
    taggings = []
    client = DynamoClient(dynamodb_table)
...
def test_listGPTInitialTaggings_empty_table(dynamodb_table):
    """
    Verifies that listGPTInitialTaggings returns an empty list when there are
    no records.
    """
    client = DynamoClient(dynamodb_table)
...
def test_listGPTInitialTaggings_single_record(
    ...
    """
    Tests that after adding a single GPTInitialTagging record,
    listGPTInitialTaggings returns it.
    """
    client = DynamoClient(dynamodb_table)
    client.addGPTInitialTagging(sample_gpt_initial_tagging)
    taggings, _ = client.listGPTInitialTaggings()
...
def test_listGPTInitialTaggings_multiple_records(dynamodb_table):
    """
    Tests that listGPTInitialTaggings returns all GPTInitialTagging records
    when multiple are added.
    """
    client = DynamoClient(dynamodb_table)
