...
def sample_word_tag():
    ...
    """
    return WordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
        line_id=10,
        word_id=5,
        tag="SampleTag",
...
def test_add_word_tag(dynamodb_table: Literal["MyMockedTable"], sample_word_tag: WordTag):
    # Arrange
...
@pytest.mark.integration
def test_add_word_tag_duplicate_raises(dynamodb_table: Literal["MyMockedTable"], sample_word_tag: WordTag):
    # Arrange
    client = DynamoClient(dynamodb_table)
    client.addWordTag(sample_word_tag)
...
@pytest.mark.integration
def test_update_word_tag(dynamodb_table: Literal["MyMockedTable"], sample_word_tag: WordTag):
    # Arrange
    client = DynamoClient(dynamodb_table)
    client.addWordTag(sample_word_tag)
...
@pytest.mark.integration
def test_delete_word_tag(dynamodb_table: Literal["MyMockedTable"], sample_word_tag: WordTag):
    # Arrange
    client = DynamoClient(dynamodb_table)
    client.addWordTag(sample_word_tag)
...
def test_word_tag_list(dynamodb_table: Literal["MyMockedTable"]):
    # Arrange
    client = DynamoClient(dynamodb_table)
    tags = [WordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
            line_id=1,
            word_id=i,
            tag=f"Tag{i}",
            timestamp_added="2021-01-01T00:00:00", )
...
def test_word_tag_list_from_image(dynamodb_table: Literal["MyMockedTable"]):
    ...
    same_image_tags = [WordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
            line_id=10,
            word_id=i,
            tag=f"ImageTag{i}",
            timestamp_added="2021-01-01T00:00:00", )
...
@pytest.mark.integration
def test_get_word_tags(dynamodb_table: Literal["MyMockedTable"], sample_word_tags: list[WordTag]):
    # Arrange
    client = DynamoClient(dynamodb_table)
    # Add them all
...
def test_word_tag_get_pagination(dynamodb_table: Literal["MyMockedTable"]):
    ...
    big_list = []
    for i in range(30):
        big_list.append(WordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed4",
                line_id=1,
                word_id=i,
                tag="PAGE",
                timestamp_added="2021-01-01T00:00:00", ))
...
def test_updateWordTags_success(dynamodb_table, sample_word_tag):
    ...
    tag2 = WordTag(image_id=sample_word_tag.image_id,
        line_id=11,
        word_id=6,
        tag="AnotherTag",
        timestamp_added="2021-01-01T00:00:00", )
    ...
    client.deleteWordTags([WordTag(image_id=tag1.image_id,
