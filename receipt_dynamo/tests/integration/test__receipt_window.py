...
def test_addReceiptWindow_raises_value_error_for_none_receipt_window(
    dynamodb_table,
):
    """
    ...
    with pytest.raises(
        ValueError, match="ReceiptWindow parameter is required"
    ):
...
def test_addReceiptWindow_raises_resource_not_found(
    ...
    with pytest.raises(
        Exception, match="Could not add receipt window to DynamoDB"
    ):
        client.addReceiptWindow(example_receipt_window)
...
def test_addReceiptWindow_success(dynamodb_table, example_receipt_window):
    ...
    response = dynamo.get_item(
        ...
        Key={
            ...
            "SK": {
                "S": f"RECEIPT#{example_receipt_window.receipt_id:05d}#"
                f"RECEIPT_WINDOW#{example_receipt_window.corner_name}"
            },
        },
    )
...
def test_addReceiptWindows_raises_value_error_for_none(dynamodb_table):
    ...
    client = DynamoClient(dynamodb_table)
    with pytest.raises(
        ValueError, match="receipt_windows parameter is required"
    ):
...
def test_addReceiptWindows_raises_value_error_for_non_list(dynamodb_table):
    ...
    client = DynamoClient(dynamodb_table)
    with pytest.raises(
        ValueError, match="receipt_windows must be provided as a list"
    ):
...
def test_addReceiptWindows_raises_resource_not_found(
    ...
    with pytest.raises(
        Exception, match="Could not add receipt windows to DynamoDB"
    ):
...
def test_addReceiptWindows_raises_provisioned_throughput(
    ...
    provisioned throughput was exceeded if batch_write_item fails with that
    code.
    """
    client = DynamoClient(dynamodb_table)
...
def test_addReceiptWindows_raises_unknown_exception(
    ...
    """
    Checks that addReceiptWindows raises a generic Exception if
    batch_write_item fails with an unrecognized error code.
    """
    client = DynamoClient(dynamodb_table)
...
@pytest.mark.integration
def test_addReceiptWindows_success_inserts_data(
    dynamodb_table, example_receipt_window
):
    """
    Checks that addReceiptWindows correctly persists multiple items to
    DynamoDB.
    """
    client = DynamoClient(dynamodb_table)
    ...
    for i in [
        ...
        response = dynamo.get_item(
            ...
            Key={
                ...
                "SK": {
                    "S": f"RECEIPT#{rw.receipt_id:05d}#"
                    f"RECEIPT_WINDOW#{rw.corner_name}"
                },
            },
        )
        assert "Item" in response, f"Item {i} should exist in DynamoDB"
        assert (
            response["Item"] == rw.to_item()
        ), f"Item {i} data mismatch in DynamoDB"
...
def test_addReceiptWindows_unprocessed_items_retry_partial_mock(
    ...
    *while still performing real writes* to DynamoDB.
    ...
    for rw in [rw1, rw2]:
        ...
        assert (
            "Item" in response
        ), f"Receipt window {rw.receipt_id} should exist."
        assert (
            response["Item"] == rw.to_item()
        ), "Item data mismatch in DynamoDB."
...
def test_deleteReceiptWindows_unprocessed_items_retry(
    ...
    """
    Simulates a scenario where the first call to batch_write_item for deletion
    returns an unprocessed item (forcing a retry) and the second call returns
    an empty unprocessed dict. Verifies that the deletion method retries and
    that the item is ultimately deleted.
    """
    client = DynamoClient(dynamodb_table)
            ...
            return {
                "UnprocessedItems": {
                    client.table_name: [
                        {
                            "DeleteRequest": {
                                "Key": example_receipt_window.key()
                            }
                        }
                    ]
                }
...
def test_deleteReceiptWindows_raises_value_error_for_none(dynamodb_table):
    """
    Verifies that deleteReceiptWindows raises a ValueError when the input is
    None.
    """
    client = DynamoClient(dynamodb_table)
    with pytest.raises(
...
def test_deleteReceiptWindows_raises_value_error_for_invalid_contents(
    ...
    """
    Verifies that deleteReceiptWindows raises a ValueError when the list
    contains non-ReceiptWindow objects.
    """
    client = DynamoClient(dynamodb_table)
    with pytest.raises(
        ValueError,
        match="All items in the receipt_windows list must be instances of the "
        "ReceiptWindow class.",
    ):
...
def test_listReceiptWindows_success_paginated(
    ...
    ), "Should have called query twice for pagination"


@pytest.mark.integration
def test_listReceiptWindows_with_LEK(
    dynamodb_table, example_receipt_window, mocker
):
    """
    ...
    ), "Expected the returned LEK to match the fake LEK from the query "
    "response."
...
def test_listReceiptWindows_invalid_limit(dynamodb_table):
    """
    Verifies that listReceiptWindows raises a ValueError when 'limit' is not "
    "an integer or None.
    """
    client = DynamoClient(dynamodb_table)
...
def test_listReceiptWindows_invalid_lastEvaluatedKey(dynamodb_table):
    """
    Verifies that listReceiptWindows raises a ValueError when
    'lastEvaluatedKey' is not a dict or None.
    """
    client = DynamoClient(dynamodb_table)
    with pytest.raises(
...
def test_listReceiptWindows_raises_resource_not_found(dynamodb_table, mocker):
    ...
    with pytest.raises(
        Exception, match="Could not list receipt windows from DynamoDB"
    ):
...
@pytest.mark.integration
def test_listReceiptWindows_raises_provisioned_throughput(
...
@pytest.mark.integration
def test_listReceiptWindows_raises_validation_exception(
    dynamodb_table, mocker
):
    """
...
@pytest.mark.integration
def test_listReceiptWindows_raises_internal_server_error(
    dynamodb_table, mocker
):
    """
