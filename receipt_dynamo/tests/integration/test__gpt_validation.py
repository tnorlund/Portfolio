...
def test_update_gpt_validation(dynamodb_table: Literal["MyMockedTable"], sample_gpt_validation):
    """
...
def test_delete_gpt_validation(dynamodb_table: Literal["MyMockedTable"], sample_gpt_validation):
    """
...
def test_gpt_validation_batch_add_list(dynamodb_table: Literal["MyMockedTable"], ):
    ...
    for i in range(3):
        validation = GPTValidation(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
            receipt_id=7 + i,
            query=f"Query {i}",
...
def test_delete_nonexistent_gpt_validation(dynamodb_table: Literal["MyMockedTable"], sample_gpt_validation):
    """
...
def test_listGPTValidations_no_limit(dynamodb_table, sample_gpt_validation):
    """
    Tests that listGPTValidations (without a limit) returns all GPTValidation records.
    """
    client = DynamoClient(dynamodb_table)
    # Clear out any pre-existing GPTValidation records for a clean test.
    # (Depending on your environment you may wish to delete records beforehand.)

    # Add two sample records.
    validation1 = sample_gpt_validation
    validation2 = GPTValidation(image_id=sample_gpt_validation.image_id,
        receipt_id=8,
...
def test_listGPTValidations_with_limit_and_pagination(dynamodb_table):
    ...
    LastEvaluatedKey when more records remain, and that subsequent calls with the LEK
    ...
    for i in range(5):
        validation = GPTValidation(image_id=base_image_id,
            receipt_id=10 + i,
            query=f"Query {i}",
            response=f"Response {i}",
            timestamp_added=datetime(2021, 1, 1, 0, 0, 0), )
...
def test_listGPTValidations_empty_table(dynamodb_table):
    """
    Tests that listGPTValidations returns an empty list and no LEK when there are no GPTValidation records.
    """
    client = DynamoClient(dynamodb_table)
