...
def sample_receipt_word_tag():
    ...
    """
    return ReceiptWordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
        receipt_id=100,
        line_id=5,
        word_id=42,
        tag="SampleTag",
...
def test_add_receipt_word_tag(dynamodb_table: Literal["MyMockedTable"],
    ...
    # Assert
    retrieved = client.getReceiptWordTag(sample_receipt_word_tag.image_id,
...
def test_delete_receipt_word_tag(dynamodb_table: Literal["MyMockedTable"],
    ...
    # Act
    client.deleteReceiptWordTag(image_id=sample_receipt_word_tag.image_id,
    ...
    with pytest.raises(ValueError, match="not found"):
        client.getReceiptWordTag(sample_receipt_word_tag.image_id,
            sample_receipt_word_tag.receipt_id,
            sample_receipt_word_tag.line_id,
            sample_receipt_word_tag.word_id,
            sample_receipt_word_tag.tag,)
...
def test_receipt_word_tag_list(dynamodb_table: Literal["MyMockedTable"]):
    # Arrange
    client = DynamoClient(dynamodb_table)
    receipt_word_tags = [ReceiptWordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
            receipt_id=10,
            line_id=1,
            word_id=i,
            tag=f"Tag{i}",
...
def test_receipt_word_tag_list_from_image(dynamodb_table: Literal["MyMockedTable"],):
    ...
    same_image_tags = [ReceiptWordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed3",
            receipt_id=10,
            line_id=2,
            word_id=i,
            tag=f"ImageTag{i}",
...
def test_receipt_word_tag_get_pagination(dynamodb_table: Literal["MyMockedTable"],):
    ...
    big_list = []
    for i in range(30):
        big_list.append(ReceiptWordTag(image_id="3f52804b-2fad-4e00-92c8-b593da3a8ed4",
                receipt_id=1,
                line_id=i,
                word_id=i,
                tag="PAGE",
    ...
    returned_tuples = {(r.image_id, r.receipt_id, r.line_id, r.word_id) for r in results}
