...
def sample_receipt_words():
    ...
    return [
        ReceiptWord(
            ...
            word_id=i,
            text=f"Word {i}",
            bounding_box={"x": 0.1 * i, "y": 0.2, "width": 0.1, "height": 0.02},
...
@pytest.mark.parametrize("error_code,expected_exception,error_fragment", ERROR_SCENARIOS)
...
@pytest.mark.integration
@pytest.mark.parametrize("error_code,expected_exception,error_fragment", UPDATE_DELETE_ERROR_SCENARIOS)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("add_receipt_word", "not-a-word", "word must be an instance of ReceiptWord"),
        ("update_receipt_word", None, "word cannot be None"),
        ("update_receipt_word", "not-a-word", "word must be an instance of ReceiptWord"),
    ],
)
...
@pytest.mark.integration
@pytest.mark.parametrize("operation", ["add_receipt_words", "update_receipt_words", "delete_receipt_words"])
def test_batch_receipt_word_validation_mixed_types(client: DynamoClient, operation: str):
    """Test validation for mixed types in batch operations."""
    mixed_list = [ReceiptWord(
        receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=10, word_id=1, text="Valid",
        bounding_box={"x": 0.1, "y": 0.2, "width": 0.1, "height": 0.02},
        top_left={"x": 0.1, "y": 0.2}, top_right={"x": 0.2, "y": 0.2},
        bottom_left={"x": 0.1, "y": 0.22}, bottom_right={"x": 0.2, "y": 0.22},
        angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
    ), "not-a-word", 123]
    
    with pytest.raises(EntityValidationError, match="must be an instance of ReceiptWord"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, FIXED_IMAGE_ID, 1, 1, EntityValidationError, "receipt_id must be an integer"),
        ("not-an-int", FIXED_IMAGE_ID, 1, 1, EntityValidationError, "receipt_id must be an integer"),
        (-1, FIXED_IMAGE_ID, 1, 1, EntityValidationError, "receipt_id must be a positive integer"),
        # image_id validation
        (1, None, 1, 1, EntityValidationError, "image_id cannot be None"),
        (1, "not-a-uuid", 1, 1, OperationError, "uuid must be a valid UUIDv4"),
        # line_id validation
        (1, FIXED_IMAGE_ID, None, 1, EntityValidationError, "line_id must be an integer"),
        (1, FIXED_IMAGE_ID, "not-an-int", 1, EntityValidationError, "line_id must be an integer"),
        (1, FIXED_IMAGE_ID, -1, 1, EntityValidationError, "line_id must be a positive integer"),
        # word_id validation
        (1, FIXED_IMAGE_ID, 1, None, EntityValidationError, "word_id must be an integer"),
        (1, FIXED_IMAGE_ID, 1, "not-an-int", EntityValidationError, "word_id must be an integer"),
        (1, FIXED_IMAGE_ID, 1, -1, EntityValidationError, "word_id must be a positive integer"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["not-a-tuple"], EntityValidationError, "indices must be a list of tuples"),
        ([("incomplete",)], EntityValidationError, "tuples with 4 elements"),
        ([(123, 1, 1, 1)], EntityValidationError, "First element of tuple must be a string"),
        ([("not-a-uuid", 1, 1, 1)], OperationError, "uuid must be a valid UUIDv4"),
        ([(FIXED_IMAGE_ID, "not-int", 1, 1)], EntityValidationError, "Second element of tuple must be an integer"),
        ([(FIXED_IMAGE_ID, 1, "not-int", 1)], EntityValidationError, "Third element of tuple must be an integer"),
        ([(FIXED_IMAGE_ID, 1, 1, "not-int")], EntityValidationError, "Fourth element of tuple must be an integer"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"missing": "pk_sk"}], EntityValidationError, "keys must contain 'PK' and 'SK'"),
        ([{"PK": {"S": "WRONG#id"}, "SK": {"S": "valid"}}], EntityValidationError, "PK must start with 'IMAGE#'"),
        ([{"PK": {"S": "IMAGE#123"}, "SK": {"S": "WRONG#"}}], EntityValidationError, "SK must start with 'RECEIPT#'"),
        ([{"PK": {"S": "IMAGE#123"}, "SK": {"S": "RECEIPT#1#LINE#1#"}}], EntityValidationError, "SK must contain 'WORD'"),
        ([{"PK": {"S": "IMAGE#123"}, "SK": {"S": "RECEIPT#00001#LINE#00001#WORD#abc"}}], EntityValidationError, "SK must contain a 5-digit word ID"),
    ],
)
...
def test_list_receipt_words_invalid_limit(client: DynamoClient, limit: Any, expected_message: str):
...
def test_list_receipt_words_invalid_last_evaluated_key(client: DynamoClient):
    ...
    with pytest.raises(EntityValidationError, match="last_evaluated_key must be a dictionary"):
...
@pytest.mark.parametrize(
    ...
    [
        (None, EntityValidationError, "embedding_status must be a string or EmbeddingStatus enum"),
        (123, EntityValidationError, "embedding_status must be a string or EmbeddingStatus enum"),
        ("INVALID_STATUS", EntityValidationError, "embedding_status must be one of:"),
    ],
)
...
@pytest.mark.integration
def test_add_receipt_word_conditional_check_failed(client: DynamoClient, sample_receipt_word: ReceiptWord):
    """Test that ConditionalCheckFailedException in add operations raises EntityAlreadyExistsError."""
    with patch.object(client._client, "put_item") as mock_put:
        mock_put.side_effect = ClientError(
            error_response={"Error": {"Code": "ConditionalCheckFailedException", "Message": "Item already exists"}},
            operation_name="PutItem",
        )
...
@pytest.mark.integration
def test_update_receipt_word_conditional_check_failed(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_delete_receipt_word_conditional_check_failed(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_add_receipt_word_success(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_add_receipt_word_duplicate_raises(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_update_receipt_word_success(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_delete_receipt_word_success(client: DynamoClient, sample_receipt_word: ReceiptWord):
...
@pytest.mark.integration
def test_add_receipt_words_success(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
    """Test successful batch addition of receipt words."""
    client.add_receipt_words(sample_receipt_words)
    
    # Verify all were added
    for word in sample_receipt_words:
        retrieved = client.get_receipt_word(word.receipt_id, word.image_id, word.line_id, word.word_id)
        assert retrieved == word


@pytest.mark.integration
def test_update_receipt_words_success(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
    ...
    for updated_word in updated_words:
        retrieved = client.get_receipt_word(
            updated_word.receipt_id, updated_word.image_id, 
            updated_word.line_id, updated_word.word_id
        )
        assert retrieved.text.startswith("Updated")

@pytest.mark.integration
def test_delete_receipt_words_success(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
    ...
    for word in sample_receipt_words:
        with pytest.raises(EntityNotFoundError):
            client.get_receipt_word(word.receipt_id, word.image_id, word.line_id, word.word_id)

@pytest.mark.integration
def test_get_receipt_words_by_indices_success(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
...
@pytest.mark.integration
def test_get_receipt_words_by_keys_success(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
    ...
    for word in sample_receipt_words:
        key = {
            ...
            "SK": {"S": f"RECEIPT#{word.receipt_id:05d}#LINE#{word.line_id:05d}#WORD#{word.word_id:05d}"},
        }
        keys.append(key)
...
@pytest.mark.integration
def test_list_receipt_words_with_pagination(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
...
@pytest.mark.integration
def test_list_receipt_words_no_limit(client: DynamoClient, sample_receipt_words: List[ReceiptWord]):
...
def test_list_receipt_words_by_embedding_status(client: DynamoClient):
    ...
    word_none = ReceiptWord(
        receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=10, word_id=1, text="None status",
        bounding_box={"x": 0.1, "y": 0.2, "width": 0.1, "height": 0.02},
        top_left={"x": 0.1, "y": 0.2}, top_right={"x": 0.2, "y": 0.2},
        bottom_left={"x": 0.1, "y": 0.22}, bottom_right={"x": 0.2, "y": 0.22},
        angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
    ...
    word_pending = ReceiptWord(
        receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=10, word_id=2, text="Pending status",
        bounding_box={"x": 0.1, "y": 0.2, "width": 0.1, "height": 0.02},
        top_left={"x": 0.1, "y": 0.2}, top_right={"x": 0.2, "y": 0.2},
        bottom_left={"x": 0.1, "y": 0.22}, bottom_right={"x": 0.2, "y": 0.22},
        angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
    ...
    pending_words = client.list_receipt_words_by_embedding_status(EmbeddingStatus.PENDING)
...
def test_list_receipt_words_from_line(client: DynamoClient):
    ...
    words_same_line = [
        ReceiptWord(
            receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=10, word_id=i, text=f"Line word {i}",
            bounding_box={"x": 0.1 * i, "y": 0.2, "width": 0.1, "height": 0.02},
            top_left={"x": 0.1 * i, "y": 0.2}, top_right={"x": 0.1 * i + 0.1, "y": 0.2},
            bottom_left={"x": 0.1 * i, "y": 0.22}, bottom_right={"x": 0.1 * i + 0.1, "y": 0.22},
            angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
        )
        for i in range(1, 3)
    ]
    ...
    word_different_line = ReceiptWord(
        receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=99, word_id=999, text="Different line",
        bounding_box={"x": 0.5, "y": 0.5, "width": 0.1, "height": 0.02},
        top_left={"x": 0.5, "y": 0.5}, top_right={"x": 0.6, "y": 0.5},
        bottom_left={"x": 0.5, "y": 0.52}, bottom_right={"x": 0.6, "y": 0.52},
        angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
    )
...
def test_list_receipt_words_from_receipt(client: DynamoClient):
    ...
    words_same_receipt = [
        ReceiptWord(
            receipt_id=1, image_id=FIXED_IMAGE_ID, line_id=i, word_id=1, text=f"Receipt word {i}",
            bounding_box={"x": 0.1, "y": 0.2 * i, "width": 0.1, "height": 0.02},
            top_left={"x": 0.1, "y": 0.2 * i}, top_right={"x": 0.2, "y": 0.2 * i},
            bottom_left={"x": 0.1, "y": 0.2 * i + 0.02}, bottom_right={"x": 0.2, "y": 0.2 * i + 0.02},
            angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
        )
        for i in range(1, 3)
    ]
    ...
    word_different_receipt = ReceiptWord(
        receipt_id=999, image_id=FIXED_IMAGE_ID_2, line_id=1, word_id=1, text="Different receipt",
        bounding_box={"x": 0.5, "y": 0.5, "width": 0.1, "height": 0.02},
        top_left={"x": 0.5, "y": 0.5}, top_right={"x": 0.6, "y": 0.5},
        bottom_left={"x": 0.5, "y": 0.52}, bottom_right={"x": 0.6, "y": 0.52},
        angle_degrees=0.0, angle_radians=0.0, confidence=0.95,
