...
def sample_job_checkpoint(sample_job):
    ...
    return JobCheckpoint(job_id=sample_job.job_id,
        timestamp=timestamp,
...
def test_addJobCheckpoint_success(job_checkpoint_dynamo, sample_job, sample_job_checkpoint):
    ...
    checkpoint = job_checkpoint_dynamo.getJobCheckpoint(sample_job_checkpoint.job_id, sample_job_checkpoint.timestamp)
...
def test_addJobCheckpoint_raises_value_error_not_instance(job_checkpoint_dynamo,):
    ...
    with pytest.raises(ValueError,
        ...
        job_checkpoint_dynamo.addJobCheckpoint("not a job checkpoint")


@pytest.mark.integration
def test_addJobCheckpoint_raises_conditional_check_failed(job_checkpoint_dynamo, sample_job, sample_job_checkpoint):
    """Test that addJobCheckpoint raises ValueError when the job checkpoint already exists"""
    # Add the job first
    job_checkpoint_dynamo.addJob(sample_job)
...
@pytest.mark.integration
def test_addJobCheckpoint_raises_resource_not_found(job_checkpoint_dynamo, sample_job_checkpoint, mocker):
    """Simulate a ResourceNotFoundException when adding a job checkpoint"""
    mock_put = mocker.patch.object(job_checkpoint_dynamo._client,
        "put_item",
...
@pytest.mark.integration
def test_getJobCheckpoint_success(job_checkpoint_dynamo, sample_job, sample_job_checkpoint):
...
@pytest.mark.integration
def test_getJobCheckpoint_raises_value_error_timestamp_none(job_checkpoint_dynamo, sample_job):
...
@pytest.mark.integration
def test_getJobCheckpoint_raises_value_error_not_found(job_checkpoint_dynamo, sample_job):
    ...
    with pytest.raises(ValueError, match="No job checkpoint found with job ID.*"):
        job_checkpoint_dynamo.getJobCheckpoint(sample_job.job_id, "nonexistent-timestamp")
...
@pytest.mark.integration
def test_updateBestCheckpoint_success(job_checkpoint_dynamo,
    sample_job,
    ...
    job_checkpoint_dynamo.updateBestCheckpoint(sample_job_checkpoint.job_id, sample_job_checkpoint.timestamp)

    # Get both checkpoints to verify the update
    checkpoint1 = job_checkpoint_dynamo.getJobCheckpoint(sample_job_checkpoint.job_id, sample_job_checkpoint.timestamp)
    checkpoint2 = job_checkpoint_dynamo.getJobCheckpoint(sample_job_checkpoint_2.job_id, sample_job_checkpoint_2.timestamp)
...
@pytest.mark.integration
def test_updateBestCheckpoint_raises_value_error_timestamp_none(job_checkpoint_dynamo, sample_job):
...
@pytest.mark.integration
def test_updateBestCheckpoint_raises_value_error_not_found(job_checkpoint_dynamo, sample_job):
    """Test that updateBestCheckpoint raises ValueError when the job checkpoint does not exist"""
    with pytest.raises(ValueError,
        match="Cannot update best checkpoint: No checkpoint found with job ID.*",):
        job_checkpoint_dynamo.updateBestCheckpoint(sample_job.job_id, "nonexistent-timestamp")
...
@pytest.mark.integration
def test_listJobCheckpoints_success(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
    ...
    checkpoints, last_evaluated_key = job_checkpoint_dynamo.listJobCheckpoints(sample_job.job_id)
...
@pytest.mark.integration
def test_listJobCheckpoints_with_limit(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
    ...
    checkpoints, last_evaluated_key = job_checkpoint_dynamo.listJobCheckpoints(sample_job.job_id, limit=1)
...
@pytest.mark.integration
def test_listJobCheckpoints_with_pagination(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
    ...
    # List the first page
    checkpoints_page1, last_evaluated_key = (job_checkpoint_dynamo.listJobCheckpoints(sample_job.job_id, limit=1))
    ...
    checkpoints_page2, last_evaluated_key2 = (job_checkpoint_dynamo.listJobCheckpoints(sample_job.job_id, limit=1, lastEvaluatedKey=last_evaluated_key))

    # Verify second page
    assert len(checkpoints_page2) == 1
    assert (checkpoints_page1[0].timestamp != checkpoints_page2[0].timestamp)  # Different checkpoint on each page
...
@pytest.mark.integration
def test_getBestCheckpoint_success(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
...
@pytest.mark.integration
def test_deleteJobCheckpoint_success(job_checkpoint_dynamo, sample_job, sample_job_checkpoint):
    ...
    job_checkpoint_dynamo.deleteJobCheckpoint(sample_job_checkpoint.job_id, sample_job_checkpoint.timestamp)

    # Verify the job checkpoint was deleted
    with pytest.raises(ValueError, match="No job checkpoint found with job ID.*"):
        job_checkpoint_dynamo.getJobCheckpoint(sample_job_checkpoint.job_id, sample_job_checkpoint.timestamp)
...
@pytest.mark.integration
def test_deleteJobCheckpoint_raises_value_error_timestamp_none(job_checkpoint_dynamo, sample_job):
...
@pytest.mark.integration
def test_listAllJobCheckpoints_success(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
...
@pytest.mark.integration
def test_listAllJobCheckpoints_with_limit(job_checkpoint_dynamo,
    sample_job,
    sample_job_checkpoint,
    sample_job_checkpoint_2,):
    ...
    checkpoints, last_evaluated_key = (job_checkpoint_dynamo.listAllJobCheckpoints(limit=1))
...
@pytest.mark.integration
def test_listJobCheckpoints_raises_client_error(job_checkpoint_dynamo, sample_job, mocker):
    """Test that listJobCheckpoints raises an exception when a ClientError occurs"""
    # Mock the client to raise a ClientError
    mock_query = mocker.patch.object(job_checkpoint_dynamo._client,
        "query",
        side_effect=ClientError({"Error": {"Code": "ResourceNotFoundException",
                    "Message": "Table not found",}},
            "Query",),)
    ...
    with pytest.raises(Exception, match="Could not list job checkpoints from the database"):
        job_checkpoint_dynamo.listJobCheckpoints(sample_job.job_id)
...
def test_listAllJobCheckpoints_raises_client_error(job_checkpoint_dynamo, mocker):
    ...
    # Call the method and verify it raises the expected exception
    with pytest.raises(Exception, match="Could not list all job checkpoints from the database"):
