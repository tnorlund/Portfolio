...
def test_word_init_valid(example_word, example_word_with_tags):
    ...
    assert example_word.bounding_box == {"x": 10.0,
        "y": 20.0,
        "width": 5.0,
        "height": 2.0,}
...
def test_word_key(example_word):
    ...
    assert example_word.key() == {"PK": {"S": "IMAGE#3f52804b-2fad-4e00-92c8-b593da3a8ed3"},
...
def test_word_rotate_limited_range(angle, use_radians, should_raise):
    ...
    """
    word = create_test_word()
    orig_corners = {"top_right": word.top_right.copy(),
    ...
    if should_raise:
        ...
        expected_top_right["x"], expected_top_right["y"] = rotate_point(orig_corners["top_right"]["x"],
            orig_corners["top_right"]["y"],
            0,
            0,
            theta,)
        expected_top_left["x"], expected_top_left["y"] = rotate_point(orig_corners["top_left"]["x"],
            orig_corners["top_left"]["y"],
            0,
            0,
            theta,)
        expected_bottom_right["x"], expected_bottom_right["y"] = rotate_point(orig_corners["bottom_right"]["x"],
            orig_corners["bottom_right"]["y"],
            0,
            0,
            theta,)
        expected_bottom_left["x"], expected_bottom_left["y"] = rotate_point(orig_corners["bottom_left"]["x"],
        ...
        xs = [expected_top_right["x"],
            expected_top_left["x"],
            expected_bottom_right["x"],
            expected_bottom_left["x"],]
        ys = [expected_top_right["y"],
            expected_top_left["y"],
...
@pytest.mark.parametrize("shx, shy, pivot_x, pivot_y, expected_corners",
    [# Test 1: Horizontal shear only (shx nonzero, shy=0)
        (0.2,
            ...
            10.0,
            20.0,
            {"top_right": {"x": 15.0 + 0.2 * (20.0 - 20.0),
                    "y": 20.0,},  # (15.0,20.0)
                # (10.0,20.0)
        ...
        (0.0,
            ...
            20.0,
            {"top_right": {"x": 15.0,
                    "y": 20.0 + 0.2 * (15.0 - 10.0),},  # (15.0,21.0)
                # (10.0,20.0)
                "top_left": {"x": 10.0, "y": 20.0 + 0.2 * (10.0 - 10.0)},
                "bottom_right": {"x": 15.0,
                    "y": 22.0 + 0.2 * (15.0 - 10.0),},  # (15.0,23.0)
                "bottom_left": {"x": 10.0,
                    "y": 22.0 + 0.2 * (10.0 - 10.0),},  # (10.0,22.0)},),
        ...
        (0.1,
            ...
            {# For each corner, calculate:
                ...
                "top_right": {"x": 15.0 + 0.1 * (20.0 - 21.0),  # 15.0 - 0.1 = 14.9
                    "y": 20.0 + 0.1 * (15.0 - 12.0),  # 20.0 + 0.3 = 20.3},
                "top_left": {"x": 10.0 + 0.1 * (20.0 - 21.0),  # 10.0 - 0.1 = 9.9
                    "y": 20.0 + 0.1 * (10.0 - 12.0),  # 20.0 - 0.2 = 19.8},
                "bottom_right": {"x": 15.0 + 0.1 * (22.0 - 21.0),  # 15.0 + 0.1 = 15.1
                    "y": 22.0 + 0.1 * (15.0 - 12.0),  # 22.0 + 0.3 = 22.3},
                "bottom_left": {"x": 10.0 + 0.1 * (22.0 - 21.0),  # 10.0 + 0.1 = 10.1
...
def test_word_iter(example_word, example_word_with_tags):
    ...
    assert word_dict["bounding_box"] == {"x": 10.0,
        "y": 20.0,
        "width": 5.0,
        "height": 2.0,}
...
def test_word_hash(example_word):
    ...
    different_word = Word("3f52804b-2fad-4e00-92c8-b593da3a8ed3",
        2,
        4,
        "test_string",
...
def test_item_to_word(example_word, example_word_with_tags):
    ...
    with pytest.raises(ValueError, match="^Error converting item to Word: "):
        itemToWord({"PK": {"S": "IMAGE#3f52804b-2fad-4e00-92c8-b593da3a8ed3"},
                "SK": {"S": "LINE#00002#WORD#00003"},
                "TYPE": {"S": "LINE"},
