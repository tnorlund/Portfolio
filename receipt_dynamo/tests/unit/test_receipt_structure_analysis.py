...
def test_receipt_structure_analysis_gsi1_key(
    ...
    assert key["GSI1PK"] == "ANALYSIS_TYPE"
    assert key["GSI1SK"] == "STRUCTURE#2023-01-01T12:00:00"


@pytest.mark.unit
def test_receipt_structure_analysis_gsi2_key(
    example_receipt_structure_analysis,
):
...
def test_receipt_structure_analysis_from_dynamo(
    ...
    """Test creating from DynamoDB item through itemToReceiptStructureAnalysis function."""
    # Using the standalone function instead of class method
    analysis = itemToReceiptStructureAnalysis(example_dynamo_item)
...
def test_item_to_receipt_structure_analysis_invalid():
    """Test validation in itemToReceiptStructureAnalysis."""
    with pytest.raises(ValueError):
        itemToReceiptStructureAnalysis({})

    with pytest.raises(ValueError):
        itemToReceiptStructureAnalysis({"image_id": "abc123"})

    with pytest.raises(ValueError):
        itemToReceiptStructureAnalysis({"receipt_id": 123})
