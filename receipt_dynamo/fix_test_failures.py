    ...
    if insert_pos > 0:
        ...
        for condition, message in fixes:
            if condition not in content:
                new_conditions.append(f"        {condition}\n            {message}\n")
        ...
        if new_conditions:
            ...
            print(f"Added {len(new_conditions)} specific error message handlers")
...
def fix_unknown_error_messages():
    ...
    insert_pos = method.find('else:\n            # Generate contextual message')
    if insert_pos > 0:
        new_method = method[:insert_pos] + additional_cases + '\n        ' + method[insert_pos:]
