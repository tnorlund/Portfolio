...
def analyze_all_files():
    ...
    total_stats = {"ValueError": 0, "RuntimeError": 0, "files_with_issues": 0}
    ...
    for filepath in sorted(files):
        ...
        for match in re.finditer(
            r"raise ValueError\((.*?)\)", content, re.DOTALL
        ):
            file_stats["ValueError"] += 1
            error_type = identify_error_type(match.group(1))
            file_stats["ValueError_types"][error_type] = (
                file_stats["ValueError_types"].get(error_type, 0) + 1
            )
        ...
        for match in re.finditer(
            ...
            error_type = identify_error_type(match.group(1))
            file_stats["RuntimeError_types"][error_type] = (
                file_stats["RuntimeError_types"].get(error_type, 0) + 1
            )
    ...
    for file_stats in sorted(
        detailed_report,
        key=lambda x: x["ValueError"] + x["RuntimeError"],
        reverse=True,
    ):
    ...
    for i, file_stats in enumerate(
        ...
        total = file_stats["ValueError"] + file_stats["RuntimeError"]
        print(
            f"{i+1:2d}. python fix_exceptions_single.py {file_stats['name']} # {total} issues"
        )
