...
def invert_affine(a: float, b: float, c: float, d: float, e: float, f: float) -> Tuple[float, float, float, float, float, float]:
    """
...
def invert_warp(a: float, b: float, c: float, d: float, e: float, f: float, g: float, h: float) -> List[float]:
    """
...
def _invert_3x3(M: List[List[float]]) -> List[List[float]]:  # pylint: disable=invalid-name
...
def solve_8x8_system(A: List[List[float]], b: List[float]) -> List[float]:  # pylint: disable=invalid-name
    """
            ...
            for k in range(j + 1, 4):
                if are_collinear(src_points[i], src_points[j], src_points[k]):
                    raise ValueError(
                        ...
                        f"cannot compute perspective transform"
                    )
