...
def handle_dynamodb_errors(operation_name: str) -> Callable:
    """
...
class ErrorHandler:
    ...
    def _handle_resource_not_found(
        ...
        if any(op in operation for op in ["queue", "receipt_field"]):
            ...
            if operation_type in self.config.OPERATION_MESSAGES:
                message = self.config.OPERATION_MESSAGES[operation_type].format(entity_type=entity_type)
            else:
                message = f"Table not found for operation {operation}"
    ...
    def _handle_throughput_exceeded(
        ...
        message = error.response.get("Error", {}).get("Message", "Provisioned throughput exceeded")
    ...
    def _raise_already_exists_error(
        self, operation: str, context: Optional[Dict[str, Any]], error: ClientError
    ) -> NoReturn:
        ...
        entity_type = self.context_extractor.extract_entity_type(operation).replace(" ", "_")
