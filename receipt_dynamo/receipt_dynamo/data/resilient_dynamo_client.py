...
class ResilientDynamoClient(DynamoClient):
    ...
    def __init__(
        ...
        if self.enable_batch_processing:
            ...
            self.flush_thread = threading.Thread(target=self._auto_flush_worker, daemon=True)
    ...
    def _batch_write_with_retry(self, metrics: List[AIUsageMetric]) -> None:
        ...
        for attempt in range(self.max_retry_attempts):
            if not self._check_circuit_breaker():
                ...
                print(f"Circuit breaker OPEN, skipping {len(remaining_metrics)} metrics")
                return
        ...
        if remaining_metrics:
            print(f"Failed to write {len(remaining_metrics)} metrics after {self.max_retry_attempts} attempts")
