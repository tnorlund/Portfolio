...
class _AIUsageMetric:
    ...
    def batch_put_ai_usage_metrics(self, metrics: List[AIUsageMetric]) -> List[AIUsageMetric]:
        """
                ...
                for request in unprocessed_requests:
                    if "PutRequest" in request:
                        ...
                        for j, metric in enumerate(metrics[i:i + 25]):
                            if metric.request_id == item.get("request_id", {}).get("S"):
    ...
    def get_ai_usage_metric(self, date: str, timestamp: str, service: str, model: str) -> Optional[Dict[str, Any]]:
        """
