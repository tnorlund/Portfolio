...
class _ReceiptLetter:
    ...
    def addReceiptLetter(self, letter: ReceiptLetter):
        ...
        try:
            ...
            if (e.response["Error"]["Code"]
                ...
                raise ValueError(f"ReceiptLetter with ID {letter.letter_id} already exists")
            else:
                raise
    ...
    def updateReceiptLetter(self, letter: ReceiptLetter):
        ...
        try:
            ...
            if (e.response["Error"]["Code"]
                == "ConditionalCheckFailedException"):
                raise ValueError(f"ReceiptLetter with ID {letter.letter_id} does not exist")
            else:
                raise

    def deleteReceiptLetter(self,
        receipt_id: int,
        image_id: str,
        line_id: int,
        word_id: int,
        ...
        try:
            ...
            if limit is None:
                ...
                while "LastEvaluatedKey" in response:
                    ...
                    response = self._client.query(**query_params)
                    receipt_letters.extend([itemToReceiptLetter(item)
            ...
            if error_code == "ResourceNotFoundException":
                raise Exception(f"Could not list receipt letters from DynamoDB: {e}") from e
    ...
    def listReceiptLettersFromWord(self, receipt_id: int, image_id: str, line_id: int, word_id: int) -> list[ReceiptLetter]:
        """Returns all ReceiptLetters for a given word."""
        receipt_letters = []
        try:
