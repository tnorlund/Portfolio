...
class _Letter:
    ...
    def addLetters(self, letters: list[Letter]):
        ...
        try:
            for i in range(0, len(letters), CHUNK_SIZE):
                ...
                request_items = [{"PutRequest": {"Item": letter.to_item()}}
                    for letter in chunk]
    ...
    def deleteLetters(self, letters: list[Letter]):
        ...
        try:
            for i in range(0, len(letters), CHUNK_SIZE):
                chunk = letters[i : i + CHUNK_SIZE]
                request_items = [{"DeleteRequest": {"Key": letter.key()}}
                    for letter in chunk]
                response = self._client.batch_write_item(RequestItems={self.table_name: request_items})
                # Handle unprocessed items if they exist
                unprocessed = response.get("UnprocessedItems", {})
    ...
    def getLetter(self, image_id: str, line_id: int, word_id: int, letter_id: int) -> Letter:
        try:
    ...
    def listLettersFromWord(self, image_id: str, line_id: int, word_id: int) -> list[Letter]:
        letters = []
        try:
