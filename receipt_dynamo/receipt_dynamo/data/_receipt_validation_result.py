...
class _ReceiptValidationResult(
    ...
    def _handle_client_error(
        ...
        if error_code == "ConditionalCheckFailedException":
            # Already handled correctly by base class
            super()._handle_client_error(error, operation, context)
        elif error_code == "ResourceNotFoundException":
            # For batch operations, use base class format
            if (
                "add_receipt_validation_results" in operation
                or "update_receipt_validation_results" in operation
                or "delete_receipt_validation_results" in operation
            ):
                # Let base class handle batch operations
                super()._handle_client_error(error, operation, context)
            # Tests expect specific messages without operation names for single operations
            ...
            # For unknown errors, tests expect specific prefixes
            # For batch operations, use base class format
            if (
                "add_receipt_validation_results" in operation
                or "update_receipt_validation_results" in operation
                ...
                raise DynamoDBError(
                    f"Could not add receipt validation result to DynamoDB"
                ) from error
            elif "update" in operation:
                raise DynamoDBError(
                    f"Could not update ReceiptValidationResult in the database"
                ) from error
            elif "delete" in operation:
                raise DynamoDBError(
                    f"Could not delete receipt validation result from the database"
                ) from error
            else:
                # Let base class handle other cases
                super()._handle_client_error(error, operation, context)
