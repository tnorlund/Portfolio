        ...
        for i, content_pattern in enumerate(content_patterns):
            if not isinstance(content_pattern, ContentPattern):
                if isinstance(content_pattern, dict):
                    content_patterns[i] = ContentPattern.from_dict(content_pattern)
...
class ReceiptStructureAnalysis:
            ...
            for key, value in self.processing_metrics.items():
                if key == "section_count":
                    ...
                    if isinstance(value, list):
                        metrics_dict["M"][key] = {"L": [{"S": str(t)} for t in value]}
                    else:
    ...
    def get_sections_with_pattern(self, pattern_type: str) -> List[ReceiptSection]:
        ...
        for section in self.sections:
            ...
            for content_pattern in section.content_patterns:
                if (
                    pattern_type.lower() in content_pattern.pattern_type.lower()
                    or pattern_type.lower() in content_pattern.description.lower()
                ):
                    matching_sections.append(section)
                    break
    ...
    def summarize_structure(self) -> str:
        ...
        """
        result: List[str] = [f"Receipt contains {len(self.sections)} sections."]
        ...
        for i, section in enumerate(self.sections):
            section_summary = [
                f"Section {i+1}: {section.name} " f"({len(section.line_ids)} lines)"
            ]
