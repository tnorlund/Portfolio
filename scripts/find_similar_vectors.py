...
class VectorSimilarityFinder:
    ...
    def download_snapshot(self, snapshot_path: str) -> str:
        ...
        print(f"📥 Downloading snapshot from s3://{self.bucket_name}/{snapshot_path}")
    ...
    def find_similar_vectors(
        ...
        print(f"   Receipt: #{target_metadata.get('receipt_id', 'N/A')} Line: #{target_metadata.get('line_id', 'N/A')}")
        ...
        for i in range(len(similar['ids'][0])):
            ...
            print(f"     Receipt: #{result_info['receipt_id']} Line: #{result_info['line_id']}")
...
def main():
    ...
    SPROUTS_VECTOR_ID = "IMAGE#63aa7edd-cf85-42fd-8c62-363981a115fe#RECEIPT#00001#LINE#00028"
    ...
    try:
        ...
        results = finder.find_similar_vectors(
            target_vector_id=SPROUTS_VECTOR_ID,
            n_results=10
        )
        ...
        print(f"\n" + "=" * 80)
        print(f"🎉 Successfully found {len(results['similar_vectors'])} similar vectors!")
