...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "mode", choices=("preflight", "verify-candidates", "run-once")
    )
    parser.add_argument(
...
def _parse_time(value: Any, field: str) -> datetime:
    ...
    try:
        ...
        raise ValueError(
            f"{field} is not valid ISO-8601: {value!r}"
        ) from error
...
def _validate_preregistration(artifact_dir: Path) -> str:
    ...
    if not declared_parts or declared_parts[0] != actual_hash:
        raise ValueError(
            "PREREGISTRATION.sha256 does not match exact JSON bytes"
        )
...
def _manifest_receipts(value: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for image_index, image in enumerate(images):
        ...
        if not isinstance(entries, list):
            raise ValueError(
                f"upload image {image_index} receipts must be a list"
            )
        for entry in entries:
            receipt = (
                dict(entry)
                if isinstance(entry, dict)
                else {"receipt_id": entry}
            )
...
def _validate_upload_manifest(
    ...
    baseline_time = (
        _parse_time(baseline_value, "t0_utc")
        if baseline_value is not None
        else None
    )
    if baseline_time is not None and baseline_time <= freeze_time:
        raise ValueError(
            "upload intake baseline must follow the corrected commit"
        )
    ...
    for index, item in enumerate(receipts):
        ...
        if uploaded_at is None:
            raise ValueError(
                f"receipt {index} needs uploaded_at or a manifest t0_utc"
            )
        ...
        if merchant is not None and not isinstance(merchant, str):
            raise ValueError(
                f"receipts[{index}].merchant must be a string or null"
            )
        normalized.append(
            {
                "image_id": key[0],
...
def _validate_truth_lock(
    ...
    *,
    ...
    if value.get("upload_manifest_sha256") != upload_sha256:
        raise ValueError(
            "truth lock is not bound to the exact upload manifest bytes"
        )
    ...
    for index, item in enumerate(receipts):
        ...
        for row_index, row in enumerate(rows):
            ...
            if row_id in row_ids:
                raise ValueError(
                    f"duplicate truth row_id {row_id} in receipt {index}"
                )
    ...
    return (
        {
            ...
            "ambiguous_row_proportion": (
                ambiguous_row_count / labeled_row_count
                if labeled_row_count
                else 0.0
            ),
            "truth_row_count": sum(
                len(item["truth_rows"]) for item in job_receipts
            ),
        },
        job_receipts,
    )
...
def _assert_no_prior_run(artifact_dir: Path) -> None:
    present = sorted(
        name for name in _OUTPUT_NAMES if (artifact_dir / name).exists()
    )
    if present:
...
def _archived_candidate(
    repo_root: Path, commit: str, parent: Path
) -> Iterator[Path]:
...
def _verify_candidates(
    repo_root: Path, python: Path, worker: Path
) -> dict[str, Any]:
...
def _metric_passes(
    result: Mapping[str, Any], name: str, threshold: float
) -> bool:
...
def _compare_results(
    ...
    snapshots_match = (
        frozen_eval["input_snapshot_sha256"]
        == corrected_eval["input_snapshot_sha256"]
    )
    frozen_receipts = {item["case"]: item for item in frozen_eval["receipts"]}
    corrected_receipts = {
        item["case"]: item for item in corrected_eval["receipts"]
    }
        ...
        per_receipt.append(
            {
                ...
                "agreement_delta": (
                    None
                    if first["agreement"] is None
                    or second["agreement"] is None
    ...
    return {
        ...
        "fragmentation": {
            ...
            "corrected_minus_frozen": {
                field: int(corrected_fragment[field])
                - int(frozen_fragment[field])
                for field in (
                    "run_count",
                    "single_row_run_count",
                    "strict_island_count",
...
def _confusion_markdown(title: str, matrix: Mapping[str, Any]) -> list[str]:
    ...
    lines = [f"### {title}", ""]
    lines.append(
        "| Truth \\ Predicted | " + " | ".join(predicted_labels) + " |"
    )
...
def _run_once(
    *,
    ...
    candidate_metadata = {
        name: _commit_metadata(repo_root, commit)
        for name, commit, _ in _CANDIDATES
    }
    state: dict[str, Any] = {
        "schema_version": 1,
    ...
    try:
        ...
        comparison = _compare_results(
            results["frozen_v4"], results["corrected"]
        )
...
def main() -> int:
    ...
    if args.mode == "preflight":
        print(
            json.dumps(
                _preflight(artifact_dir, repo_root), indent=2, sort_keys=True
            )
        )
        return 0
