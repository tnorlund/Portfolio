...
echo ""
echo "2. Start the runner as a LaunchAgent (independent of the terminal, restarts on crash):"
echo "   cd ${RUNNER_DIR} && ./svc.sh install && ./svc.sh start"
echo "   (status: ./svc.sh status; stop: ./svc.sh stop)"
echo "   LaunchAgents stop at logout and start again at login, including after a reboot."
echo ""
echo "   For a one-off foreground run instead:"
echo "   cd ${RUNNER_DIR} && ./run.sh"
echo ""
echo -e "${YELLOW}3. MacBook hosts: keep the machine awake or jobs die mid-run with"
echo -e "   'The self-hosted runner lost communication with the server'.${NC}"
echo "   Either disable sleep on AC power:"
echo "     sudo pmset -c sleep 0 disksleep 0"
echo "   or keep a caffeinate session running alongside the runner:"
