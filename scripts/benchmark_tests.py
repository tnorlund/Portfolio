...
def run_tests(package: str, config: str, options: List[str]) -> Tuple[float, bool]:
    ...
    cmd = [sys.executable, "-m", "pytest", f"{package}/tests", *options]
...
def benchmark_configurations():
    ...
    configurations = {
        "baseline": ["-m", "not end_to_end", "--tb=short", "-v"],
        "parallel_2_workers": ["-n", "2", "-m", "not end_to_end", "--tb=short", "-q"],
        "parallel_auto": ["-n", "auto", "-m", "not end_to_end", "--tb=short", "-q"],
        "parallel_fast": [
            "-n",
            "auto",
            "-m",
    ...
    for package in packages:
        ...
        for config_name, options in configurations.items():
            ...
            subprocess.run(
                [sys.executable, "-m", "pytest", "--cache-clear"], capture_output=True
            )
