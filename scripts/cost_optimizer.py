...
class CostOptimizer:
        ...
        default_config = {
            ...
            'file_patterns': {
                'security': [
                    ...
                    '**/login*.py', '**/password*.py', '**/token*.py'
                ],
                'architecture': [
                    '**/models/*.py', '**/core/*.py', '**/base*.py',
                    '**/abstract*.py', '**/interface*.py'
                ],
                'tests': [
    ...
    def analyze_pr_characteristics(self, pr_data: Dict) -> Dict:
        ...
        test_files = [f for f in changed_files if self._matches_pattern(f, test_patterns)]
    ...
    def _calculate_complexity(self, pr_data: Dict) -> int:
        ...
        if len(description) > 100:
            score += 10
        if any(word in description.lower() for word in ['breaking', 'major', 'refactor']):
            score += 10
    ...
    def check_budget_limits(self) -> Dict:
        ...
        return {
            ...
            'can_proceed': (
                usage['monthly_cost'] < config['monthly_budget'] and
                usage['daily_cost'] < config['daily_limit']
            )
        }
    ...
    def log_usage(self, model: str, pr_number: int, cost: float, 
                  input_tokens: int, output_tokens: int, reason: str):
...
def main(pr_number: Optional[int], repository: Optional[str], 
         config: Optional[str], report: bool, check_budget: bool):
...
""")
    ...
    estimated_cost = optimizer.estimate_cost(model, estimated_input_tokens, estimated_output_tokens)
