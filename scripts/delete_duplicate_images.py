#!/usr/bin/env python3
"""
Delete duplicate images and all their related sub-records.

This script:
1. Loads the deletion plan
2. For each duplicate image, deletes all related data:
   - Receipt word labels
   - Receipt words
   - Receipt letters
   - Receipt lines
...
Usage:
    # Dry-run
    python scripts/delete_duplicate_images.py --stack dev \
...
def main():
    parser = argparse.ArgumentParser(
        description="Delete duplicate images and all related sub-records"
    )
    parser.add_argument(
        "--stack",
        required=True,
    ...
    try:
        # Load label report to get images to delete
        ...
        logger.info(f"Found {len(images_to_delete)} duplicate images to delete")
        ...
        if args.dry_run:
            ...
            if not args.yes:
                response = input(
                    f"Are you sure you want to delete {len(images_to_delete)} images? (yes/no): "
                )
        ...
        for i, image_id in enumerate(sorted(images_to_delete), 1):
            logger.info(
