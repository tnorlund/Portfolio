...
if [ -z "$ASG_NAME" ]; then
    ...
    if [ -n "$INSTANCE_ID" ] && [ "$INSTANCE_ID" != "None" ]; then
        ...
        if [ -n "$INSTANCE_IP" ] && [ "$INSTANCE_IP" != "None" ]; then
            ...
            echo "   sudo /usr/bin/aws s3 cp s3://$BUCKET/wheels/ /opt/wheels/ --recursive"
            echo "   sudo pip3 install --force-reinstall /opt/wheels/receipt_dynamo-*.whl"
            echo "   sudo pip3 install --force-reinstall /opt/wheels/receipt_layoutlm-*.whl"
            echo ""
