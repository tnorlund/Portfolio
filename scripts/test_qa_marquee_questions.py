...
def setup_langsmith_tracing(config: dict, secrets: dict) -> None:
    ...
    # Set project name
    os.environ["LANGCHAIN_PROJECT"] = "question-answering-marquee"
...
async def run_question_with_own_graph(
    ...
    async with _semaphore:
        ...
        try:
            ...
            logger.info("Question %d/%d completed in %.1fs", index, total, duration)
...
def print_summary(results: list[QuestionResult]) -> None:
    ...
    print("QUICK RESULTS")
    print("-" * 70)
    print(f"{'#':<3} {'Question':<45} {'Amount':<12} {'Time':<8}")
    print("-" * 70)
    ...
    for i, r in enumerate(results, 1):
        ...
        print(f"{i:<3} {q_short:<45} {amount:<12} {r.duration_seconds:.1f}s {status}")
...
def main():
    ...
    setup_langsmith_tracing(config, secrets)
