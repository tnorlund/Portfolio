...
RESTORABLE_TYPES = {
    ...
    # reading the STORED row, so a missing summary loses them). Line items
    # ARE regenerated by prod's own updater after the copy lands, but they
    # are copied anyway so the partition is never briefly empty, and listed
    # here so their presence never blocks a REPLACE.
    "RECEIPT_ROW",
    "RECEIPT_SECTION",
    "RECEIPT_LINE_ITEM",
    "RECEIPT_SUMMARY",
...
def _fingerprint_env(client: DynamoClient) -> dict:
    ...
    # NOTE: RECEIPT_LINE_ITEM is copied but deliberately NOT fingerprinted.
    # Prod re-derives line items itself: the line-item updater Lambda fires
    # off the stream when sections/summaries land and rewrites them with
    # prod's extractor (stamp "line-items-blocks-v2"), which does not agree
    # with the Swift worker that produced dev's rows
    # ("swift-worker-v1+line-items-blocks-v2"). Observed in the 2026-09-13
    # rehearsal: 3 items copied, prod rewrote to 1 minutes later. Hashing
