...
class PRAnalyzer:
    ...
    def _get_pr_diff(self, pr_number: int) -> str:
        ...
        try:
            ...
            pr = self.repo.get_pull_request(pr_number)
            return requests.get(
                pr.diff_url, headers={"Authorization": f"token {self.token}"}
            ).text
    ...
    def extract_cursor_findings(self, pr_data: Dict) -> List[Dict]:
        ...
        for comment in all_comments:
            ...
            if (
                hasattr(comment, "user")
                and comment.user
                and (
                    "cursor" in comment.user.login.lower()
                    or "bot" in comment.user.login.lower()
                )
            ):
    ...
    def analyze_architecture_impact(self, pr_data: Dict) -> Dict:
        ...
        analysis = {"concerns": [], "improvements": [], "recommendations": []}
        ...
        if any("conftest.py" in f for f in changed_files):
            analysis["concerns"].append(
                "Global test configuration changes - ensure backward compatibility"
            )

        if any(".github/workflows" in f for f in changed_files):
            analysis["concerns"].append(
                "CI/CD workflow changes - test thoroughly before merge"
            )

        if any("scripts/" in f for f in changed_files):
            analysis["improvements"].append(
                "Build/deployment script improvements detected"
            )

        # Check for test organization patterns
        test_files = [
            f for f in changed_files if "test_" in f or f.endswith("_test.py")
        ]
        if test_files:
            if any("/unit/" in f for f in test_files) and any(
                "/integration/" in f for f in test_files
            ):
                analysis["improvements"].append(
                    "Following directory-based test organization pattern"
                )
            elif any("@pytest.mark" in pr_data["diff"]):
                analysis["recommendations"].append(
                    "Consider migrating to directory-based test organization for better CI performance"
                )
    ...
    def analyze_performance_impact(self, pr_data: Dict) -> Dict:
        ...
        analysis = {"optimizations": [], "concerns": [], "metrics": {}}
        ...
        if pr_data["additions"] > 500:
            analysis["concerns"].append(
                f"Large change set ({pr_data['additions']} additions) - consider splitting PR"
            )
    ...
    def analyze_test_strategy(self, pr_data: Dict) -> Dict:
        ...
        if any("pytest" in f for f in pr_data["changed_files"]):
            analysis["recommendations"].append(
                "Test configuration changed - verify all test types still work"
            )
    ...
    def generate_review_summary(
        self,
        pr_data: Dict,
        cursor_findings: List[Dict],
        architecture: Dict,
        performance: Dict,
        testing: Dict,
        fast_validation_passed: bool = True,
    ) -> str:
