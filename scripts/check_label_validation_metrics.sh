#!/bin/bash
# Check Label Validation Agent Custom Metrics from CloudWatch
#
# Usage:
#   ./scripts/check_label_validation_metrics.sh [hours]
#
# Options:
#   hours    Number of hours to look back (default: 24)
#
# Examples:
#   ./scripts/check_label_validation_metrics.sh 24    # Last 24 hours (default)
#   ./scripts/check_label_validation_metrics.sh 2      # Last 2 hours
#   ./scripts/check_label_validation_metrics.sh 168   # Last week

set -e

HOURS="${1:-24}"
REGION="${AWS_REGION:-us-east-1}"
NAMESPACE="LabelValidationAgent"
...
echo "==========================================="
echo "LABEL VALIDATION AGENT METRICS"
echo "==========================================="
...
# ============================================
# 1. VALIDATION DECISION METRICS BY LABEL TYPE
# ============================================
echo "📊 VALIDATION DECISION METRICS BY LABEL TYPE:"
echo ""

for label in "${LABELS[@]}"; do
    labels_processed=$(get_metric "LabelsProcessed" "$label")
    valid_count=$(get_metric "ValidCount" "$label")
    invalid_count=$(get_metric "InvalidCount" "$label")
    needs_review_count=$(get_metric "NeedsReviewCount" "$label")
    ...
    if [ "$labels_processed" != "0" ] || [ "$batches" != "0" ]; then
        ...
        if [ "$labels_processed" != "0" ]; then
            valid_rate=$(echo "scale=1; $valid_count * 100 / $labels_processed" | bc 2>/dev/null || echo "0")
            invalid_rate=$(echo "scale=1; $invalid_count * 100 / $labels_processed" | bc 2>/dev/null || echo "0")
            review_rate=$(echo "scale=1; $needs_review_count * 100 / $labels_processed" | bc 2>/dev/null || echo "0")
            echo "    Valid Rate: ${valid_rate}%"
            echo "    Invalid Rate: ${invalid_rate}%"
            echo "    Needs Review Rate: ${review_rate}%"
        fi
...
for label in "${LABELS[@]}"; do
    ...
    valid=$(get_metric "ValidCount" "$label")
    invalid=$(get_metric "InvalidCount" "$label")
    needs_review=$(get_metric "NeedsReviewCount" "$label")
    batches=$(get_metric "BatchSucceeded" "$label")
    failed_batches=$(get_metric "BatchFailed" "$label")
...
echo "  Total Needs Review: $total_needs_review"

if [ "$total_labels" != "0" ]; then
    valid_rate=$(echo "scale=2; $total_valid * 100 / $total_labels" | bc 2>/dev/null || echo "0")
    invalid_rate=$(echo "scale=2; $total_invalid * 100 / $total_labels" | bc 2>/dev/null || echo "0")
    review_rate=$(echo "scale=2; $total_needs_review * 100 / $total_labels" | bc 2>/dev/null || echo "0")
    echo "  Valid Rate: ${valid_rate}%"
    echo "  Invalid Rate: ${invalid_rate}%"
    echo "  Needs Review Rate: ${review_rate}%"
fi
