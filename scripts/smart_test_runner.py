...
def should_run_test(test_file: Path, package_dir: Path, changed_files: Set[str], 
    ...
    if cache_file.exists():
        try:
            ...
            if test_key in cache:
                ...
                last_run = datetime.fromisoformat(test_info.get('last_success', '2000-01-01'))
...
def update_test_cache(test_file: Path, package_dir: Path, cache_file: Path, 
                     success: bool):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Smart test runner with change detection")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Show which tests would run")
    parser.add_argument("--force", action="store_true", help="Run all tests regardless of changes")
    parser.add_argument("--base-ref", default="origin/main", help="Base git reference for change detection")
    ...
    print(f"   Changed files: {len(changed_files) if '*' not in changed_files else 'all'}")
    ...
    cmd = [
        ...
        str(package_dir),
        test_paths_str,
        "--test-type", "integration" if "integration" in test_paths_str else "unit"
    ]
