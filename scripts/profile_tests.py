...
def generate_recommendations(results: Dict[str, Any], slow_tests: List[Dict], file_stats: Dict) -> List[str]:
    """Generate optimization recommendations."""
    recommendations = []
    
    # Slow test recommendations
    if slow_tests:
        recommendations.append(f"Mark {len(slow_tests)} slow tests with @pytest.mark.slow:")
        for test in slow_tests[:5]:  # Show top 5
            recommendations.append(f"  - {test['test']} ({test['duration']:.2f}s)")
    
    # File-level recommendations
    slowest_files = sorted(file_stats.items(), key=lambda x: x[1]['total_time'], reverse=True)[:3]
    if slowest_files:
        recommendations.append("Slowest test files to optimize:")
        for file_name, stats in slowest_files:
            recommendations.append(f"  - {file_name}: {stats['total_time']:.2f}s total, {stats['avg_time']:.2f}s avg")
    ...
    if test_count > 50 and total_time > 30:
        ...
        recommendations.append(f"Parallel execution could reduce time from {total_time:.1f}s to ~{theoretical_parallel_time:.1f}s")
    ...
    for package, test_path in packages:
        ...
        try:
            ...
            if 'total_measured_time' in analysis:
                ...
                print(f"  Slow tests (>{analysis['slow_threshold']:.1f}s): {len(analysis['slow_tests'])}")
