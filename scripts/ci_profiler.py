...
class SimpleCIProfiler:
        ...
        for job_name, job_metrics in jobs.items():
            analysis["jobs"][job_name] = {
                ...
                "avg_memory_usage": sum(
                    m["system_info"]["memory_total"]
                    - m["system_info"]["memory_available"]
    ...
    def generate_simple_report(self, analysis: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            f"**Analysis Period:** {analysis['summary']['time_range']['start']} "
            f"to {analysis['summary']['time_range']['end']}",
            "",
            "## Summary",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Job name for metrics collection",
    )
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze collected metrics"
    )
    parser.add_argument(
