...
def visualize_final_clusters_cropped(
    ...
    clustering_params = {
        ...
        "vertical_gap_threshold": (
            vertical_gap_threshold
            if vertical_gap_threshold is not None
            else 0.15
        ),
        "reassign_x_proximity_threshold": (
            reassign_x_proximity_threshold
            if reassign_x_proximity_threshold is not None
            else 0.1
        ),
        "reassign_y_proximity_threshold": (
            reassign_y_proximity_threshold
            if reassign_y_proximity_threshold is not None
            else 0.15
        ),
        "vertical_proximity_threshold": (
            vertical_proximity_threshold
            if vertical_proximity_threshold is not None
            else 0.05
        ),
        "vertical_x_proximity_threshold": (
            vertical_x_proximity_threshold
            if vertical_x_proximity_threshold is not None
            else 0.1
        ),
        "merge_x_proximity_threshold": (
            merge_x_proximity_threshold
            if merge_x_proximity_threshold is not None
            else 0.4
        ),
        "merge_min_score": (
            merge_min_score if merge_min_score is not None else 0.5
        ),
        "join_iou_threshold": (
            join_iou_threshold if join_iou_threshold is not None else 0.1
        ),
    }
