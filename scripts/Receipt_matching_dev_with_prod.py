...
def download_detailed_data_for_comparison(table_name: str, output_dir: str, force_download: bool = False):
    """
...
def compare_labels(
    ...
    for prod_img in tqdm(prod_data, desc="Comparing labels", unit="image"):
        ...
        prod_labels_by_key = {
            (img['image_id'], img['receipt_id'], img['line_id'], img['word_id']): img
        ...
        for key, prod_label in prod_labels_by_key.items():
            ...
            if key in dev_labels_by_key:
                ...
                elif prod_label.get('validation_status') != dev_label.get('validation_status'):
                    label_differences.append({
                        'image_id': image_id,
                        'receipt_id': prod_label.get('receipt_id'),
                        'line_id': prod_label.get('line_id'),
    ...
    status_diffs = [d for d in label_differences if d.get('type') == 'validation_status_diff']
    text_diffs = [d for d in label_differences if d.get('type') != 'validation_status_diff']
    ...
    for diff in status_diffs[:20]:  # Show first 20
        ...
        report.append(f"    Receipt: {diff['receipt_id']}, Line: {diff['line_id']}, Word: {diff['word_id']}")
    ...
    for diff in text_diffs[:20]:  # Show first 20
        report.append(f"  Image ID: {diff['image_id']}")
        report.append(f"    Receipt: {diff['receipt_id']}, Line: {diff['line_id']}, Word: {diff['word_id']}")
        report.append(f"    PROD: {diff['prod_label']} (status: {diff['prod_validation_status']})")
        report.append(f"    DEV: {diff['dev_label']} (status: {diff['dev_validation_status']})")
        report.append("")
...
def rollback_prod_labels_from_backup(
    ...
    for label_dict in tqdm(all_backup_labels, desc="Preparing restore", unit="label"):
        label_obj = ReceiptWordLabel(
            ...
            timestamp_added=label_dict.get('timestamp_added', datetime.now().isoformat()),
    ...
    try:
        ...
        for image_id in tqdm(all_image_ids, desc="Fetching current labels"):
            try:
                ...
                current_labels, _ = prod_client.list_receipt_word_labels_for_image(image_id)
                for label in current_labels:
                    key = (label.image_id, label.receipt_id, label.line_id, label.word_id, label.label)
        ...
        if labels_to_update:
            ...
            batch_size = 25
            updated_count = 0
            for i in tqdm(range(0, len(labels_to_update), batch_size), desc="Updating batches", unit="batch"):
        ...
        if labels_to_add:
            ...
            batch_size = 25
            added_count = 0
            for i in tqdm(range(0, len(labels_to_add), batch_size), desc="Adding batches", unit="batch"):
        ...
        backup_keys = {
            (label.image_id, label.receipt_id, label.line_id, label.word_id, label.label)
            for label in backup_label_objects
        }
        
        labels_to_delete = [
            label for key, label in current_labels_dict.items()
...
def sync_prod_labels_from_dev(
    ...
    for prod_img in tqdm(prod_data, desc="Analyzing", unit="image"):
        ...
        all_positions = set(list(prod_labels_by_key.keys()) + list(dev_labels_by_key.keys()))
        ...
        for position_key in all_positions:
            ...
            for label_value, dev_label in dev_by_label_value.items():
                if label_value in prod_by_label_value:
                    ...
                    if (prod_label.get('label') != dev_label.get('label') or
                        prod_label.get('validation_status') != dev_label.get('validation_status') or
                        prod_label.get('reasoning') != dev_label.get('reasoning')):
                        needs_update = True
    ...
    for item in tqdm(labels_to_update, desc="Preparing updates", unit="label"):
        ...
        label_obj = ReceiptWordLabel(
            ...
            timestamp_added=dev_label.get('timestamp_added', datetime.now().isoformat()),
    ...
    if labels_to_update_objects:
        ...
        updated_count = 0
        try:
            for i in tqdm(range(0, len(labels_to_update_objects), batch_size), desc="Updating batches", unit="batch"):
    ...
    if labels_to_add_objects:
        ...
        added_count = 0
        try:
            for i in tqdm(range(0, len(labels_to_add_objects), batch_size), desc="Adding batches", unit="batch"):
...
def main():
    parser = argparse.ArgumentParser(description="Download and compare receipts between environments")
    parser.add_argument('command', choices=['download', 'download-detailed', 'compare', 'run', 'compare-labels', 'sync-labels', 'rollback'],
    ...
    if args.command == 'download':
        ...
        download_detailed_data_for_comparison(table_name, args.output_dir, force_download=args.force)
        ...
        if not args.prod_file or not args.dev_file:
            print("Error: --prod-file and --dev-file are required for compare-labels command")
            sys.exit(1)
        compare_labels(args.prod_file, args.dev_file, args.report)
    
    elif args.command == 'sync-labels':
        if not args.prod_file or not args.dev_file:
            print("Error: --prod-file and --dev-file are required for sync-labels command")
            sys.exit(1)
        ...
        if not args.prod_file:
            ...
            print("This should be the PROD backup JSON file created by download-detailed")
            sys.exit(1)
        ...
        download_detailed_data_for_comparison(prod_table, prod_dir, force_download=args.force)
        
        print(f"\n{'='*80}")
        print("Step 2: Downloading DEV data")
        print(f"{'='*80}\n")
        dev_dir = "./dev_detailed"
        download_detailed_data_for_comparison(dev_table, dev_dir, force_download=args.force)
