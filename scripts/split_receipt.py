...
def setup_environment() -> Dict[str, str]:
    ...
    try:
        ...
        if not table_name:
            table_name = env.get("dynamodb_table_name") or env.get(
                "receipts_table_name"
            )
            if table_name:
        ...
        if not site_bucket:
            ...
            site_bucket = env.get("cdn_bucket_name") or env.get(
                "site_bucket_name"
            )
            if site_bucket:
...
def recluster_receipt_lines(
    ...
    Args:
        ...
        x_eps: Epsilon for X-axis DBSCAN (default: 0.08 = 8% of image width)
        angle_tolerance: Angle difference tolerance in degrees for splitting clusters (default: 3.0)
        vertical_gap_threshold: Split cluster if vertical gap > this fraction of image height (default: 0.15 = 15%)
        reassign_x_proximity_threshold: X-proximity threshold for reassignment phase (default: 0.1 = 10% of image width)
        reassign_y_proximity_threshold: Y-proximity threshold for reassignment phase (default: 0.15 = 15% of image height)
        vertical_proximity_threshold: Vertical proximity threshold for vertical stacking (default: 0.05 = 5% of image height)
        vertical_x_proximity_threshold: X-proximity threshold for vertical stacking (default: 0.1 = 10% of image width)
        merge_min_score: Minimum coherence score to merge clusters (default: 0.5)
        merge_x_proximity_threshold: Don't merge if clusters are > this fraction apart horizontally (default: 0.4 = 40%)
        join_iou_threshold: IoU threshold for joining overlapping clusters (default: 0.1)

    Current default values (tuned for image-level OCR):
        ...
        - join_iou_threshold: 0.1

    For receipt-level OCR (more accurate coordinates), you may want to:
        - Increase x_eps (e.g., 0.12-0.15) to create larger initial clusters
        - Increase merge_x_proximity_threshold (e.g., 0.5-0.6) to allow merging clusters further apart
        - Increase vertical_gap_threshold (e.g., 0.20-0.25) to be less aggressive about splitting
    """
    # Phase 1: X-axis clustering with epsilon
    ...
    unassigned_lines = [
        ...
        if line.line_id not in all_clustered_line_ids
    ]
        ...
        for unassigned_line in unassigned_lines:
            ...
            for cluster_id, cluster_lines in cluster_dict.items():
                ...
                if cluster_x_coords and cluster_y_coords:
                    cluster_mean_x = sum(cluster_x_coords) / len(
                        cluster_x_coords
                    )
                    cluster_mean_y = sum(cluster_y_coords) / len(
                        cluster_y_coords
                    )
            ...
            if nearest_cluster_id is not None:
                ...
                new_cluster_id = (
                    max(cluster_dict.keys()) + 1 if cluster_dict else 1
                )
...
def calculate_receipt_bounds(
    ...
    receipt_min_y = (
        original_receipt.bottom_left["y"] * image_height
    )  # Bottom in OCR space
    receipt_max_y = (
        original_receipt.top_left["y"] * image_height
    )  # Top in OCR space
    ...
    for word in words:
        ...
        word_bottom_left_x = (
            receipt_min_x + word.bottom_left["x"] * receipt_width
        )
        word_bottom_left_y = (
            receipt_min_y + word.bottom_left["y"] * receipt_height
        )
        word_bottom_right_x = (
            receipt_min_x + word.bottom_right["x"] * receipt_width
        )
        word_bottom_right_y = (
            receipt_min_y + word.bottom_right["y"] * receipt_height
        )
    ...
    return {
        "top_left": {
            ...
            "y": min(image_height, max_y)
            / image_height,  # Top in OCR space (larger y)
        },
        "top_right": {
        ...
        "bottom_left": {
            "x": max(0, min_x) / image_width,
            "y": max(0, min_y)
            / image_height,  # Bottom in OCR space (smaller y)
        },
        "bottom_right": {
            "x": min(image_width, max_x) / image_width,
...
def create_split_receipt_image(
    ...
    try:
        ...
        top_left_y = int(
            image_height - bounds["top_left"]["y"] * image_height
        )  # Top in OCR = convert to image space
        bottom_right_x = int(bounds["bottom_right"]["x"] * image_width)
        bottom_right_y = int(
            image_height - bounds["bottom_left"]["y"] * image_height
        )  # Bottom in OCR = convert to image space
        ...
        bottom_right_y = max(
            top_left_y + 1, min(bottom_right_y, actual_height)
        )

        # Ensure valid crop region
        if bottom_right_x <= top_left_x or bottom_right_y <= top_left_y:
            print(
                f"⚠️  Invalid crop region: ({top_left_x}, {top_left_y}) to ({bottom_right_x}, {bottom_right_y})"
            )
            return None
...
def create_split_receipt_records(
    ...
    receipt = Receipt(
        ...
        raw_s3_key=(
            f"raw/{image_id}_RECEIPT_{new_receipt_id:05d}.png"
            if receipt_image
            else ""
        ),
        ...
        sha256=(
            calculate_sha256_from_bytes(receipt_image.tobytes())
            if receipt_image
            else original_receipt.sha256
        ),
        ...
        or original_receipt.cdn_webp_s3_key,
    ...
    try:
        transform_coeffs, orig_receipt_width, orig_receipt_height = (
            original_receipt.get_transform_to_image(image_width, image_height)
        )
    except ImportError:
        raise ValueError(
            "Transform methods not available - required for coordinate transformation"
        )
    ...
    new_receipt_min_y_abs = (
        bounds["bottom_left"]["y"] * image_height
    )  # Bottom in OCR space
    new_receipt_max_y_abs = (
        bounds["top_left"]["y"] * image_height
    )  # Top in OCR space
    ...
    def img_to_receipt_coord(img_x, img_y):
        ...
        receipt_x = (
            (img_x - new_receipt_min_x_abs) / new_receipt_width_abs
            if new_receipt_width_abs > 0
            else 0.0
        )
        receipt_y = (
            (ocr_y - new_receipt_min_y_abs) / new_receipt_height_abs
            if new_receipt_height_abs > 0
            else 0.0
        )
    ...
    for old_line_id in sorted(words_by_line.keys()):
        line_words = sorted(
            words_by_line[old_line_id], key=lambda w: w.word_id
        )
        original_line = next(
        ...
        line_top_left_x, line_top_left_y = img_to_receipt_coord(
            line_tl_img["x"], line_tl_img["y"]
        )
        line_top_right_x, line_top_right_y = img_to_receipt_coord(
            line_tr_img["x"], line_tr_img["y"]
        )
        line_bottom_left_x, line_bottom_left_y = img_to_receipt_coord(
            line_bl_img["x"], line_bl_img["y"]
        )
        line_bottom_right_x, line_bottom_right_y = img_to_receipt_coord(
            line_br_img["x"], line_br_img["y"]
        )
        ...
        line_min_x_receipt = min(
            line_top_left_x,
            line_top_right_x,
            line_bottom_left_x,
            line_bottom_right_x,
        )
        line_max_x_receipt = max(
            line_top_left_x,
            line_top_right_x,
            line_bottom_left_x,
            line_bottom_right_x,
        )
        line_min_y_receipt = min(
            line_top_left_y,
            line_top_right_y,
            line_bottom_left_y,
            line_bottom_right_y,
        )
        line_max_y_receipt = max(
            line_top_left_y,
            line_top_right_y,
            line_bottom_left_y,
            line_bottom_right_y,
        ...
        receipt_line = ReceiptLine(
            ...
            bounding_box={
                ...
                "width": (line_max_x_receipt - line_min_x_receipt)
                * new_receipt_width_abs,
                "height": (line_max_y_receipt - line_min_y_receipt)
                * new_receipt_height_abs,
            },
    ...
    for old_line_id in sorted(words_by_line.keys()):
        ...
        for word in line_words:
            ...
            word_top_left_x, word_top_left_y = img_to_receipt_coord(
                word_tl_img["x"], word_tl_img["y"]
            )
            word_top_right_x, word_top_right_y = img_to_receipt_coord(
                word_tr_img["x"], word_tr_img["y"]
            )
            word_bottom_left_x, word_bottom_left_y = img_to_receipt_coord(
                word_bl_img["x"], word_bl_img["y"]
            )
            word_bottom_right_x, word_bottom_right_y = img_to_receipt_coord(
                word_br_img["x"], word_br_img["y"]
            )

            # Calculate bounding box in new receipt space
            word_min_x_receipt = min(
                word_top_left_x,
                word_top_right_x,
                word_bottom_left_x,
                word_bottom_right_x,
            )
            word_max_x_receipt = max(
                word_top_left_x,
                word_top_right_x,
                word_bottom_left_x,
                word_bottom_right_x,
            )
            word_min_y_receipt = min(
                word_top_left_y,
                word_top_right_y,
                word_bottom_left_y,
                word_bottom_right_y,
            )
            word_max_y_receipt = max(
                word_top_left_y,
                word_top_right_y,
                word_bottom_left_y,
                word_bottom_right_y,
            ...
            receipt_word = ReceiptWord(
                ...
                bounding_box={
                    ...
                    "width": (word_max_x_receipt - word_min_x_receipt)
                    * new_receipt_width_abs,
                    "height": (word_max_y_receipt - word_min_y_receipt)
                    * new_receipt_height_abs,
                },
                ...
                bottom_right={
                    "x": word_bottom_right_x,
                    "y": word_bottom_right_y,
                },
...
def migrate_receipt_word_labels(
    ...
    for label in original_labels:
        ...
        new_label = ReceiptWordLabel(
            ...
            or f"Migrated from receipt {label.receipt_id}, word {label.word_id}",
            timestamp_added=datetime.now(timezone.utc),
...
def save_records_locally(
    ...
    for i, result in enumerate(split_results, start=1):
        ...
        with open(receipt_dir / "lines.json", "w") as f:
            json.dump(
                [dict(line) for line in result["receipt_lines"]],
                f,
                indent=2,
                default=str,
            )
        ...
        with open(receipt_dir / "words.json", "w") as f:
            json.dump(
                [dict(word) for word in result["receipt_words"]],
                f,
                indent=2,
                default=str,
            )
        ...
        if result["receipt_letters"]:
            with open(receipt_dir / "letters.json", "w") as f:
                json.dump(
                    [dict(letter) for letter in result["receipt_letters"]],
                    f,
                    indent=2,
                    default=str,
                )
        ...
        if result["receipt_labels"]:
            with open(receipt_dir / "labels.json", "w") as f:
                json.dump(
                    [dict(label) for label in result["receipt_labels"]],
                    f,
                    indent=2,
                    default=str,
                )
            ...
            json.dump(
                {
                    ...
                    "word_id_map": {
                        f"{k[0]}_{k[1]}": v
                        for k, v in result["word_id_map"].items()
                    },
                },
                f,
                indent=2,
...
def create_embeddings_and_compaction_run(
    ...
    try:
            ...
            try:
                ...
                if chroma_client_path.exists():
                    spec = importlib.util.spec_from_file_location(
                        "chroma_client", str(chroma_client_path)
                    )
                    if spec and spec.loader:
                        chroma_client_module = importlib.util.module_from_spec(
                            spec
                        )
                        ...
                        raise ImportError(
                            "Could not load chroma_client module"
                        )
                else:
                    raise ImportError(
                        f"ChromaClient not found at {chroma_client_path}"
                    )
        ...
        from receipt_label.merchant_resolution.embeddings import (
            upsert_embeddings,
        )
        ...
        if not receipt_lines or not receipt_words:
            print(
                f"⚠️  No lines/words found for receipt {receipt_id}; skipping embedding"
            )
            return None
...
def delete_receipt_embeddings_from_chromadb(
    ...
    try:
        ...
        print(
            f"   ⚠️  ChromaDB deletion not available (receipt_chroma not installed)"
        )
    except Exception as e:
...
def main():
    ...
    if not chromadb_bucket and not args.skip_embedding:
        print(
            f"⚠️  CHROMADB_BUCKET not set; use --chromadb-bucket or --skip-embedding"
        )
    ...
    original_receipt_labels, _ = client.list_receipt_word_labels_for_receipt(
        image_id, original_receipt_id
    )

    print(
        f"   Receipt: {len(original_receipt_lines)} lines, {len(original_receipt_words)} words"
    )
    ...
    if len(cluster_dict) < 2:
        print(
            f"⚠️  Only {len(cluster_dict)} cluster(s) found - nothing to split!"
        )
        return
    ...
    if (
        ...
        and image_entity.raw_s3_bucket
        and image_entity.raw_s3_key
    ):
        try:
            print(
            ...
            print(
                f"   ✅ Loaded image: {actual_image_width}x{actual_image_height}"
            )

            # Verify dimensions match DynamoDB (warn if they don't)
            if (
                actual_image_width != image_entity.width
                or actual_image_height != image_entity.height
            ):
                print(f"⚠️  Image dimensions mismatch!")
                print(
                    f"   DynamoDB: {image_entity.width}x{image_entity.height}"
                )
            ...
            print(
                f"   Receipt images will not be created, but data will still be split"
            )
            print(
                f"   Using DynamoDB dimensions: {actual_image_width}x{actual_image_height}"
            )
    ...
    if not final_site_bucket:
        raise ValueError(
            "SITE_BUCKET not set - cannot upload receipt images to CDN"
        )

    for cluster_id, cluster_lines in sorted(cluster_dict.items()):
        print(
            f"   Creating receipt {new_receipt_id} from cluster {cluster_id}..."
        )
        ...
        print(
            f"      Receipt {new_receipt_id}: {len(result['receipt_lines'])} lines, "
    ...
    if not args.skip_embedding and chromadb_bucket:
        ...
        merchant_name = None
        try:
            receipt_metadata = client.get_receipt_metadata(
                image_id, original_receipt_id
            )
            if receipt_metadata:
                merchant_name = (
                    receipt_metadata.merchant_name
                    or receipt_metadata.canonical_merchant_name
                )
        except Exception:
            ...
            for result in split_results:
                ...
                if receipt_id in compaction_completed:
                    ...
                    try:
                        ...
                        print(
                            f"   ✅ Compaction completed for receipt {receipt_id}"
                        )
                    except (TimeoutError, RuntimeError) as e:
                        print(
                            f"⚠️  Compaction failed or timed out for receipt {receipt_id}: {e}"
                        )
                        print(
                            f"   Will add labels anyway - they'll update when compaction completes later"
                        )
        else:
            print(
                f"⚠️  No embeddings created; labels will be added but may not update ChromaDB"
            )
    else:
        ...
        print(
            f"   Labels will be added, but embeddings may not exist in ChromaDB yet"
        )
    ...
    for result in split_results:
        ...
        if result["receipt_labels"]:
            ...
            for i in range(0, total_labels, CHUNK_SIZE):
                ...
                if receipt_id in compaction_completed:
                    print(
                        f"   Adding labels chunk {chunk_num}/{total_chunks} ({len(chunk)} labels) for receipt {receipt_id} (compaction completed)..."
                    )
                    client.add_receipt_word_labels(chunk)
                elif not args.skip_embedding:
                    print(
                        f"   Adding labels chunk {chunk_num}/{total_chunks} ({len(chunk)} labels) for receipt {receipt_id} (compaction pending)..."
                    )
                    client.add_receipt_word_labels(chunk)
                else:
                    print(
                        f"   Adding labels chunk {chunk_num}/{total_chunks} ({len(chunk)} labels) for receipt {receipt_id} (embedding skipped)..."
                    )
                    client.add_receipt_word_labels(chunk)

            if receipt_id in compaction_completed:
                print(
                    f"      ✅ Added {total_labels} labels (compaction run: {compaction_completed[receipt_id]})"
                )
            elif not args.skip_embedding:
                print(
                    f"      ✅ Added {total_labels} labels (will update when compaction completes)"
                )
            else:
                print(
                    f"      ✅ Added {total_labels} labels (will update when embeddings are created)"
                )
        else:
    ...
    if args.delete_original and compaction_completed:
        ...
        if not all_compaction_complete:
            print(
                f"⚠️  Some compaction runs did not complete - original receipt will NOT be deleted"
            )
            print(
                f"   You can manually delete it later after compaction completes"
            )

    # Delete original receipt and its embeddings (if compaction completed or skipped)
    if args.delete_original and (
        not compaction_completed or all_compaction_complete
    ):
        ...
        if chromadb_bucket and not args.skip_embedding:
            try:
                ...
                print(
                    f"   Compactor will handle deletion automatically when Receipt is deleted"
                )
        ...
        try:
            metadata = client.get_receipt_metadata(
                image_id, original_receipt_id
            )
            if metadata:
            ...
            if runs:
                ...
                for run in runs:
                    client.delete_compaction_run(
                        image_id, original_receipt_id, run.run_id
                    )
