...
def get_dataset_stats(table_name: str, output_file: str = None) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "class_imbalance": {
            ...
            "max_label": max(entity_labels.items(), key=lambda x: x[1])[0] if entity_labels else None,
            "min_label": min(entity_labels.items(), key=lambda x: x[1])[0] if entity_labels else None,
        },
        "available_labels": list(CORE_LABELS.keys()),
        "labels_with_data": [label for label, count in label_counts.items() if count > 0],
        "labels_without_data": [label for label, count in label_counts.items() if count == 0],
    }
