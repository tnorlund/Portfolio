...
class ChromaDBQuerier:
    ...
    def load_chromadb(self, local_dir: str, collection_name: str = "lines") -> ChromaDBClient:
        ...
        try:
            ...
            print(f"✅ Loaded collection '{collection_name}' with {count:,} vectors")
            ...
            try:
                ...
                if all_collections:
                    ...
                    if all_collections:
                        ...
                        collection = client.get_collection(collection_name)
                        count = collection.count()
                        print(f"  Using collection '{collection_name}' with {count:,} vectors")
    ...
    def query_vectors(
        ...
        for query_text in query_texts:
            ...
            for i in range(len(results['ids'][0])):
                ...
                if result['metadata']:
                    print(f"     Merchant: {result['metadata'].get('merchant_name', 'Unknown')}")
                    print(f"     Confidence: {result['metadata'].get('confidence', 'N/A')}")
                    if 'prev_line' in result['metadata'] and result['metadata']['prev_line']:
                        print(f"     Previous line: {result['metadata']['prev_line']}")
                    if 'next_line' in result['metadata'] and result['metadata']['next_line']:
                        print(f"     Next line: {result['metadata']['next_line']}")
            
            results_dict[query_text] = query_results
        
        return results_dict
    
    def display_sample_data(self, client: ChromaDBClient, collection_name: str, limit: int = 10):
...
def main():
    ...
    source_group.add_argument(
        '--snapshot',
        action='store_true',
        help='Download the latest snapshot'
    )
    ...
    try:
        ...
        # Load ChromaDB
        client, collection_name = querier.load_chromadb(str(download_dir), args.collection)
