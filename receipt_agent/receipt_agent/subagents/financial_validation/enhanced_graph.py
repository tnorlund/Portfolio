...
def create_enhanced_financial_validation_tools(
    ...
    def detect_currency() -> dict:
        ...
        return {
            "currency": detected,
            "evidence": evidence,
            "confidence": 0.9 if len(evidence) > 0 and "Defaulting" not in evidence[0] else 0.5,
        }
    ...
    def validate_grand_total_math() -> dict:
        ...
        for discount_type in ["DISCOUNT", "COUPON"]:
            if discount_type in financial_labels:
                discounts += sum(abs(item["numeric_value"] or 0) for item in financial_labels[discount_type])
        ...
        if grand_total is not None and subtotal is not None:
            ...
            issues.append({"type": "missing_grand_total", "message": "No GRAND_TOTAL label found"})
            is_valid = False
        elif subtotal is None:
            issues.append({"type": "missing_subtotal", "message": "No SUBTOTAL label found"})
            is_valid = False
        ...
        return {
            ...
            "values": {
                ...
                "tax": tax,
                "discounts": discounts,
                "expected_total": subtotal + tax - discounts if subtotal is not None else None,
            }
        }
    ...
    def validate_subtotal_math() -> dict:
        ...
        if "LINE_TOTAL" in financial_labels:
            line_totals = [
                item["numeric_value"] for item in financial_labels["LINE_TOTAL"]
        ...
        if subtotal is not None and line_totals:
            ...
            issues.append({"type": "missing_subtotal", "message": f"No SUBTOTAL found but {len(line_totals)} LINE_TOTAL values exist"})
            is_valid = False
        elif subtotal is not None and not line_totals:
            issues.append({"type": "missing_line_totals", "message": f"SUBTOTAL exists ({subtotal}) but no LINE_TOTAL values found"})
            is_valid = False
    ...
    def validate_line_item_math() -> dict:
        ...
        for label in labels:
            ...
            line_groups[line_id].append({
                ...
                "text": text,
                "numeric_value": extract_number(text) if label.get("label") in ["QUANTITY", "UNIT_PRICE", "LINE_TOTAL"] else None,
            })
        ...
        for line_id, line_labels in line_groups.items():
            ...
            line_total_items = [item for item in line_labels if item["label"] == "LINE_TOTAL"]
            ...
            quantity_items = [item for item in line_labels if item["label"] == "QUANTITY"]
            unit_price_items = [item for item in line_labels if item["label"] == "UNIT_PRICE"]
            
            quantity = quantity_items[0]["numeric_value"] if quantity_items else None
            unit_price = unit_price_items[0]["numeric_value"] if unit_price_items else None
            ...
            line_info = {
                ...
                "has_product_name": any(item["label"] == "PRODUCT_NAME" for item in line_labels),
            }
            ...
            if quantity is not None and unit_price is not None:
                ...
                line_info["math_valid"] = abs(line_total - calculated_total) <= tolerance
            ...
            if "LINE_TOTAL" in label_set:
                ...
                if missing:
                    missing_fields.append({
                        ...
                        "severity": "high" if "PRODUCT_NAME" in missing else "medium",
                    })
        
        return {
            "missing_fields": missing_fields,
            "lines_with_issues": len(missing_fields),
            "total_line_items": len([line_id for line_id, labels in line_groups.items() if "LINE_TOTAL" in labels]),
        }
...
async def run_enhanced_financial_validation(
    ...
    tools, tool_state = create_enhanced_financial_validation_tools(
        state_holder["receipt"], 
        table_structure
    )
