...
from receipt_agent.tools.receipt_grouper import create_receipt_grouper_tools, ImageContext
...
class GroupingState(BaseModel):
    ...
    # Terminal state
    grouping: Optional[dict] = Field(default=None, description="Final grouping when complete")
    ...
    def agent_node(state: GroupingState) -> dict:
        ...
        if hasattr(response, 'tool_calls') and response.tool_calls:
            ...
            if 'submit_grouping' in tool_names:
                logger.info("submit_grouping called - workflow should end after tool execution")
        else:
