...
class ResilientLLM(LLMInvoker):
    """
    Deprecated: Use LLMInvoker instead.

    This class is kept for backward compatibility. It wraps LLMInvoker
    with the old ResilientLLM interface.
    """
    ...
    def __init__(
        ...
        fallback_free_llm: Any = None,
        fallback_paid_llm: Any = None,
        **kwargs,
    ):
        """Initialize with backward-compatible parameters."""
        # Use the first non-None LLM provided
        llm = primary_llm or fallback_llm or fallback_free_llm or fallback_paid_llm
        if llm is None:
            llm = create_llm()
        ...
        self.fallback_free_calls = 0
        self.fallback_free_successes = 0
        self.fallback_paid_calls = 0
        self.fallback_paid_successes = 0
        self.all_failed = 0
        self.both_failed = 0

    def invoke(self, messages: Any, config: Optional[dict] = None, **kwargs) -> Any:
        """Invoke with backward-compatible stats tracking."""
        self.primary_calls += 1
        try:
            result = super().invoke(messages, config=config, **kwargs)
    ...
    async def ainvoke(
        ...
        """Async invoke with backward-compatible stats tracking."""
        self.primary_calls += 1
        try:
            result = await super().ainvoke(messages, config=config, **kwargs)
            self.primary_successes += 1
            return result
        except LLMRateLimitError:
            self.all_failed += 1
    ...
    def get_stats(self) -> dict[str, Any]:
        """Get statistics with backward-compatible keys."""
        ...
        return {
            **base_stats,
            ...
            "fallback_rate": 0.0,
            "paid_fallback_rate": 0.0,
            "overall_success_rate": (
                self.primary_successes / self.primary_calls
