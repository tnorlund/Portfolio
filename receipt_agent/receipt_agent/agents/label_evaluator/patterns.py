...
def compute_merchant_patterns(
    ...
    if max_relationship_dimension >= 3:
        ...
        logger.debug(
            "Selected %s label %s-tuples for constellation patterns",
            len(selected_tuples),
            max_relationship_dimension,
        )

    # NOTE: label_pair_geometry and batch-specific geometry computation removed.
    # The detection rules that consumed them (check_geometric_anomaly,
    # check_position_anomaly, check_constellation_anomaly, check_unexpected_label_pair)
    # are disabled due to 89-100% false positive rates.
    # Re-add geometry computation when those rules are re-enabled.

    logger.info(
        "Skipped label pair geometry (disabled detection rules). "
        "Observed %s unique label pairs.",
        len(all_pair_frequencies),
    )
