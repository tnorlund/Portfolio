...
# ---------------------------------------------------------------------------
# Reconciliation passes ported from sprouts_parameterization.py so the GENERIC
# merchant path benefits from the same objective-verifier fixes. Local copies
# (no cross-file import); the Sprouts-specific canonical-name test is generalized
# to the candidate's merchant_name. All passes operate on the parallel-array
# shape (tokens, bboxes, ner_tags) that `_flatten_lines` produces.
# ---------------------------------------------------------------------------
...
def _sanitize_entity_labels(
    ...
    run that the objective `labels_valid` check would flag is set entirely to
    ``O``. Tag-only: no token is dropped, no bbox is changed (geometry/arithmetic
    untouched). Orphan ``I-`` tags are re-promoted to ``B-`` to keep valid BIO.
    """
    tags = list(ner_tags)
...
def _dedupe_payment_blocks(
    ...
    Cloned/augmented base receipts can carry two tender sections; the verifier
    collects every ``[X*x]{4,}`` token's trailing-4 and fails on >1 distinct tail.
    ...
    tokens. Total/geometry rows are never dropped, so arithmetic and bbox validity
    are preserved.
    """
    tokens = list(tokens)
    bboxes = list(bboxes)
...
def _deoverlap_line_words(
    ...
    Pass 1 sweeps left-to-right and either drops stacked unlabeled ``O`` junk or
    nudges labeled content right to clear overlaps. Pass 2 re-opens a real
    word-gap on every line. Pure geometry: stays clamped in ``[0, 1000]`` and
    preserves right-aligned price columns via a monotone running shift.
    """
...
def _flatten_lines(
    ...
    # (ported from the Sprouts path; mirrors its ordering):
    #   1. collapse duplicate header blocks -> one MERCHANT_NAME / ADDRESS_LINE.
    #   2. collapse duplicate payment/card blocks -> one masked-card trailing-4.
    #   3. de-overlap AND re-space words sharing a visual line (footer squish).
    #   4. content-aware label sanitization: demote-to-O mislabeled entity runs.
    tokens, bboxes, tags = _dedupe_header_blocks(
        tokens, bboxes, tags, merchant_name
    )
    tokens, bboxes, tags = _dedupe_payment_blocks(tokens, bboxes, tags)
    tokens, bboxes, tags = _deoverlap_line_words(tokens, bboxes, tags)
    tokens, bboxes, tags = _sanitize_entity_labels(tokens, bboxes, tags)
    return tokens, bboxes, tags
