...
def create_label_validation_tools(
    ...
    def get_word_context() -> dict:
        ...
        **NOTE**: Word context is already provided in the initial prompt above.
        ...
        try:
            ...
            logger.error("Error getting word context: %s", e)
            return {"error": str(e)}
    ...
    def get_merchant_place() -> dict:
        ...
        **NOTE**: Merchant place data is already provided in the initial prompt above.
        ...
        try:
            ...
            logger.error("Error getting place metadata: %s", e)
        ...
        try:
            ...
            if target_embedding is None:
                ...
                if not target_word:
                    return [
                        {
                            "error": f"Word '{ctx.word_text}' not found in DynamoDB"
                        }
                    ]
            ...
            for _doc_id, doc, meta, dist in zip(
                ...
                try:
                    ...
                    if (
                        ...
                        if similar_line:
                            words_in_line = (
                                dynamo_client.list_receipt_words_from_line(
                                    receipt_id=similar_receipt_id,
                                    image_id=similar_image_id,
                                    line_id=similar_line_id,
                                )
                        ...
                        if all_similar_lines and similar_line:
                            ...
                            start_idx = max(
                                0, target_line_index - N_LINES_CONTEXT
                            )
                            end_idx = min(
                                len(sorted_lines),
                            ...
                            for i, receipt_line in enumerate(context_lines):
                                ...
                                if i > 0:
                                    ...
                                    curr_centroid = (
                                        receipt_line.calculate_centroid()
                                    )
                            ...
                            if similar_place:
                                similar_receipt_place = {
                                    ...
                                    "phone_number": getattr(
                                        similar_place, "phone_number", None
                                    ),
                                    "website": getattr(
                                        similar_place, "website", None
                                    ),
                                }
                        except Exception as e:
                            logger.debug(
                                "Could not fetch receipt place for similar word: %s",
                                e,
                            )
                        ...
                        try:
                            ...
                            logger.debug(
                                "Could not fetch audit trail for similar word: %s",
                                e,
                            )
                    ...
                    logger.debug(
                        "Could not fetch context for similar word: %s",
                        e,
                    )
            ...
            logger.info(
                "search_similar_words returned %s results",
                len(output),
            )
            return output

        except Exception as e:
            logger.error("Error searching similar words: %s", e)
            return [{"error": str(e)}]
    ...
    def get_all_labels_for_word() -> dict:
        ...
        try:
            ...
            if current_label:
                ...
                while (
                    label_to_trace and label_to_trace.label_consolidated_from
                ):
                ...
                consolidation_chain.append(
                    {
                        ...
                        "action": (
                            ...
                            if current_label.validation_status
                            == "NEEDS_REVIEW"
                            else "current"
                        ),
                    }
            ...
            logger.error("Error getting all labels for word: %s", e)
            return {"error": str(e)}
    ...
    def get_labels_on_receipt() -> dict:
        ...
        try:
            ...
            logger.error("Error getting labels on receipt: %s", e)
            return {"error": str(e)}
    ...
    def submit_decision(
        ...
        logger.info(
            "Decision submitted: %s (confidence=%.2f%%)",
            decision,
            confidence * 100.0,
        )
