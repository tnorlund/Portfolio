...
from receipt_agent.agents.label_suggestion.tools import (
    ReceiptContext,
  ...
  "reasoning": "explanation"
                ...
                try:
                    ...
                    if json_match:
                        ...
                        if (
                            llm_result.get("label_type")
                            and llm_result["label_type"] in CORE_LABELS
                        ):
                            suggestions.append(
                                {
                                    "word_id": word_id,
    ...
    if suggestions:
        ...
        return {
            ...
            "performance": {
                ...
                "time_per_word_seconds": (
                    total_processing_time / len(unlabeled_words)
                    if unlabeled_words
                    else 0
                ),
                "avg_chroma_query_time": (
                    sum(chroma_query_times) / len(chroma_query_times)
                    if chroma_query_times
                    else 0
                ),
