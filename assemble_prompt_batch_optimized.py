...
from receipt_dynamo.entities import ReceiptWord, ReceiptWordLabel, ReceiptLine, ReceiptMetadata
...
class BatchedPromptAssembler:
    ...
    def __init__(self, dynamo_client: DynamoClient, chroma_client: ChromaDBClient):
    ...
    def assemble_prompts(
        ...
        context_lines_map = self._batch_get_context_lines(labels, context_lines)
        logger.info("Retrieved context lines for %d receipts", len(context_lines_map))
        ...
        logger.info("Retrieved %d embeddings from ChromaDB", len(embeddings_map))
        
        # Step 5: Assemble all ValidationPromptData objects
        prompts = self._assemble_prompt_data(
            labels, target_words, context_lines_map, 
            metadata_map, embeddings_map, context_lines
        )
        logger.info("Assembled %d validation prompts", len(prompts))
        
        return prompts
    
    def _batch_get_target_words(self, labels: List[ReceiptWordLabel]) -> Dict[Tuple[str, int, int, int], ReceiptWord]:
        ...
        for label in labels:
            keys.append({
                ...
                "SK": {"S": f"RECEIPT#{label.receipt_id:05d}#LINE#{label.line_id:05d}#WORD#{label.word_id:05d}"},
            })
    ...
    def _batch_get_context_lines(
        ...
        for label in labels:
            ...
            for line_id in range(min_line_id, max_line_id + 1):
                line_indices_set.add((label.image_id, label.receipt_id, line_id))
        ...
        receipt_lines = self.dynamo_client.get_receipt_lines_by_indices(line_indices)
    ...
    def _batch_get_receipt_metadata(self, labels: List[ReceiptWordLabel]) -> Dict[Tuple[str, int], ReceiptMetadata]:
        ...
        metadata_list = self.dynamo_client.get_receipt_metadatas_by_indices(indices)
    ...
    def _batch_get_embeddings(self, target_words: Dict[Tuple[str, int, int, int], ReceiptWord]) -> Dict[str, Dict]:
        ...
        if response.get("ids"):
            for i, chroma_id in enumerate(response["ids"]):
                embeddings_map[chroma_id] = {
                    ...
                    "metadata": response["metadatas"][i] if response["metadatas"] else None,
                }
...
def display_receipt_context_from_prompt_data(prompt_data: ValidationPromptData) -> None:
...
def display_validation_results_from_prompt_data(
    ...
    print(f"Receipt context: Line {target_word.line_id}, Word {target_word.word_id}")

    # Display receipt context with visual alignment
    display_receipt_context_from_prompt_data(prompt_data)

    print(f"\n🧠 SEMANTIC SIMILARITY:")
    print(f"  Similar words where '{prompt_data.label_to_validate}' was VALID: ({len(words_with_label_valid)} found)")
    if words_with_label_valid:
        for word_info in words_with_label_valid[:3]:
            print(f"    ✓ '{word_info['text']}' (distance: {word_info['distance']:.3f}) - {word_info['merchant']}")
    else:
        print("    None found")

    print(f"  Similar words where '{prompt_data.label_to_validate}' was INVALID: ({len(words_with_label_invalid)} found)")
    ...
    if valid_count > invalid_count:
        ...
        print(f"     (Based on {valid_count} valid vs {invalid_count} invalid similar examples)")
    elif invalid_count > valid_count:
        print(f"\n   → Suggestion: '{prompt_data.label_to_validate}' is likely INVALID for '{target_word.text}'")
        print(f"     (Based on {invalid_count} invalid vs {valid_count} valid similar examples)")
    else:
        print(f"\n   → No clear suggestion - needs manual review")
        print(f"     ({valid_count} valid vs {invalid_count} invalid similar examples)")
...
def ensure_chroma_snapshots(pulumi_env: Dict) -> None:
    ...
    if not chroma_s3_bucket:
        raise ValueError("ChromaDB bucket name not found in Pulumi environment")
    ...
    if not LOCAL_CHROMA_WORD_PATH.exists() or not any(LOCAL_CHROMA_WORD_PATH.iterdir()):
        ...
        try:
            ...
            logger.warning("Could not download word embeddings using SnapshotManager: %s", e)
...
def main():
    ...
    try:
        ...
        pulumi_env = load_env()
        if not pulumi_env:
            raise ValueError("Pulumi environment variables not found, try adding the Pulumi API key")
        ...
        logger.info("DynamoDB client created in %.2fs", time.time() - dynamo_start)

        # Query for labels that need validation
        query_start = time.time()
        logger.info("Querying for labels that need validation...")
        receipt_word_labels, last_evaluated_key = dynamo_client.get_receipt_word_labels_by_label(
            label=LABEL_TO_VALIDATE,
            limit=1000,
        )
        logger.info("Labels query completed in %.2fs", time.time() - query_start)

        labels_that_need_validation = [
            label for label in receipt_word_labels
        ...
        prompts = assembler.assemble_prompts(labels_that_need_validation, context_lines=3)
        logger.info("Batch assembly completed in %.2fs", time.time() - assembler_start)
        ...
        for i, prompt_data in enumerate(prompts, 1):
            try:
                ...
                words_with_label_valid, words_with_label_invalid = analyze_label_similarity(
