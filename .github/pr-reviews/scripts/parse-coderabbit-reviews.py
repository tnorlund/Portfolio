...
def extract_code_locations(body: str) -> List[Tuple[str, str, str]]:
    ...
    pattern = r'`([^`]+\.(?:py|ts|tsx|js|jsx))`.*?(?:\((\d+(?:-\d+)?)\))?.*?\*\*([^*]+)\*\*'
...
def categorize_issue(description: str) -> str:
    ...
    if any(word in desc_lower for word in ['critical', 'bug', 'breaks', 'incorrect', 'wrong']):
        return 'critical'
        ...
        return 'resolved'
    elif any(word in desc_lower for word in ['consider', 'optional', 'prefer', 'nitpick']):
        return 'suggestion'
    else:
        return 'warning'
...
def main():
    ...
    for review in sorted(reviews, key=lambda r: r['submittedAt'], reverse=True)[:5]:
        ...
        print(f"- **{timestamp}** (commit `{commit_oid}`): "
