...
def main():
    ...
    combined = {
        ...
        "summary": {
            ...
            "reviews_by_author": [
                {"author": k, "count": v}
                for k, v in sorted(
                    reviews_by_author.items(), key=lambda x: x[1], reverse=True
                )
            ],
    ...
    print(
        f"   Inline comments: {combined['summary']['total_inline_comments']}",
        file=sys.stderr,
    )
    print(
        f"   PR comments: {combined['summary']['total_pr_comments']}", file=sys.stderr
    )
    print(
        f"   CodeRabbit reviews: {combined['summary']['coderabbit_reviews']}",
        file=sys.stderr,
    )
    ...
    print(
        f"   python3 scripts/parse-coderabbit-reviews-complete.py {output_file}",
        file=sys.stderr,
    )
